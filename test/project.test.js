...
test('ни на сайте, ни в приложении нет чужих скриптов', () => {
  /* Счётчик посещений (GoatCounter) стоял на странице полтора дня
     и убран: собирать что бы то ни было о российских посетителях
     через зарубежный сервис — разговор с 152-ФЗ, которого не хочется.
     Проверка следит, чтобы чужие адреса не вернулись ни тихой правкой,
     ни чужим советом: страница обещает, что ничего никуда не уходит,
     и обещание должно держаться в разметке, а не на словах. */
  for (const файл of ['index.html', path.join('desktop', 'renderer', 'index.html')]) {
    const html = fs.readFileSync(path.join(КОРЕНЬ, файл), 'utf8');

    /* Скрипты только свои: без адреса (внутренние) или относительным
       путём. Любой https:// в src — это уже чужой сервер. */
    for (const [, src] of html.matchAll(/<script[^>]*\ssrc="([^"]*)"/g)) {
      assert.ok(!/^(https?:)?\/\//.test(src), `${файл}: чужой скрипт ${src}`);
    }

    /* И политика безопасности не пускает наружу никого: единственный
       источник — мы сами (плюс blob: и data: для своих потоков
       и картинок). */
    const политика = html.match(/Content-Security-Policy" content="([^"]*)"/);
    assert.ok(политика, `${файл}: нет политики безопасности`);
    assert.ok(!/https?:\/\//.test(политика[1]),
      `${файл}: в политике появился чужой адрес — ${политика[1]}`);
  }
});
