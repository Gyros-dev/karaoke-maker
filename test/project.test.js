...
test('в приложении нет счётчика посещений', () => {
  /* На сайте стоит GoatCounter — иначе не узнать, приходит ли кто-то:
     GitHub Pages логов не отдаёт. В приложении ему делать нечего:
     оно хвалится тем, что ничего никуда не отправляет, и это должно
     быть правдой до последней строчки. Вырезает его sync-renderer;
     проверка стережёт, чтобы вырезание не отвалилось молча. */
  const сайт = fs.readFileSync(path.join(КОРЕНЬ, 'index.html'), 'utf8');
  const приложение = fs.readFileSync(
    path.join(КОРЕНЬ, 'desktop', 'renderer', 'index.html'), 'utf8');

  assert.match(сайт, /data-goatcounter=/, 'счётчик пропал с самого сайта');
  assert.ok(!/goatcounter|gc\.zgo\.at/.test(приложение),
    'счётчик доехал до приложения — sync-renderer его не вырезал');

  /* И политика безопасности приложения не пускает наружу никого:
     единственный источник — оно само (плюс blob: и data: для своих
     потоков и картинок). */
  const политика = приложение.match(/Content-Security-Policy" content="([^"]*)"/);
  assert.ok(политика, 'в приложении нет политики безопасности');
  assert.ok(!/https?:\/\//.test(политика[1]),
    'в политике приложения появился чужой адрес: ' + политика[1]);
});
