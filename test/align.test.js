...
test('fit: строки за концом песни прижимаются в песню и считаются', () => {
  /* Нейросеть иногда называет времена ЗА концом записи — последним
     словам достаются времена, которых в песне нет. У проверяющего
     строки вставали на 2:24,06 и 2:24,13 при песне 2:24,03: на дорожке
     их не видно, мышью не поправить, в караоке они не загораются
     никогда, а в .lrc уезжали ровно эти времена. */
  const recWords = [
    { text: 'на', start: 7.6, end: 7.9 },
    { text: 'небе', start: 8.1, end: 8.4 },
    { text: 'звёзды', start: 8.6, end: 9.0 },
  ];
  const длина = 8;
  const итог = Align.fit('на\nнебе\nзвёзды', recWords, { duration: длина });
  assert.ok(итог.ok);
  // Ни одна строка не начинается за концом песни
  for (const l of итог.lines) assert.ok(l.time < длина, `строка на ${l.time} при песне ${длина}`);
  // Слова тоже остались внутри
  for (const l of итог.lines) {
    for (const w of l.words) assert.ok(w.end <= длина + 1e-9);
  }
  // Порядок строк прижатие не перепутало
  for (let i = 1; i < итог.lines.length; i++) {
    assert.ok(итог.lines[i].time > итог.lines[i - 1].time);
  }
  // И о тех, кому не хватило места, сказано числом — об этом потом
  // говорится человеку в отчёте о разметке
  assert.strictEqual(итог.статистика.безМеста, 2);
  // Прижатые помечены «на глазок»
  assert.strictEqual(итог.lines[итог.lines.length - 1].сомнительная, true);
});
