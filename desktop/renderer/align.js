...
(function (root, factory) {
  ...
  function fit(text, recWords, opts) {
    ...
    // Сколько секунд занимает слог в этой песне — по самим опорам
    const first = anchors[0];
    const lastA = anchors[anchors.length - 1];
    let syl = 0;
    for (let k = first; k <= lastA; k++) syl += syllables(words[k].text);
    const secPerSyl = syl > 0
      ? Math.max(0.08, Math.min(0.8, (times[lastA].end - times[first].time) / syl))
      : DEF_SYL;

    // Голова: до первой опоры отматываем назад ровно столько, сколько
    // нужно на эти слоги, но не дальше начала записи
    if (first > 0) {
      let need = 0;
      for (let k = 0; k < first; k++) need += syllables(words[k].text) * secPerSyl;
      const start = Math.max(0, times[first].time - need);
