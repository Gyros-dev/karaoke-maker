...
(function (root, factory) {
  ...
  function fit(text, recWords, opts) {
    ...
    /* ---------- Строки, которым не хватило песни ----------

       Текста бывает больше, чем спето: проверяющий вставил текст длиннее
       песни, и последние три строки встали на 2:23,99, 2:24,06 и 2:24,13
       при песне 2:24,03. Две последние начинались уже ПОСЛЕ её конца:
       на дорожке их не видно, мышью не поправить, в караоке они
       не загорятся никогда, а в .lrc уехали ровно эти времена.

       Прижимаем такие строки обратно в песню, по шагу на строку снизу
       вверх (порядок при этом сохраняется), помечаем их «на глазок»
       и считаем, скольким не хватило места, — об этом потом говорится
       человеку прямым текстом. */
    const ШАГ_ХВОСТА = 0.05;
    let безМеста = 0;
    if (duration > 0) {
      let предел = duration - ШАГ_ХВОСТА;
      for (let i = lines.length - 1; i >= 0; i--) {
        const l = lines[i];
        if (l.time <= предел) break;
        безМеста++;
        const сдвиг = предел - l.time;
        l.time += сдвиг;
        l.end = Math.min(duration, l.end + сдвиг);
        l.words.forEach((w) => {
          w.time += сдвиг;
          w.end = Math.min(duration, w.end + сдвиг);
        });
        l.сомнительная = true;
        предел -= ШАГ_ХВОСТА;
      }
    ...
    return {
      ...
      статистика: {
        ...
        // Скольким строкам песни не хватило вовсе (см. выше)
        безМеста,
        строк: lines.length,
      },
    };
