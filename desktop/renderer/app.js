...
/* Число с разрядами по языку песни: по-русски пробелами, по-английски
   запятыми. Своё форматирование, а не toLocaleString, — чтобы разряды
   были такими же, как везде в студии, и чтобы пробел был неразрывным:
   иначе число ломается переносом посередине. */
function счётСРазрядами(n, язык) {
  const цифры = String(Math.round(n));
  const части = [];
  for (let i = цифры.length; i > 0; i -= 3) части.unshift(цифры.slice(Math.max(0, i - 3), i));
  return части.join(язык === 'ru' ? ' ' : ',');
}
...
function побежатьСчёт(цель, язык) {
  ...
  if (reduceMotion.matches) {
    писать(поле, счётСРазрядами(цель, язык));
    return;
  }
  ...
  const шаг = (сейчас) => {
    ...
    писать(поле, счётСРазрядами(цель * плавно, язык));
...
function drawФиналКадра(g2d, W, H, bgImg, прошло, финал, лого) {
  ...
  const кегльПодписи = Math.round(H * 0.028);
  ...
  const плавно = 1 - Math.pow(1 - доля, 3);
  const число = счётСРазрядами(финал.цель * плавно, финал.язык) + ' %';
  y += кегльЧисла * 0.62;
