...
function применитьДеление() {
  ...
  if (!м) return;
  const справка = м.шаг.querySelector(':scope > .editor-hint[open]');
  const минОкон = справка ? МИН_ОКОН_СО_СПРАВКОЙ : МИН_ОКОН;
  ...
  h = Math.min(h, Math.max(60, м.пул - минОкон));
...
/* Развёрнутая справка забирает у шага треть высоты — значит, окнам
   и дорожке досталось меньше, и делить надо заново. Без этого окна
   упирались бы в свой нижний предел, а лишнее вылезало бы за низ
   подложки студии вместе с кнопками «← Текст» / «Караоке →». */
$('editor-hint').addEventListener('toggle', () => {
