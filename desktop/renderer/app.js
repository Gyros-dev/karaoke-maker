...
/* ---------- Переключатель темы: выпадающий список ----------

   Тем стало три, и обойма из трёх кнопок подряд перевешивала в шапке
   всё остальное. Поэтому здесь одна кнопка со значком нынешней темы,
   а по нажатию — список из трёх с названиями. Языковой переключатель
   рядом остаётся парой кнопок: языков два, и обойма для них короче
   любого списка.

   Собирается кодом теми же приёмами, что и языковой (см.
   собратьПереключателиЯзыка): подписи одинаково читаются на любом
   языке, и держать их в разметке незачем.

   С клавиатуры работает как обычное меню: на кнопке Enter, пробел
   или ↓ открывают и ставят фокус на выбранную тему, в списке ↑ и ↓
   ходят по пунктам, Enter выбирает, Esc закрывает и возвращает фокус
   на кнопку. Щёлкнули мимо — тоже закрывается. */
let списокТемОткрыт = false;

function открытьСписокТем(да) {
  списокТемОткрыт = !!да;
  document.querySelectorAll('.theme-switch').forEach((узел) => {
    const кнопка = узел.querySelector('.theme-btn');
    const список = узел.querySelector('.theme-menu');
    if (!кнопка || !список) return;
    список.classList.toggle('hidden', !списокТемОткрыт);
    кнопка.setAttribute('aria-expanded', списокТемОткрыт ? 'true' : 'false');
    if (списокТемОткрыт) {
      const текущий = список.querySelector('.theme-item[aria-checked="true"]')
        || список.querySelector('.theme-item');
...
/* Соседний пункт по кругу: с последнего ↓ ведёт на первый, а с первого
   ↑ — на последний. Так ходят меню в системе, и на трёх пунктах
   упереться в край было бы особенно обидно. */
function перейтиПоСписку(список, откуда, шаг) {
  const пункты = [...список.querySelectorAll('.theme-item')];
...
function собратьПереключателиТемы() {
  document.querySelectorAll('.theme-switch').forEach((узел) => {
    ...
    кнопка.dataset.theme = тема;
    кнопка.setAttribute('aria-haspopup', 'menu');
    кнопка.setAttribute('aria-expanded', 'false');
    кнопка.appendChild(значокSVG(ЗНАЧОК_ТЕМЫ[тема] || 'palette'));
    кнопка.appendChild(значокSVG('caret', 'theme-caret'));
    кнопка.title = t('тема.выбрать', { 'имя': t('тема.' + тема) });
    кнопка.setAttribute('aria-label', кнопка.title);
    узел.appendChild(кнопка);

    const список = document.createElement('div');
    список.className = 'theme-menu hidden';
    список.setAttribute('role', 'menu');
    список.setAttribute('aria-label', t('шапка.тема'));
    ТЕМЫ.forEach((код) => {
      const пункт = document.createElement('button');
      пункт.type = 'button';
      пункт.className = 'theme-item';
      пункт.setAttribute('role', 'menuitemradio');
      пункт.dataset.theme = код;
      пункт.appendChild(значокSVG(ЗНАЧОК_ТЕМЫ[код] || 'palette'));
      const подпись = document.createElement('span');
      подпись.textContent = t('тема.' + код);
      пункт.appendChild(подпись);
      пункт.appendChild(значокSVG('check', 'theme-tick'));
      пункт.title = t('тема.' + код + '.полно');
      пункт.setAttribute('aria-label', пункт.title);
      пункт.setAttribute('aria-checked', код === тема ? 'true' : 'false');
      пункт.addEventListener('click', () => {
        /* Порядок важен: сначала закрываем (пока список ещё тот самый),
           потом ставим тему — установитьТему пересобирает переключатель,
           и прежние узлы уходят вместе со списком. Фокус после этого
           возвращаем на свежую кнопку, иначе он падал бы на body. */
        открытьСписокТем(false);
        установитьТему(код);
        const свежая = узел.querySelector('.theme-btn');
      ...
      пункт.addEventListener('keydown', (e) => {
        ...
        else if (e.key === 'Escape') { e.preventDefault(); открытьСписокТем(false); кнопка.focus(); }
        else if (e.key === 'Tab') открытьСписокТем(false);
      });
      список.appendChild(пункт);
    });
    узел.appendChild(список);

    кнопка.addEventListener('click', () => открытьСписокТем(!списокТемОткрыт));
    кнопка.addEventListener('keydown', (e) => {
      if (e.key === 'ArrowDown' || e.key === 'ArrowUp') {
        e.preventDefault();
        открытьСписокТем(true);
      }
    });
  });
}
...
document.addEventListener('pointerdown', (e) => {
  if (!списокТемОткрыт) return;
  if (e.target && e.target.closest && e.target.closest('.theme-switch')) return;
  открытьСписокТем(false);
});
/* Esc закрывает список откуда угодно: фокус мог уехать с пункта
   (скажем, обратно на кнопку) — а список так и остался бы висеть */
document.addEventListener('keydown', (e) => {
  if (e.key === 'Escape' && списокТемОткрыт) {
    открытьСписокТем(false);
