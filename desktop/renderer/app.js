...
function tickPlayer() {
  ...
  if ($('step-4').classList.contains('active')) {
    ...
    if (audio.playing) проверитьФинал(audio.position());
  }
...
/* Похвала в одну строку: кегль подбираем по ширине окна.

   Фразы разной длины — от «Ого!» до «Спасибо за песню! Браво, ты
   настоящий Karaoke Punch!». Перенос по словам делал из одной похвалы
   то одну строку, то три, и всё, что под ней, прыгало от песни
   к песне. Теперь строка всегда одна, а не влезающую ужимаем кеглем.

   Ставим сразу и ещё раз на следующем кадре: пока окно только что
   показалось, ширины у него ещё нет, и первый замер вышел бы нулевым. */
function подогнатьПохвалу() {
  const поле = $('finale-praise');
  const короб = поле && поле.parentElement;
  if (!поле || !короб) return;
  поле.style.fontSize = '';
  const место = короб.clientWidth;
  if (!место) return;
  const нужно = поле.scrollWidth;
  if (нужно <= место) return;
  const кегль = parseFloat(getComputedStyle(поле).fontSize) || 16;
  поле.style.fontSize = Math.max(11, Math.floor(кегль * (место / нужно))) + 'px';
}

let финалБег = null;
let финалФазы = [];

function спрятатьФинал() {
  const узел = $('finale');
  if (!узел || узел.classList.contains('hidden')) return;
  if (финалБег) { cancelAnimationFrame(финалБег); финалБег = null; }
  финалФазы.forEach(clearTimeout);
  финалФазы = [];
  узел.classList.add('hidden');
  узел.classList.remove('бежит', 'знак', 'лого', 'счёт');
}
...
function показатьФинал() {
  ...
  const цель = Math.floor(ФИНАЛ_СЧЁТ_ОТ + Math.random() * (ФИНАЛ_СЧЁТ_ДО - ФИНАЛ_СЧЁТ_ОТ));

  писать($('finale-praise'), случайнаяПохвала(язык));
  подогнатьПохвалу();
  писать($('finale-sub'), t('финал.подпись', { 'язык': язык }, язык));
  писать($('finale-again'), t('финал.ещё', null, язык));
  писать($('finale-close'), t('финал.закрыть', null, язык));
  писать($('finale-unit'), '%');
  узел.classList.remove('hidden');

  /* Три удара: надпись — логотип — счёт поверх логотипа. Времена те же,
     что и в кадре видео (ФАЗА_ЗНАК, ФАЗА_ЛОГО), чтобы просмотр
     показывал ровно то, что получится в файле. */
  финалФазы.forEach(clearTimeout);
  финалФазы = [];
  узел.classList.remove('знак', 'лого', 'счёт');
  узел.classList.add('знак');
  const кСчёту = () => {
    узел.classList.remove('знак', 'лого');
    узел.classList.add('счёт');
    /* Кегль похвалы меряем ЗДЕСЬ: до этого мига короб с ней скрыт,
       ширины у него нет, и замер вышел бы нулевым. */
    подогнатьПохвалу();
    побежатьСчёт(цель, язык);
  };
  финалФазы.push(setTimeout(() => {
    узел.classList.remove('знак');
    узел.classList.add('лого');
  }, ФАЗА_ЗНАК * 1000));
  финалФазы.push(setTimeout(кСчёту, ФАЗА_ЛОГО * 1000));

}

/* Цифра бежит вверх с замедлением: быстро набирает и мягко подходит
   к своей невозможной величине. Кто просил меньше движения — тому
   сразу итог. */
function побежатьСчёт(цель, язык) {
  const узел = $('finale');
  const поле = $('finale-num');
  if (!поле) return;
  if (reduceMotion.matches) {
    писать(поле, счётЦифрами(цель));
    return;
  }
  узел.classList.add('бежит');
  const начало = performance.now();
  const шаг = (сейчас) => {
    const доля = Math.min(1, (сейчас - начало) / ФИНАЛ_БЕГ);
    const плавно = 1 - Math.pow(1 - доля, 3);
    писать(поле, счётЦифрами(цель * плавно));
    if (доля < 1) финалБег = requestAnimationFrame(шаг);
    else { финалБег = null; узел.classList.remove('бежит'); }
  };
  финалБег = requestAnimationFrame(шаг);
}

/* Когда выходит похвала: через ФИНАЛ_ПАУЗА после конца последней
   спетой строки. Нет разметки — считаем по концу песни: хвалить
   всё равно за что-то надо. */
...
/* Показан ли уже — за это воспроизведение. Иначе похвала выскакивала бы
   заново каждый кадр после своей секунды, а закрыть её было бы нельзя. */
let финалПоказан = false;

function сброситьФинал() {
  финалПоказан = false;
  спрятатьФинал();
}

/* Три оговорки, и каждая про то, чтобы праздник не вылез там,
   где его не ждут. */
function можноФинал() {
  if (финалПоказан) return false;
  if (!$('step-4') || !$('step-4').classList.contains('active')) return false;
  if (videoExport.active) return false;   // идёт запись — в кадре свой финал
  if (audio.stopAt != null) return false; // играли отрывок, а не песню
  return true;
}

/* Зовётся каждый кадр из общего цикла: смотрим, не пора ли хвалить. */
function проверитьФинал(pos) {
  if (!можноФинал()) return;
  const когда = времяФинала();
  if (когда == null || pos < когда) return;
  финалПоказан = true;
  показатьФинал();
}

// Песня доиграла сама — хвалим, если ещё не успели (короткая песня,
// где последнее слово совпадает с концом)
function песняКончилась() {
  проверитьФинал(audio.duration || 0);
}

if ($('finale')) {
  $('finale-close').addEventListener('click', спрятатьФинал);
  $('finale-again').addEventListener('click', () => {
    спрятатьФинал();
    audio.play(0);
    updatePlayerUI();
  });
  // Щелчок мимо коробки и Esc закрывают — как у любого окна поверх
  $('finale').addEventListener('click', (e) => {
    if (e.target === $('finale')) спрятатьФинал();
  });
  document.addEventListener('keydown', (e) => {
    if (e.key === 'Escape') спрятатьФинал();
  });
}
...
$('btn-play').addEventListener('click', () => {
  if (audio.playing) audio.pause();
    ...
    сброситьФинал();
    audio.play();
    audio.onEnded = () => { updatePlayerUI(); песняКончилась(); };
  }
  updatePlayerUI();
});
...
window.addEventListener('resize', () => {
  // Похвала подогнана под прежнюю ширину — пересчитываем
  if (!$('finale').classList.contains('hidden')) подогнатьПохвалу();
