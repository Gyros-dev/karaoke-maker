...
function показатьФинал() {
  ...
  /* Цифра бежит вверх с замедлением: быстро набирает и мягко
     подходит к своей невозможной величине. Кто просил меньше
     движения — тому сразу итог. */
...
const ФИНАЛ_В_КАДРЕ = 6;        // сколько секунд похвалы дописываем к записи
const ФИНАЛ_КАДР_БЕГ = 2.2;     // за столько секунд цифра добегает до итога

function drawФиналКадра(g2d, W, H, bgImg, прошло, финал) {
  const st = state.style;

  // Фон — как у песни: цвет, картинка или наша заливка
  if (st.bgMode === 'color') {
  ...
  // Притемнение: на светлой картинке белый текст иначе теряется
  g2d.fillStyle = 'rgba(6, 6, 10, 0.72)';
  g2d.fillRect(0, 0, W, H);
  ...
  const кегльФразы = Math.round(H * 0.062);
  ...
  const доля = Math.min(1, Math.max(0, прошло / ФИНАЛ_КАДР_БЕГ));
  const плавно = 1 - Math.pow(1 - доля, 3);
  ...
  drawФиналКадра.последний = { ряды, число, подпись: финал.подпись, доля,
    кегльФразы, кегльЧисла };
}
...
async function exportVideo() {
  ...
  ticker.onTick = () => {
    ...
    if (pos >= финалС) {
      drawФиналКадра(g2d, W, H, bgImg, pos - финалС, финалКадра);
      рисоватьЗнак(g2d, W, H, watermark);
    } else {
