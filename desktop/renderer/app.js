...
function имяТональности(тоника, лад) {
  ...
  const буква = НОТЫ_БУКВЫ[к] + (лад === 'минор' ? 'm' : '');
  return t('тон.имя', { 'нота': слово, 'лад': t(лад === 'минор' ? 'тон.минор' : 'тон.мажор'), 'буква': буква });
}
...
function обновитьНайденнуюТональность() {
  ...
  const своя = имяТональности(тональностьПесни.тоника, тональностьПесни.лад);
  const сдвиг = тон.выбран;
  писать(строка, сдвиг === 0
    ? t('тон.песняВ', { 'тональность': своя })
    : t('тон.песняВиСдвиг', {
      'тональность': своя,
      'сдвиг': тонПодпись(сдвиг),
      'станет': имяТональности(тональностьПесни.тоника + сдвиг, тональностьПесни.лад),
    }));
}
...
const ДОЛЯ_ИНСПЕКТОРА = 0.17;   // 210 px из 1232 — столько он занимал раньше
const МИН_ИНСПЕКТОРА = 168;     // уже — и время «1:27,440» не влезает в поле
const МАКС_ИНСПЕКТОРА = 360;
...
function resizeTimeline() {
  ...
  применитьДеление();
  // Заодно и ширину между просмотром и инспектором: их делят те же
  // события — открытие редактора и изменение размеров окна
  применитьШиринуИнспектора();
  const c = $('timeline');
