...
async function собратьРолик(о) {
  ...
  const цель = new Сборщик.ArrayBufferTarget();
  const сборщик = new Сборщик.Muxer(мп4 ? {
    ...
    /* Оглавление в начале файла: так ролик открывается и перематывается
       сразу, не дочитываясь до конца. */
    fastStart: 'in-memory',
  } : {
    target: цель,
  ...
  try {
    ...
    const имя = (state.fileName || 'song').replace(/\.[^.]+$/, '');
    прибрать();
    download(new Blob([цель.buffer], { type: мп4 ? 'video/mp4' : 'video/webm' }),
      t('экспорт.имяВидео', { 'имя': имя, ext }));
  } catch (e) {
    прибрать();
    try {
