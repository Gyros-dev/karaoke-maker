...
const ФИНАЛ_ПАУЗА = 3;
const ФИНАЛ_СЧЁТ_ОТ = 1000000;
const ФИНАЛ_СЧЁТ_ДО = 999999999;
...
function drawФиналКадра(g2d, W, H, bgImg, прошло, финал, лого) {
  ...
  if (лого && лого.width) {
    const k = ударНаМиг(прошло, ФАЗА_ЛОГО > прошло ? ФАЗА_ЗНАК : ФАЗА_ЛОГО);
    const сторона = Math.min(W, H) * (третья ? 0.62 : 0.74) * k;
    const w = сторона * (лого.width / лого.height);
    g2d.save();
    g2d.globalAlpha = третья ? 0.28 : 1;
  ...
  /* Похвала переносится по словам: фразы разной длины, а кегль
     единый — иначе длинная уехала бы за края кадра. */
  const k3 = ударНаМиг(прошло, ФАЗА_ЛОГО);
  const кегльФразы = Math.round(H * 0.062 * k3);
  g2d.font = `700 ${кегльФразы}px ${family}`;
  const слова = финал.фраза.split(' ');
  const ряды = [];
  let ряд = '';
  for (const сл of слова) {
    const проба = ряд ? ряд + ' ' + сл : сл;
    if (ряд && g2d.measureText(проба).width > макс) { ряды.push(ряд); ряд = сл; }
    else ряд = проба;
  }
  if (ряд) ряды.push(ряд);
