...
(function () {
  ...
  const очередь = {
    ...
    отмена: false,
    дела: [],
    песни: [],        // выбранные аудиофайлы
    тексты: [],       // выбранные .txt и .lrc (может не быть вовсе)
    шаг: 1,           // на каком шаге окно: 1 песни, 2 тексты, 3 папка, 4 готово
    папка: null,      // { dir, sep } — куда складывать готовые проекты
    текущее: null,    // дело, которое идёт прямо сейчас
    разбудить: null,  // чем оборвать ожидание долгого шага (см. сОбрывом)
    стадия: null,     // 'разделение' | 'разметка' | ... — что рвать при отмене
  };
  ...
  /* Что человек получит с этой песни. Написано ДО начала работы —
     ради этого и заведено: «не понятно, какой будет результат». */
  function чтоБудет(д) {
    if (!д.текст) return t('очередь.будет.нет');
    return t(д.текст.вид === 'lrc' ? 'очередь.будет.lrc' : 'очередь.будет.txt');
  }

  function рядОчереди(i) {
    const список = document.getElementById('queue-list');
    return список ? список.querySelector('[data-row="' + i + '"]') : null;
  }
  ...
  function обновитьРяд(д) {
    const ряд = рядОчереди(очередь.дела.indexOf(д));
    if (!ряд) return;
    const что = ряд.querySelector('.queue-state');
    if (что) {
      что.textContent = д.подпись
        || (д.состояние === 'ждёт' ? чтоБудет(д) : t('очередь.состояние.' + д.состояние));
    }
  ...
  function отметить(д, состояние, подпись) {
    д.состояние = состояние;
    д.подпись = подпись || null;
    обновитьРяд(д);
  }

  /* Живой ход текущей песни. Проценты приходят из тех же setProgress
     и setAsrProgress, что рисуют полосу разделения и распознавания:
     свои окна во время очереди не показываются, и без этого человек
     видел бы застывшее «убираем вокал…» и не знал, сколько ждать. */
  function ходОчереди(процент, что, подсказка) {
    if (!очередь.идёт || !очередь.текущее) return;
    const п = Number.isFinite(процент) && процент > 0
      ? Math.round(Math.min(100, процент)) + '% · ' : '';
    отметить(очередь.текущее, 'идёт', п + (что || ''));
    const заметка = document.getElementById('queue-note');
    if (заметка && подсказка) заметка.textContent = подсказка;
  }

  function кнопкиОчереди(вид) {
    const начать = document.getElementById('queue-go');
    const папка = document.getElementById('queue-show');
    const отмена = document.getElementById('queue-cancel');
    // «Начать» появляется только когда выбрано всё: песни и папка
    if (начать) начать.classList.toggle('hidden', вид !== 'готовы');
    if (папка) папка.classList.toggle('hidden', вид !== 'конец');
    if (отмена) {
      отмена.textContent = вид === 'идёт' ? t('очередь.прервать')
        : вид === 'конец' ? t('очередь.закрыть') : t('очередь.отменить');
    }
  }
  ...
  /* Окно очереди ведёт человека по шагам и на каждом говорит, что
     он делает и что получится. Первая сборка молча открывала выбор
     файлов, и человек справедливо не понял ни что выбирать, ни как
     студия свяжет текст с песней:

       «нет окна, описывающего „выбери один или несколько треков"…
        так же как программа поймёт, какой текст к какому треку
        относится? они должны называться одинаково или что?»

     Теперь три шага: песни → тексты (можно пропустить) → папка,
     и только потом «Начать». Правило связи написано словами, а не
     подразумевается. */
  function шагОчереди(n) {
    очередь.шаг = n;
    for (let i = 1; i <= 3; i++) {
      const блок = document.getElementById('queue-step-' + i);
      if (блок) блок.classList.toggle('hidden', i !== n);
    }
    /* «Что будет» нужно, пока человек выбирает; когда всё выбрано,
       то же самое говорят сводка и строки песен, и повторять незачем. */
    const оЧём = document.getElementById('queue-about');
    if (оЧём) оЧём.classList.toggle('hidden', n > 3);
    кнопкиОчереди(n === 4 ? 'готовы' : 'выбор');
  }

  function открытьОчередь() {
    if (очередь.идёт) { показатьОчередь(true); return; }
    очередь.дела = [];
    очередь.песни = [];
    очередь.папка = null;
    очередь.отмена = false;
    очередь.текущее = null;
    нарисоватьОчередь();
    писатьОчередь('queue-summary', '');
    писатьОчередь('queue-warn', '');
    писатьОчередь('queue-note', '');
    шагОчереди(1);
    показатьОчередь(true);
  }

  function писатьОчередь(id, текст) {
    const el = document.getElementById(id);
    if (el) el.textContent = текст;
  }

  /* Показать, что выбрано и что с этим будет. Зовётся после каждого
     шага: человек видит итог сразу, а не в конце. */
  function пересобратьОчередь() {
    очередь.дела = сопоставитьОчередь(очередь.песни.concat(очередь.тексты || []));
    нарисоватьОчередь();
    const сТекстом = очередь.дела.filter((д) => д.текст).length;
    const сводка = document.getElementById('queue-summary');
    if (сводка) {
      сводка.textContent = t('очередь.сводка', { n: очередь.дела.length, 'сТекстом': сТекстом });
      if (очередь.папка) {
        const где = document.createElement('span');
        где.className = 'queue-where';
        где.textContent = ' ' + t('очередь.куда', { 'папка': очередь.папка.dir });
        сводка.append(где);
      }
    }
    /* Тексты, которым не нашлось песни, — самая частая ошибка: имя
       не совпало. Молчать о них нельзя, человек будет ждать разметку,
       которой не будет. */
    const ничьи = (очередь.тексты || []).filter((ф) => {
      const основа = основаИмени(ф.name).toLowerCase();
      return !очередь.песни.some((п) => основаИмени(п.name).toLowerCase() === основа);
    });
    писатьОчередь('queue-warn', ничьи.length
      ? t('очередь.ничьиТексты', { n: ничьи.length, 'имена': ничьи.map((ф) => ф.name).join(', ') })
      : '');
  }

  async function папкуОчереди() {
    const куда = await window.desktop.queueFolder();
    if (!куда || !куда.ok) return false;
    очередь.папка = куда;
    пересобратьОчередь();
    писатьОчередь('queue-note', t('очередь.скольькоЖдать'));
    шагОчереди(4);
    return true;
  }

  /* Готовая очередь без окон выбора — этим ходит самопроверка
     и этим же можно позвать очередь из кода. */
  async function подготовитьОчередь(файлы, готоваяПапка) {
    if (очередь.идёт) return;
    const дела = сопоставитьОчередь(файлы);
    if (!дела.length) { alert(t('очередь.нетПесен')); return; }
    очередь.песни = файлы.filter((ф) => !/\.(lrc|txt)$/i.test(ф.name));
    очередь.тексты = файлы.filter((ф) => /\.(lrc|txt)$/i.test(ф.name));
    очередь.отмена = false;
    очередь.текущее = null;
    const куда = готоваяПапка || await window.desktop.queueFolder();
  ...
  /* Прервать по-настоящему, а не «после нынешней песни»: у разделения
     и распознавания есть свои кнопки отмены, и в них вся уборка —
     потоки, недокачанная модель, окна. Человек написал: «даже если
     выбрана одна, отменить нельзя» — вот это и лечим.

     Тонкость, на которой это ломалось. Отмена убивает рабочий поток,
     а обещание removeVocals ждёт от него ответа — и не дожидается
     никогда: в обычном окне его никто не ждёт, а очередь висела на нём
     насмерть. Поэтому каждый долгий шаг идёт наперегонки с обрывом:
     кто первый, того и ответ. */
  function сОбрывом(обещание) {
    return Promise.race([
      обещание,
      new Promise((resolve) => { очередь.разбудить = resolve; }),
    ]).finally(() => { очередь.разбудить = null; });
  }

  function прерватьОчередь() {
    очередь.отмена = true;
    const кнопка = document.getElementById(
      очередь.стадия === 'разметка' ? 'btn-asr-cancel' : 'btn-ai-cancel');
    if (кнопка) кнопка.click();
    if (очередь.разбудить) очередь.разбудить();
    const заметка = document.getElementById('queue-note');
    if (заметка) заметка.textContent = t('очередь.прерываем');
  }

  async function гнатьОчередь() {
    if (очередь.идёт || !очередь.дела.length || !очередь.папка) return;
    // Модели спрашиваем до начала: вопрос посреди очереди держал бы
    // её до возвращения человека, ради чего она и заведена
    if (!(await ensureModel())) return;
    if (очередь.дела.some((д) => д.текст && д.текст.вид === 'txt')) {
    ...
    /* Окна с сообщениями по дороге глушим: окно посреди очереди держит
       её до возвращения человека, ради чего она и заведена. Последнее
       сказанное храним — оно пригодится, если что-то не вышло.

       А вот СУДИМ не по сказанному, а по сделанному: разделение говорит
       «Готово!» и в случае удачи, и очередь считала эту радость бедой
       (первый же прогон встал на «Готово: 0 из 1»). Смотрим на итог:
       появилась минусовка — значит вышло. */
    const прежнийAlert = window.alert;
    let сказанное = '';
    window.alert = (текст) => { сказанное = String(текст); };
    ...
    try {
      for (const д of очередь.дела) {
        ...
        очередь.текущее = д;
        try {
          очередь.стадия = 'песня';
          отметить(д, 'идёт', t('очередь.шаг.песня'));
          await сОбрывом(handleFile(д.песня));
          if (очередь.отмена) break;
          if (!state.originalBuffer) throw new Error(t('очередь.неОткрылась'));

          очередь.стадия = 'разделение';
          отметить(д, 'идёт', t('очередь.шаг.разделение'));
          await сОбрывом(removeVocals());
          if (очередь.отмена) break;
          // Судим по итогу: минусовка от нейросети — это state.customInst
          if (!state.customInst) throw new Error(сказанное || t('очередь.неРазделилось'));

          if (д.текст) {
            const текст = await д.текст.файл.text();
            if (д.текст.вид === 'lrc') {
              очередь.стадия = 'lrc';
              отметить(д, 'идёт', t('очередь.шаг.lrc'));
              применитьLrc(текст);
            } else {
              document.getElementById('lyrics-input').value = текст;
              очередь.стадия = 'разметка';
              отметить(д, 'идёт', t('очередь.шаг.разметка'));
              await сОбрывом(fitLyrics());
              if (очередь.отмена) break;
              // И тут по итогу: у строк появились времена — значит легло
              if (!state.lines.some((л) => л.time != null)) {
                throw new Error(сказанное || t('очередь.неРазметилось'));
              }
            }
          }

          очередь.стадия = 'сохранение';
          отметить(д, 'идёт', t('очередь.шаг.сохраняем'));
          ...
          const имя = основаИмени(д.песня.name).replace(/[\\/:*?"<>|]/g, '_')
            + '.' + РАСШИРЕНИЕ_ПРОЕКТА;
          const ответ = await window.desktop.projectWrite(
            очередь.папка.dir + очередь.папка.sep + имя, файлыПроекта);
          if (!ответ || !ответ.ok) throw new Error((ответ && ответ.error) || '');
          отметить(д, 'готово', t('очередь.готово', { 'папка': имя }));
          готовых++;
        } catch (e) {
          отметить(д, 'ошибка', (e && e.message) || String(e));
        }
      }
    } finally {
    ...
    // Прерванные так и помечаем: иначе они остались бы «идёт» навсегда
    очередь.дела.forEach((д) => {
      if (д.состояние === 'идёт' || (очередь.отмена && д.состояние === 'ждёт')) {
        отметить(д, 'ждёт', t('очередь.состояние.прервано'));
      }
    });
    const заметка = document.getElementById('queue-note');
    if (заметка) {
  ...
  if (кнопкаОчереди) кнопкаОчереди.addEventListener('click', открытьОчередь);

  const полеПесен = document.getElementById('queue-songs');
  const кнопкаПесен = document.getElementById('btn-queue-songs');
  if (кнопкаПесен && полеПесен) {
    кнопкаПесен.addEventListener('click', () => полеПесен.click());
    полеПесен.addEventListener('change', (e) => {
  ...
  const полеТекстов = document.getElementById('queue-texts');
  const кнопкаТекстов = document.getElementById('btn-queue-texts');
  if (кнопкаТекстов && полеТекстов) {
    кнопкаТекстов.addEventListener('click', () => полеТекстов.click());
    полеТекстов.addEventListener('change', (e) => {
      const файлы = Array.from(e.target.files || []);
      e.target.value = '';
      if (!файлы.length) return;
      очередь.тексты = файлы;
      пересобратьОчередь();
      шагОчереди(3);
    });
  }
  const безТекстов = document.getElementById('btn-queue-notexts');
  if (безТекстов) {
    безТекстов.addEventListener('click', () => {
      очередь.тексты = [];
      пересобратьОчередь();
      шагОчереди(3);
    });
  }
  const кнопкаПапки = document.getElementById('btn-queue-folder');
  if (кнопкаПапки) кнопкаПапки.addEventListener('click', папкуОчереди);

  const пускОчереди = document.getElementById('queue-go');
  if (пускОчереди) {
    пускОчереди.addEventListener('click', () => {
      // Шаги прячем: дальше окно показывает ход, а не выбор
      шагОчереди(0);
      гнатьОчередь();
    });
  }
  const папкаОчереди = document.getElementById('queue-show');
  if (папкаОчереди) {
    папкаОчереди.addEventListener('click', () => {
      if (очередь.папка) window.desktop.projectReveal(очередь.папка.dir);
    });
  }
  const отменаОчереди = document.getElementById('queue-cancel');
  if (отменаОчереди) {
    отменаОчереди.addEventListener('click', () => {
      if (очередь.идёт) { прерватьОчередь(); return; }
      показатьОчередь(false);
    });
  }
  // Для самопроверки: очередь без окон выбора файлов
  window.__очередь = {
    открыть: открытьОчередь,
    сопоставить: сопоставитьОчередь,
    подготовить: подготовитьОчередь,
    гнать: гнатьОчередь,
    прервать: прерватьОчередь,
    состояние: очередь,
  };
  ...
  function setProgress(percent, status, hint) {
    ...
    ходОчереди(percent, status, hint);
  }

  function showOverlay(show) {
    /* Во время очереди своего окна у разделения нет: его ход виден
       строкой самой очереди, а два окна друг поверх друга — путаница.
       Кнопка отмены при этом на месте и жмётся кодом (см. прерватьОчередь). */
    $('ai-overlay').classList.toggle('hidden', !show || очередь.идёт);
  ...
  function showAsrOverlay(show) {
    // Во время очереди — то же самое, что и у разделения (см. showOverlay)
    $('asr-overlay').classList.toggle('hidden', !show || очередь.идёт);
    if (!show) вернутьКлавиатуру();
  }
  ...
  /* О ЧЁМ метка. Без этого она переживала то, что делает её ложью:
     человек очистил проект, открыл другую песню, убрал вокал — и ещё
     до того, как вставил текст, читал «Времена проставлены. Строк: 28».
     Гасла метка только когда поле текста чистили РУКАМИ: и очистка
     проекта, и загрузка песни меняют поле кодом, а события input
     при этом нет. */
  let итогПро = null;

  function показатьИтогРазметки(собрать, безПрокрутки) {
    собратьИтог = собрать || null;
    итогПро = собрать ? { песня: state.fileName || null } : null;
    ...
    try {
      ...
      показатьИтогРазметки(() => ({
        ...
        пояснение: t('asr.меткаРасслышала', { 'процент': процент }) + (с.сомнительныхСтрок
          ...
          + (с.безМеста ? t('asr.меткаБезМеста', { n: с.безМеста }) : ''),
      }));
      ...
      // Текста оказалось больше, чем спето, — говорим прямо
      if (с.безМеста) итог += t('asr.итогБезМеста', { n: с.безМеста });
  ...
  /* Метка жива, пока жива работа, о которой она. Умерла работа —
     сменилась песня, опустело поле текста, пропали времена — метку
     убираем. Времена ищем и в разметке, и в свежем итоге подгонки
     (__asrLines): сразу после расчёта строки ещё не разобраны, они
     разберутся на входе в редактор. */
  function проверитьИтогРазметки() {
    if (!собратьИтог) return;
    const естьТекст = !!$('lyrics-input').value.trim();
    const естьВремена = !!window.__asrLines
      || (Array.isArray(state.lines) && state.lines.some((l) => l.time != null));
    const таЖеПесня = итогПро && итогПро.песня === (state.fileName || null);
    if (!естьТекст || !естьВремена || !таЖеПесня) показатьИтогРазметки(null);
  }

  $('lyrics-input').addEventListener('input', updateAsrMode);
  // Пришли на шаг «Текст» — самое время пересмотреть, не врёт ли метка
  document.addEventListener('шаг', (e) => {
    if (e.detail && e.detail.шаг === 2) проверитьИтогРазметки();
  });
  updateAsrMode();

  /* Наружу — ради самопроверки: она ставит метку и смотрит, переживёт
     ли та смену песни. Другого способа добраться до неё нет, метка
     живёт внутри этого замыкания. */
  window.__меткаРазметки = { показать: показатьИтогРазметки, проверить: проверитьИтогРазметки };
