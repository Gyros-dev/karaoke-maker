...
(function () {
  ...
  /* ============================================================
     ...
     Студия — на компьютере, а поют, глядя в телефон: его держат
     в руке, и он не занят редактором. Приложение поднимает страницу
     в местной сети (см. main.js) и показывает ссылку кодом; телефон
     наводят камерой.

     Отдаём то же, что видит сцена: строки, слова и их времена берутся
     из lineSpan и lineWords — одного кода с просмотром, караоке
     и записью видео. Звук — минусовка, если она есть, иначе сама
     песня: петь под оригинал тоже случается.
     ============================================================ */
  async function собратьДляТелефона() {
    const строки = syncedLines();
    const разметка = {
      имя: (state.fileName || '').replace(/\.[^.]+$/, ''),
      строки: строки.map((л, i) => {
        const span = lineSpan(строки, i);
        const слова = lineWords(л, span);
        return {
          текст: л.text,
          от: +span.start.toFixed(3),
          до: +span.end.toFixed(3),
          слова: слова.map((w) => ({
            текст: w.text,
            от: +w.start.toFixed(3),
            до: +w.end.toFixed(3),
          })),
        };
      }),
    };
    /* Минусовку собираем ту же, что уходит в .wav: с оригиналом
       на размеченных отрезках. Нет её вовсе — отдаём саму песню
       её же байтами, без пересжатия. */
    const минус = state.instrumentalBuffer ? await собратьМинусовку() : null;
    if (минус) {
      const blob = bufferToWav(минус);
      return { разметка, звук: new Uint8Array(await blob.arrayBuffer()), тип: 'audio/wav' };
    }
    if (state.songBytes && state.songBytes.length) {
      return { разметка, звук: state.songBytes, тип: 'audio/mpeg' };
    }
    return null;
  }

  function нарисоватьКод(ссылка) {
    const холст = document.getElementById('sing-qr');
    if (!холст) return false;
    const сетка = qrКод(ссылка);
    if (!сетка) return false;
    const n = сетка.length;
    const поле = 2;                       // белая кайма: без неё код не читается
    const сторона = холст.width;
    const клетка = Math.floor(сторона / (n + поле * 2));
    const отступ = Math.floor((сторона - клетка * n) / 2);
    ...
    for (let y = 0; y < n; y++) {
      for (let x = 0; x < n; x++) {
        if (сетка[y][x]) g.fillRect(отступ + x * клетка, отступ + y * клетка, клетка, клетка);
      }
    }
    return true;
  }

  function показатьПение(да) {
    const окно = document.getElementById('sing-overlay');
    if (окно) окно.classList.toggle('hidden', !да);
  }

  async function начатьПениеСТелефона() {
    if (!state.originalBuffer) { alert(t('петь.нетПесни')); return; }
    if (!state.lines.some((l) => l.time != null)) { alert(t('петь.нетРазметки')); return; }
    const кнопка = document.getElementById('btn-sing-phone');
    if (кнопка) кнопка.disabled = true;
    try {
      const груз = await собратьДляТелефона();
      if (!груз) { alert(t('петь.нетПесни')); return; }
      const о = await window.desktop.singStart(груз);
      if (!о || !о.ok) { alert(t('петь.неПоднялся', { 'причина': (о && о.error) || '' })); return; }
      const адрес = document.getElementById('sing-url');
      if (адрес) адрес.textContent = о.ссылка;      // адрес — только текстом
      if (!нарисоватьКод(о.ссылка)) {
        const холст = document.getElementById('sing-qr');
        if (холст) холст.classList.add('hidden');   // не влез в код — хватит адреса
      }
      показатьПение(true);
    } finally {
  ...
  const кнопкаПения = document.getElementById('btn-sing-phone');
  if (кнопкаПения) кнопкаПения.addEventListener('click', начатьПениеСТелефона);
  const стопПения = document.getElementById('sing-stop');
  if (стопПения) {
    стопПения.addEventListener('click', async () => {
      await window.desktop.singStop();
      показатьПение(false);
    });
  }
  // Для самопроверки: тот же путь, что и по кнопке, но без окон
  window.__пение = { собрать: собратьДляТелефона, код: нарисоватьКод, начать: начатьПениеСТелефона };

  /* ============================================================
     Очередь: несколько песен подряд, без человека у экрана

     Разделение и разметка — минуты на песню, и всё это время человек
     сидел и смотрел, чтобы в конце нажать «сохранить» и начать
     следующую. Теперь он выбирает сразу несколько песен (и, если есть,
     тексты к ним — файл с тем же именем и расширением .txt или .lrc),
     говорит, куда складывать готовое, и уходит.

     Что делается с каждой: открыть песню → отделить вокал → положить
     текст (готовый .lrc ставит и времена, простой .txt размечается
     нейросетью) → сохранить папку проекта.

     Отмена честная, но не мгновенная: останавливаемся ПОСЛЕ нынешней
     песни. Рвать разделение на середине незачем — расчёт уже сделан,
     и через минуту он превратится в готовый проект.
     ============================================================ */
  const очередь = { идёт: false, отмена: false, дела: [] };

  function основаИмени(имя) {
    return String(имя).replace(/\.[^.]+$/, '');
  }

  /* Песня и текст к ней сходятся по имени: «песня.mp3» и «песня.txt».
     Регистр не важен — в одной папке лежат и «Песня.MP3», и «песня.txt». */
  function сопоставитьОчередь(файлы) {
    const тексты = new Map();
    const песни = [];
    for (const ф of файлы) {
      const вид = /\.lrc$/i.test(ф.name) ? 'lrc' : /\.txt$/i.test(ф.name) ? 'txt' : null;
      if (вид) тексты.set(основаИмени(ф.name).toLowerCase(), { файл: ф, вид });
      else песни.push(ф);
    }
    return песни.map((песня) => ({
      песня,
      текст: тексты.get(основаИмени(песня.name).toLowerCase()) || null,
      состояние: 'ждёт',
      беда: null,
    }));
  }

  function нарисоватьОчередь() {
    const список = document.getElementById('queue-list');
    if (!список) return;
    список.textContent = '';
    очередь.дела.forEach((д, i) => {
      const ряд = document.createElement('div');
      ряд.className = 'queue-row' + (д.состояние === 'идёт' ? ' now' : '')
        + (д.состояние === 'готово' ? ' done' : '')
        + (д.состояние === 'ошибка' ? ' bad' : '');
      ряд.dataset.row = String(i);
      const имя = document.createElement('span');
      имя.className = 'queue-name';
      имя.textContent = д.песня.name;          // имя файла человека — только текстом
      const что = document.createElement('span');
      что.className = 'queue-state';
      что.textContent = д.беда || t('очередь.состояние.' + д.состояние);
      ряд.append(имя, что);
      список.append(ряд);
    });
  }

  function отметить(д, состояние, шаг, беда) {
    д.состояние = состояние;
    д.беда = беда || null;
    д.шаг = шаг || null;
    const список = document.getElementById('queue-list');
    if (список) {
      const ряд = список.querySelector('[data-row="' + очередь.дела.indexOf(д) + '"]');
      if (ряд) {
        const что = ряд.querySelector('.queue-state');
        if (что) {
          что.textContent = д.беда || (шаг ? t('очередь.шаг.' + шаг)
            : t('очередь.состояние.' + состояние));
        }
        ряд.className = 'queue-row' + (состояние === 'идёт' ? ' now' : '')
          + (состояние === 'готово' ? ' done' : '') + (состояние === 'ошибка' ? ' bad' : '');
      }
    }
  }

  function показатьОчередь(да) {
    const окно = document.getElementById('queue-overlay');
    if (окно) окно.classList.toggle('hidden', !да);
  }

  async function пуститьОчередь(файлы) {
    if (очередь.идёт) return;
    const дела = сопоставитьОчередь(файлы);
    if (!дела.length) { alert(t('очередь.нетПесен')); return; }

    // Куда складывать — спрашиваем ОДИН раз на всю очередь
    const куда = await window.desktop.queueFolder();
    if (!куда || !куда.ok) return;
    // И модели тоже: вопрос посреди очереди остановил бы её до утра.
    // Разделение нужно всегда, распознавание — только если есть .txt,
    // которые придётся размечать нейросетью
    if (!(await ensureModel())) return;
    if (дела.some((д) => д.текст && д.текст.вид === 'txt')) {
      if (!(await ensureAsrModel(ASR_ЛУЧШАЯ))) return;
    }

    очередь.идёт = true;
    очередь.отмена = false;
    очередь.дела = дела;
    window.__очередьИдёт = true;
    нарисоватьОчередь();
    показатьОчередь(true);
    const заметка = document.getElementById('queue-note');
    const кнопка = document.getElementById('queue-cancel');
    if (заметка) заметка.textContent = '';
    if (кнопка) кнопка.textContent = t('очередь.отменить');

    /* Беды по дороге складываем, а не показываем окном: окно посреди
       очереди останавливает её до возвращения человека, а он ушёл —
       ради этого очередь и заведена. Покажем всё разом в конце. */
    const прежнийAlert = window.alert;
    const собранные = [];
    window.alert = (текст) => { собранные.push(String(текст)); };

    let готовых = 0;
    try {
      for (const д of очередь.дела) {
        if (очередь.отмена) break;
        собранные.length = 0;
        try {
          отметить(д, 'идёт', 'песня');
          await handleFile(д.песня);
          if (!state.originalBuffer) throw new Error(t('очередь.неОткрылась'));

          отметить(д, 'идёт', 'разделение');
          await removeVocals();
          if (собранные.length) throw new Error(собранные[собранные.length - 1]);

          if (д.текст) {
            const текст = await д.текст.файл.text();
            if (д.текст.вид === 'lrc') {
              отметить(д, 'идёт', 'lrc');
              применитьLrc(текст);
            } else {
              document.getElementById('lyrics-input').value = текст;
              отметить(д, 'идёт', 'разметка');
              await fitLyrics();
              if (собранные.length) throw new Error(собранные[собранные.length - 1]);
            }
          }

          отметить(д, 'идёт', 'сохраняем');
          const файлыПроекта = await собратьПапкуПроекта();
          // Имя папки — от песни; косые черты в имя не пускаем
          const имя = основаИмени(д.песня.name).replace(/[\\/:*?"<>|]/g, '_')
            + '.' + РАСШИРЕНИЕ_ПРОЕКТА;
          const ответ = await window.desktop.projectWrite(куда.dir + куда.sep + имя, файлыПроекта);
          if (!ответ || !ответ.ok) throw new Error((ответ && ответ.error) || '');
          отметить(д, 'готово');
          готовых++;
        } catch (e) {
          отметить(д, 'ошибка', null, (e && e.message) || String(e));
        }
      }
    } finally {
      window.alert = прежнийAlert;
      очередь.идёт = false;
      window.__очередьИдёт = false;
    }

    if (заметка) {
      заметка.textContent = t('очередь.итог', { 'готово': готовых, 'всего': очередь.дела.length })
        + (очередь.отмена ? ' ' + t('очередь.остановлена') : '');
    }
    if (кнопка) кнопка.textContent = t('очередь.закрыть');
  }

  const кнопкаОчереди = document.getElementById('btn-queue');
  const полеОчереди = document.getElementById('queue-input');
  const началоОчереди = document.getElementById('queue-start');
  if (началоОчереди) началоОчереди.classList.remove('hidden');
  if (кнопкаОчереди && полеОчереди) {
    кнопкаОчереди.addEventListener('click', () => полеОчереди.click());
    полеОчереди.addEventListener('change', async (e) => {
      const файлы = Array.from(e.target.files || []);
      e.target.value = '';
      if (файлы.length) await пуститьОчередь(файлы);
    });
  }
  const отменаОчереди = document.getElementById('queue-cancel');
  if (отменаОчереди) {
    отменаОчереди.addEventListener('click', () => {
      if (!очередь.идёт) { показатьОчередь(false); return; }
      очередь.отмена = true;
      const заметка = document.getElementById('queue-note');
      if (заметка) заметка.textContent = t('очередь.остановимся');
    });
  }
  // Для самопроверки: очередь без окон выбора файлов
  window.__очередь = { сопоставить: сопоставитьОчередь, пустить: пуститьОчередь, состояние: очередь };

  /* Беды страницы — в журнал приложения.

     Смотреть было некуда: инструменты разработчика мы нарочно убрали,
     а вывод в терминал никто не видит — приложение запускают значком.
     Теперь всякая необработанная ошибка и всякий отказ обещания
     уезжают в файл рядом с настройками, и человеку есть что показать,
     когда он пишет «не работает» («Показать журнал» в руководстве). */
  const вЖурнал = (текст) => {
    try { if (window.desktop && window.desktop.logError) window.desktop.logError(текст); }
    catch (e) { /* журнал не должен мешать работе */ }
  };
  window.addEventListener('error', (e) => {
    const где = e.filename ? ' (' + e.filename + ':' + e.lineno + ':' + e.colno + ')' : '';
    вЖурнал(((e.error && e.error.stack) || e.message || 'ошибка') + где);
  });
  window.addEventListener('unhandledrejection', (e) => {
    const п = e.reason;
    вЖурнал('необработанный отказ: ' + ((п && п.stack) || (п && п.message) || String(п)));
  });
  // Для самопроверки: путь тот же, что у настоящей беды
  window.__вЖурнал = вЖурнал;

  const кнопкаЖурнала = document.getElementById('btn-log');
  if (кнопкаЖурнала) {
    кнопкаЖурнала.addEventListener('click', async () => {
      const о = await window.desktop.logShow();
      if (!о || !о.ok) alert(t('журнал.неОткрылся'));
    });
  }
  ...
  function вернутьКлавиатуру() {
    ...
    try {
      ...
      /* Спрашиваем страницу, а не гадаем: если она сама себя фокусом
         не считает, буквы набираться не будут — так и было на Windows
         после системного окна (см. дневник клавиатуры). Тогда просим
         главный процесс сделать настоящий круг фокуса окна. Проверяем
         дважды: сразу и следующим тактом — системное окно возвращает
      ...
      /* А вот и главное. Человек уточнил решающее: после системного
         окна текст УДАЛЯТЬ можно, а печатать нельзя. Значит, клавиши
         до поля доходят — иначе не работал бы и Backspace, — и дело
         не в фокусе вовсе. Так ведёт себя порванная связь с методом
         ввода (IME): команды доходят, а буквы складываются в никуда.
         На Windows её рвёт как раз системное окно.

         Лечится единственным, что эту связь пересобирает, — настоящим
         кругом фокуса на самом поле. Выделение при этом сохраняем
         руками: без этого курсор прыгнул бы в начало, а человек
         продолжает печатать с того места, где остановился. */
      if (где && где !== document.body && где.offsetParent) {
        const начало = где.selectionStart;
        const конец = где.selectionEnd;
        где.blur();
        где.focus();
        if (начало != null && typeof где.setSelectionRange === 'function') {
          try { где.setSelectionRange(начало, конец); } catch (e) { /* не текстовое поле */ }
        }
      }
  ...
  window.__runFitTest = async (buffer, text, language, key, opts) => {
    try {
      ...
      return {
        ...
        /* Времена САМИХ СЛОВ: ими и живёт пословная подсветка, и
           сверять подгонку надо по ним, а не только по началам строк. */
        словаПоСтрокам: state.lines.map((l) => (l.words || [])
          .map((w) => +(+w.time).toFixed(2))),
        /* Что ВИДНО на дорожке и на сцене. Это не то же самое, что
           записанное время: строку подтягивает к настоящему вступлению
           голоса (см. lineStart), и сверять подгонку честнее по этому
           числу — человек работает с ним. */
        показанные: (() => {
          const syn = syncedLines();
          return syn.map((_, i) => +lineStart(syn, i).toFixed(2));
        })(),
