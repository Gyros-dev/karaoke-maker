...
(function () {
  ...
  /* Выбрали файлы — сначала показываем, ЧТО будет, и только по кнопке
     «Начать» беремся за дело. Раньше работа начиналась сразу после
     выбора папки, и человек оставался перед окном, из которого нельзя
     было ни понять, сколько ждать, ни выйти. */
  async function подготовитьОчередь(файлы, готоваяПапка) {
    ...
    // Папку можно передать готовой — так самопроверка обходится
    // без окна выбора, а живой ход остаётся ровно таким же
    ...
    нарисоватьОчередь();
    const сТекстом = дела.filter((д) => д.текст).length;
    const сводка = document.getElementById('queue-summary');
    if (сводка) {
      сводка.textContent = t('очередь.сводка', { n: дела.length, 'сТекстом': сТекстом });
      const где = document.createElement('span');
      где.className = 'queue-where';
      где.textContent = ' ' + t('очередь.куда', { 'папка': куда.dir });
      сводка.append(где);
    }
    const заметка = document.getElementById('queue-note');
    if (заметка) заметка.textContent = t('очередь.скольькоЖдать');
    кнопкиОчереди('готовы');
  ...
  if (кнопкаОчереди && полеОчереди) {
    ...
    полеОчереди.addEventListener('change', async (e) => {
      ...
      e.target.value = '';
      if (файлы.length) await подготовитьОчередь(файлы);
    });
  }
  const пускОчереди = document.getElementById('queue-go');
  if (пускОчереди) пускОчереди.addEventListener('click', гнатьОчередь);
