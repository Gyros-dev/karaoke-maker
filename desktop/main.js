...
/* ============================================================
   ...
   Студия — на компьютере, а поют, глядя в телефон: его держат в руке,
   и он не занят редактором. Приложение поднимает маленький сервер
   в местной сети и показывает ссылку кодом на экране — телефон
   наводят камерой и открывают сцену.

   Что отдаём: страницу (desktop/phone.html), разметку строк и слов
   и звук — минусовку, если она есть, иначе саму песню.

   Осторожность:
     • сервер поднимается ТОЛЬКО по кнопке и гаснет вместе с окном;
     • в ссылке случайный ключ, без него любой запрос получает 404 —
       сосед по кафейной сети наугад не попадёт;
     • отдаётся ровно три вещи, никакого доступа к файлам;
     • ничего не уходит в интернет: адрес местный (192.168.…),
       и наружу его не пускает домашний роутер.
   ============================================================ */
const петь = { сервер: null, ключ: null, порт: 0, данные: null, звук: null, тип: '' };

function местныйАдрес() {
  const os = require('os');
  const сети = os.networkInterfaces();
  const свои = [];
  for (const имя of Object.keys(сети)) {
    for (const а of сети[имя] || []) {
      if (а.family !== 'IPv4' || а.internal) continue;
      // Домашние сети — первыми: 192.168.*, 10.*, 172.16–31.*
      const домашний = /^192\.168\./.test(а.address) || /^10\./.test(а.address)
        || /^172\.(1[6-9]|2\d|3[01])\./.test(а.address);
      свои.push({ адрес: а.address, домашний });
    }
  }
  свои.sort((a, b) => Number(b.домашний) - Number(a.домашний));
  return свои.length ? свои[0].адрес : null;
}

function остановитьПение() {
  if (петь.сервер) {
    try { петь.сервер.close(); } catch (e) { /* уже закрыт */ }
  }
  петь.сервер = null;
  петь.ключ = null;
  петь.порт = 0;
...
function начатьПение(о) {
  остановитьПение();
  const http = require('http');
  петь.ключ = crypto.randomBytes(6).toString('hex');
  петь.данные = Buffer.from(JSON.stringify(о.разметка || {}), 'utf8');
  петь.звук = Buffer.from(о.звук || []);
  петь.тип = о.тип || 'audio/wav';
  const страница = fs.readFileSync(path.join(__dirname, 'phone.html'));

  return new Promise((resolve) => {
    const сервер = http.createServer((req, res) => {
      /* Ключ в пути — единственный пропуск. Всё остальное молча
         получает 404: сервер не должен рассказывать, что он вообще
         тут есть, тому, кто ключа не знает. */
      /* Путь раскодируем: имена у нас русские («данные.json»), и всякий
         клиент — и телефон, и наша же проверка — присылает их
         процентами. Кривую запись не разбираем вовсе. */
      let путь = String(req.url || '').split('?')[0];
      try { путь = decodeURIComponent(путь); } catch (e) { res.writeHead(400); res.end(); return; }
      const начало = '/' + петь.ключ;
      if (!путь.startsWith(начало + '/') && путь !== начало) {
        res.writeHead(404); res.end(); return;
      }
      /* Ссылка без косой черты в конце — беда тихая: страница отдастся,
         а «звук» и «данные.json» она попросит из корня и не получит.
         Поэтому уводим на путь с чертой; в самой ссылке она тоже есть. */
      if (путь === начало) {
        res.writeHead(302, { Location: начало + '/' });
        res.end();
        return;
      }
      const что = путь.slice(начало.length).replace(/^\//, '') || 'страница';
      const отдать = (тип, тело, длинно) => {
        res.writeHead(200, {
          'Content-Type': тип,
          'Content-Length': тело.length,
          'Cache-Control': длинно ? 'public, max-age=600' : 'no-store',
        });
        res.end(req.method === 'HEAD' ? undefined : тело);
      };
      if (что === 'страница') return отдать('text/html; charset=utf-8', страница, false);
      if (что === 'данные.json') return отдать('application/json; charset=utf-8', петь.данные, false);
      if (что !== 'звук') { res.writeHead(404); res.end(); return; }

      /* Звук отдаём кусками: телефон просит его диапазонами, и без
         поддержки Range перемотка на iPhone не работает вовсе. */
      const всего = петь.звук.length;
      const range = /^bytes=(\d*)-(\d*)$/.exec(req.headers.range || '');
      if (!range) {
        res.writeHead(200, {
          'Content-Type': петь.тип, 'Content-Length': всего, 'Accept-Ranges': 'bytes',
        });
        res.end(req.method === 'HEAD' ? undefined : петь.звук);
        return;
      }
      const от = range[1] ? Number(range[1]) : 0;
      const до = range[2] ? Math.min(Number(range[2]), всего - 1) : всего - 1;
      if (!(от <= до && до < всего)) {
        res.writeHead(416, { 'Content-Range': 'bytes */' + всего });
        res.end();
        return;
      }
      res.writeHead(206, {
        'Content-Type': петь.тип,
        'Content-Range': 'bytes ' + от + '-' + до + '/' + всего,
        'Content-Length': до - от + 1,
        'Accept-Ranges': 'bytes',
      });
      res.end(req.method === 'HEAD' ? undefined : петь.звук.subarray(от, до + 1));
    });
    сервер.on('error', (e) => {
      записатьВЖурнал('пение', 'сервер не поднялся: ' + ((e && e.message) || e));
      остановитьПение();
      resolve({ ok: false, error: (e && e.message) || String(e) });
    });
    // Порт выбирает система: занятый порт — не наша забота
    сервер.listen(0, '0.0.0.0', () => {
      петь.сервер = сервер;
      петь.порт = сервер.address().port;
      const адрес = местныйАдрес();
      if (!адрес) {
        остановитьПение();
        resolve({ ok: false, error: 'нет сети' });
        return;
      }
      const ссылка = 'http://' + адрес + ':' + петь.порт + '/' + петь.ключ + '/';
      записатьВЖурнал('пение', 'сервер поднят: ' + ссылка);
      resolve({ ok: true, ссылка, адрес, порт: петь.порт });
    });
  });
}

ipcMain.handle('sing-start', async (_evt, о) => {
  if (!о || typeof о !== 'object') return { ok: false, error: 'плохой вызов' };
  try {
    return await начатьПение(о);
  } catch (e) {
    записатьВЖурнал('пение', (e && e.stack) || String(e));
    return { ok: false, error: (e && e.message) || String(e) };
  }
});

ipcMain.handle('sing-stop', async () => { остановитьПение(); return { ok: true }; });
ipcMain.handle('sing-state', async () => ({
  идёт: !!петь.сервер, порт: петь.порт,
}));

// Окно закрылось — сервер гаснет вместе с ним: пение живёт ровно
// столько, сколько открыта студия
app.on('window-all-closed', остановитьПение);
app.on('before-quit', остановитьПение);
...
function createWindow() {
  ...
  win.webContents.on('unresponsive', () => записатьВЖурнал('окно', 'страница не отвечает'));
    ...
    win.webContents.once('did-finish-load', async () => {
      ...
      const report = await win.webContents.executeJavaScript(`(() => ({
        ...
        права: __раздел('права', () => {
          const кнопка = () => document.querySelector('#btn-export-video span');
          const строка = () => document.getElementById('export-rights');
          const снимок = () => ({
            кнопка: кнопка().textContent.trim(),
            подсказка: document.getElementById('btn-export-video').title,
      ...
      /* Пение с телефона: сервер поднимается и отдаёт что надо.

         Проверяем настоящим кругом: страница просит у главного
         процесса сервер, а мы тут же ходим по выданной ссылке своими
         же запросами — как сходил бы телефон. Заодно смотрим, что
         без ключа не отдаётся ничего и что после остановки сервера
         не остаётся вовсе. */
      {
        const груз = await win.webContents.executeJavaScript(`__раздел('пениеПодготовка', async () => {
          const c = new OfflineAudioContext(1, 3 * 8000, 8000);
          state.originalBuffer = c.createBuffer(1, 3 * 8000, 8000);
          state.instrumentalBuffer = state.originalBuffer;
          state.customInst = false;
          state.songBytes = new Uint8Array([1, 2, 3, 4, 5]);
          state.fileName = 'проверка.mp3';
          audio.duration = 3;
          state.lines = [
            { text: 'раз два', time: 0.5, end: 1.5, ручноеНачало: true, ручнойКонец: true },
            { text: 'три', time: 2, end: 2.6, ручноеНачало: true, ручнойКонец: true },
          ];
          const собранное = await window.__пение.собрать();
          const о = await window.desktop.singStart(собранное);
          return { о, строк: собранное.разметка.строки.length,
            слов: собранное.разметка.строки[0].слова.length,
            звука: собранное.звук.length, тип: собранное.тип,
            кодНарисован: window.__пение.код(о.ссылка) };
        })`, true);

        const сходить = (url, заголовки) => new Promise((resolve) => {
          const http = require('http');
          const req = http.get(url, { headers: заголовки || {} }, (res) => {
            const куски = [];
            res.on('data', (c) => куски.push(c));
            res.on('end', () => resolve({
              код: res.statusCode, тип: res.headers['content-type'] || '',
              длина: Buffer.concat(куски).length, тело: Buffer.concat(куски),
            }));
          });
          req.on('error', (e) => resolve({ код: 0, ошибка: (e && e.message) || String(e) }));
          req.setTimeout(4000, () => { req.destroy(); resolve({ код: 0, ошибка: 'таймаут' }); });
        });

        /* Ходим на 127.0.0.1, а не по показанному адресу: с macOS 15
           обращение по сетевому адресу требует разрешения «Локальная
           сеть», и в проверке оно только мешало бы. Сервер слушает
           все адреса разом, так что это тот же самый сервер. */
        const ссылка = груз.о && груз.о.ссылка;
        const свой = ссылка ? ссылка.replace(/\/\/[^:]+:/, '//127.0.0.1:') : null;
        const страница = свой ? await сходить(свой) : { код: 0 };
        // Та же ссылка без косой черты: телефон получит уход на верную
        const безЧерты = свой ? await сходить(свой.replace(/\/$/, '')) : { код: 0 };
        const данные = свой ? await сходить(свой + 'данные.json') : { код: 0 };
        const звук = свой ? await сходить(свой + 'звук', { Range: 'bytes=0-99' }) : { код: 0 };
        const чужой = свой
          ? await сходить(свой.replace(/[0-9a-f]+\/$/, 'deadbeef/') + 'данные.json') : { код: 0 };
        const разбор = данные.код === 200 ? JSON.parse(данные.тело.toString('utf8')) : null;
        await win.webContents.executeJavaScript('window.desktop.singStop()', true);
        const послеСтопа = свой ? await сходить(свой) : { код: 0 };

        report.пениеСТелефона = {
          ссылка: ссылка ? ссылка.replace(/\d+\.\d+\.\d+\.\d+/, 'адрес') : null,
          строк: груз.строк,
          слов: груз.слов,
          звукаБайт: груз.звука,
          кодНарисован: груз.кодНарисован,
          страница: страница.код,
          безЧерты: безЧерты.код,
          беда: страница.ошибка || данные.ошибка || звук.ошибка || null,
          данные: данные.код,
          звук: звук.код,
          чужойКлюч: чужой.код,
          послеСтопа: послеСтопа.код,
          строкВРазметке: разбор ? разбор.строки.length : null,
          вНорме: !!ссылка && груз.кодНарисован
            && груз.строк === 2 && груз.слов === 2 && груз.звука > 1000
            // Страница, разметка и кусок звука отдаются
            && страница.код === 200 && данные.код === 200 && звук.код === 206
            && безЧерты.код === 302
            // Без ключа — ничего, а после остановки нет и сервера
            && чужой.код === 404 && послеСтопа.код === 0
            && разбор.строки.length === 2,
        };
      }
