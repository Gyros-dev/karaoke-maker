  ...
  if (process.env.KARAOKE_SELFTEST === '1') {
    ...
    win.webContents.once('did-finish-load', async () => {
      ...
      report.финал = await win.webContents.executeJavaScript(`__раздел('финал', () => {
        ...
        goToStep(3);
        песняКончилась();
        const наЧужомШаге = !узел.classList.contains('hidden');
        goToStep(4);
        videoExport.active = true;
        песняКончилась();
        const вЗаписи = !узел.classList.contains('hidden');
        videoExport.active = false;
        audio.stopAt = 10;
        песняКончилась();
        const наОтрывке = !узел.classList.contains('hidden');
        audio.stopAt = null;
        песняКончилась();
        const наСвоёмМесте = !узел.classList.contains('hidden');
      ...
      if (process.env.KARAOKE_SHOT) {
        try {
          const итогСцены = await win.webContents.executeJavaScript(`(async () => {
            ...
            document.getElementById('lyrics-input').value = тексты.join('\\n');
            const t = [[5, 9], [6.5, 10.5], [14, 18], [20, 24]];
            const p = [1, 2, 3, 0];
            state.lines = тексты.map((text, i) => ({
              text, time: t[i][0], end: t[i][1],
            ...
            if (${JSON.stringify(process.env.KARAOKE_SHOT_SCENE || '')} === 'финал') {
              goToStep(4);
              показатьФинал();
