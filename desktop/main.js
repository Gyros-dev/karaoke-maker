  ...
  if (process.env.KARAOKE_SELFTEST === '1') {
    ...
    win.webContents.once('did-finish-load', async () => {
      ...
      report.финал = await win.webContents.executeJavaScript(`__раздел('финал', () => {
        ...
        const цифры = счётЦифрами(384951726);
        ...
        goToStep(4);                      // хвалят только на шаге «Караоке»
        сброситьФинал();
        const когда = времяФинала();
        проверитьФинал(когда - 0.5);
        const раноНеВышло = узел.classList.contains('hidden');
        проверитьФинал(когда);
        const вСвойЧас = !узел.classList.contains('hidden');
        // Второй раз не выскакивает: закрыл — значит закрыл
        спрятатьФинал();
        проверитьФинал(когда + 5);
        const дваждыНе = узел.classList.contains('hidden');

        // Праздник не вылезает там, где его не звали
        const проба = (наладить) => {
          сброситьФинал();
          наладить();
          проверитьФинал(когда + 1);
          const вышло = !узел.classList.contains('hidden');
          спрятатьФинал();
          return вышло;
        };
        const наЧужомШаге = проба(() => goToStep(3));
        goToStep(4);
        const вЗаписи = проба(() => { videoExport.active = true; });
        videoExport.active = false;
        const наОтрывке = проба(() => { audio.stopAt = 10; });
        audio.stopAt = null;
        const наСвоёмМесте = проба(() => {});

        /* Три удара: надпись — логотип — счёт поверх логотипа.
           Проверяем и кадр видео, и экран: просмотр обязан показывать
           то же, что уедет в файл. */
        const фазаКадра = (t) => {
          const c = document.createElement('canvas');
        ...
        сброситьФинал();
        проверитьФинал(когда + 1);
        const экранСразу = узел.classList.contains('знак');
        спрятатьФинал();

        // Esc убирает — как у любого окна поверх
        сброситьФинал();
        проверитьФинал(когда + 1);
        document.dispatchEvent(new KeyboardEvent('keydown', { key: 'Escape', bubbles: true }));
        const escУбрал = узел.classList.contains('hidden');

        const итог = {
          языкРусской, языкАнгл, похвалаРус, похвалаАнгл,
          цифры, когда, раноНеВышло, вСвойЧас, дваждыНе,
          удары, экранСразу,
          наЧужомШаге, вЗаписи, наОтрывке, наСвоёмМесте, escУбрал,
        ...
        итог.вНорме =
          ...
          && экранСразу
          // Выходит по последнему слову, а не по концу песни, и один раз
          && Math.abs(когда - (6 + 3)) < 0.35
          && раноНеВышло && вСвойЧас && дваждыНе
          // Не на чужом шаге, не в записи, не после отрывка — и на своём месте да
          && !наЧужомШаге && !вЗаписи && !наОтрывке && наСвоёмМесте && escУбрал
