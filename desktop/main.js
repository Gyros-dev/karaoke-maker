    ...
    win.webContents.once('did-finish-load', async () => {
      ...
      report.громкостьОтрезка = await win.webContents.executeJavaScript(`__раздел('громкостьОтрезка', async () => {
        ...
        const итог = {
          ...
          поСтроке, послеПравки, вПроекте, послеОтмены,
      ...
      report.финал = await win.webContents.executeJavaScript(`__раздел('финал', () => {
        ...
        // Счёт: всегда больше сотни, разряды по языку песни
        const разрядыРу = счётСРазрядами(384951726, 'ru');
        const разрядыЕн = счётСРазрядами(384951726, 'en');
        ...
        const итог = {
          ...
          разрядыРу, разрядыЕн, когда, раноНеВышло, вСвойЧас, дваждыНе,
          удары, экранСразу,
        ...
        итог.вНорме =
          ...
          // Разряды: по-русски пробелом, по-английски запятой
          && разрядыРу === '384\\u00A0951\\u00A0726' && разрядыЕн === '384,951,726'
