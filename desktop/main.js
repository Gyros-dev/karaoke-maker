  ...
  if (process.env.KARAOKE_SELFTEST === '1') {
    ...
    win.webContents.once('did-finish-load', async () => {
      ...
      /* Очередь из нескольких песен.
         ...
         состоит и на чём она уже спотыкалась вживую: как песни
         сходятся с текстами, что человеку СРАЗУ написано, что он
         получит и куда это ляжет, и что окно закрывается, а не держит
         его до конца работы. */
      report.очередь = await win.webContents.executeJavaScript(`__раздел('очередь', async () => {
        ...
        // Показ до начала работы: сводка, что будет с каждой песней, куда ляжет
        await о.подготовить([
          файл('Первая.mp3'), файл('Первая.txt'), файл('Вторая.mp3'),
        ], { ok: true, dir: '/tmp/очередь', sep: '/' });
        const строки = [...document.querySelectorAll('#queue-list .queue-row')]
          .map((р) => р.querySelector('.queue-state').textContent);
        итог.окноОткрыто = !document.getElementById('queue-overlay').classList.contains('hidden');
        итог.сводка = document.getElementById('queue-summary').textContent;
        итог.рядыДоНачала = строки;
        итог.естьНачать = !document.getElementById('queue-go').classList.contains('hidden');
        итог.папкаНеСпрошена = о.состояние.папка.dir === '/tmp/очередь';
        итог.неНачалось = !о.состояние.идёт;

        // Окно закрывается, пока работа не пошла
        document.getElementById('queue-cancel').click();
        итог.закрылось = document.getElementById('queue-overlay').classList.contains('hidden');

        // Пустой выбор ничего не запускает и говорит об этом
        const прежний = window.alert;
        let сказано = '';
        window.alert = (текст) => { сказано = String(текст); };
        await о.подготовить([файл('текст.txt')], { ok: true, dir: '/tmp', sep: '/' });
        ...
        итог.вНорме = итог.песен === 3 && итог.текстПоИмени === 'txt'
          ...
          // Человеку сразу написано: сколько песен, у скольких текст и куда
          && итог.окноОткрыто && итог.сводка.indexOf('/tmp/очередь') > 0
          && /2/.test(итог.сводка)
          // И что будет с каждой: у второй текста нет — только минусовка
          && итог.рядыДоНачала.length === 2
          && итог.рядыДоНачала[0] !== итог.рядыДоНачала[1]
          && итог.естьНачать && итог.неНачалось && итог.папкаНеСпрошена
          && итог.закрылось && итог.пустаяОчередь;
        return итог;
      })`, true);
