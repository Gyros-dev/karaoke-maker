        ...
        <details class="ai-more">
          ...
          <div class="ai-more-body">
            ...
            <p data-i18n="ии.пояснение">Мы замерили: три прохода со сдвигом дают тот же результат, что один.
              ...
              Выбор оставлен на случай, если на твоей записи разница всё-таки
...
function patchHtml(src) {
  ...
  /* 5.5. СЧЁТЧИК ПОСЕЩЕНИЙ — только на сайте.

     На странице стоит GoatCounter: узнать, приходит ли кто-нибудь,
     иначе нечем — GitHub Pages логов не отдаёт. В приложении ему делать
     нечего: оно хвалится тем, что ничего никуда не отправляет, и это
     должно быть правдой до последней строчки. Вырезаем вместе
     с пояснением; политику безопасности приложение и так получает
     свою, без чужих адресов. Стережёт это быстрая проверка
     «в приложении нет счётчика». */
  const СЧЁТЧИК = /<!-- СЧЁТЧИК ПОСЕЩЕНИЙ[\s\S]*?<script data-goatcounter[\s\S]*?<\/script>\n/;
  if (!СЧЁТЧИК.test(s)) throw new Error('не нашёл счётчик посещений — он был в index.html');
  s = s.replace(СЧЁТЧИК, '');
