...
/* ============================================================
   ...
   в канал. Число берётся оттуда же, откуда в терминале, — из
   releases.js. Это важнее, чем кажется: если бы канал считал
   по-своему, рано или поздно он разошёлся бы со сводкой, и было бы
   непонятно, кому верить.

   Посещений сайта в сводке нет: счётчик со страницы убран (см. шапку
   stats.js). Остались скачивания и проверки обновления — то, что
   считает сам GitHub, никого при этом не отслеживая.
...
async function сводка() {
  const { строки, итогПоПлатформам, всего, опись } = await собрать();
  const последний = строки[строки.length - 1];

  const было = прошлое();
  const разница = было && typeof было.всего === 'number' ? всего - было.всего : null;
  const разницаОписи = было && typeof было.опись === 'number' ? опись - было.опись : null;
  ...
  /* Опись latest.yml — единственное число про ЖИВЫЕ копии: её
     спрашивает не человек, а установленное приложение, когда ходит
     за обновлением (при запуске и дальше раз в шесть часов, только
     Windows). Отсюда и грубая прикидка: четыре вопроса в сутки —
     это одна копия, которая весь день работала. Прикидку показываем
     только когда есть с чем сравнить и когда вопросов хватает,
     чтобы деление имело смысл. */
  const проверки = разницаОписи === null
    ? `всего **${опись}** за всё время`
    : `**${знак(разницаОписи)}** за сутки · всего ${опись}`
      + (разницаОписи >= 4 ? ` · это примерно ${Math.round(разницаОписи / 4)} живых копий` : '');

  const поля = [
    { name: 'Скачивания установщиков', value: скачивания },
    { name: 'Проверок обновления', value: проверки },
  ];
  if (последний) {
    поля.push({
  ...
  запомнить({ всего, опись, когда: new Date().toISOString() });
}
