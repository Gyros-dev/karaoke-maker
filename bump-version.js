...
function bumpSite(version) {
  ...
  const before = html;
  /* Картинка карточки ссылки (og:image) тоже с номером: он сбивает
     кэш у мессенджеров, когда карточку перерисовывают. */
  html = html.replace(/(style\.css|app\.js|timing\.js|i18n\.js|fft\.js|tempo\.js|mp4-muxer\.js|webm-muxer\.js|card\.jpg)\?v=[\d.]+/g, `$1?v=${version}`);
