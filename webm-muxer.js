/* ============================================================
   webm-muxer 5.1.4 — Vanilagy (Julian Bartels), MIT. Взято с npm,
   файл build/webm-muxer.js (сборка для браузера: заводит глобаль).
   Складывает сжатые кадры VP8/VP9 и звук Opus в файл WebM.

   ЧТО ИЗМЕНЕНО: ничего, кроме этой шапки.

   ПОЧЕМУ ЛЕЖИТ У НАС. Сборщика в проекте нет, а политика безопасности
   (CSP) пускает только свои файлы — значит, любая чужая библиотека
   переезжает к нам целиком и навсегда. Заодно это страховка: выпуск
   не зависит от чужого хранилища.

   ПРО «УСТАРЕЛО». Автор объявил обе эти библиотеки устаревшими в пользу
   своей новой (Mediabunny). Мы остаёмся на этих сознательно: они делают
   ровно одно законченное дело — складывают уже сжатые куски в контейнер,
   — а формат контейнера не меняется десятилетиями. Новая идёт под
   MPL-2.0 и только модулями: и лицензия тяжелее, и в наш уклад
   без сборщика она не встаёт.
   ============================================================ */
"use strict";
var WebMMuxer = (() => {
