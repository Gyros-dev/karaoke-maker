...
async function собрать(толькоВерсия) {
  ...
  let всего = 0;
  let описи = 0;
  /* Опись latest.yml — особая: её спрашивает НЕ человек, а установленная
     копия, когда ходит за обновлением (раз в шесть часов и при запуске).
     Поэтому её держим отдельно от блок-карт: по ней одной видно, сколько
     копий живо на самом деле. Автообновление у нас только для Windows. */
  let опись = 0;
  let блокКарты = 0;
  ...
  for (const рел of выпуски.slice().reverse()) {
    ...
    for (const файл of рел.assets || []) {
      ...
      if (!кто) {
        описи += файл.download_count;
        if (/^latest.*\.yml$/.test(файл.name)) опись += файл.download_count;
        else блокКарты += файл.download_count;
        continue;
      }
  ...
  return { строки, итогПоПлатформам, всего, описи, опись, блокКарты };
}
