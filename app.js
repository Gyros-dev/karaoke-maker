...
const state = {
  ...
  /* Байты выбранного файла песни как есть. Нужны проекту папкой:
     в него ложится ИСХОДНЫЙ файл человека, а не наша пересжатая копия
...
function defaultStyle() {
  return {
    ...
    /* Цвета партий дуэта. Раньше стояли числами в коде и в CSS, и
       ...
       в стиле, уезжают в проект и в видео (см. цветПартии). */
    part1: '#38bdf8',   // первый голос
    part2: '#fb7185',   // второй голос
    partBoth: '#fbbf24' // поют вместе
  };
}
...
/* ---------- Уровни дорожек в наушниках ----------
   ...
   Отдельной записи голоса нет: он берётся вычитанием минусовки из песни
...
/* Появление и затухание оригинала.

   Раньше на границе отрезка стояло короткое перекрёстное затухание
   в 50 мс — ровно столько, чтобы не щёлкало. Со стороны это «тихо-тихо,
   а потом сразу очень громко»: человек так и написал. Теперь у каждого
   отрезка свои появление и затухание, и по умолчанию они те же 50 мс —
   прежние проекты звучат как звучали.

   Затухание лежит ВНУТРИ отрезка, как ручки фейда у клипа в Final Cut:
   для длинного появления это единственный честный порядок — человек
   отметил кусок, и звук нарастает внутри него, а не заранее снаружи. */
const ОТРЕЗОК_ФЕЙД = 0.05;   // самое короткое затухание — чтобы не щёлкало
const ФЕЙД_МАКС = 5;         // дольше пяти секунд — это уже не появление
...
const УСИЛ_МАКС = 1.5;
/* Появление и затухание: не короче щелчкового минимума и не длиннее
   половины самого отрезка — иначе они наложились бы друг на друга
   и посреди куска оригинал не звучал бы в полную силу ни мгновения. */
function зажатьФейд(v, длина) {
  const предел = Math.max(ОТРЕЗОК_ФЕЙД, Math.min(ФЕЙД_МАКС, (длина || 0) / 2));
  const n = +v;
  if (!Number.isFinite(n)) return ОТРЕЗОК_ФЕЙД;
  return Math.min(предел, Math.max(ОТРЕЗОК_ФЕЙД, n));
}
...
function нормОтрезки(spans, длина) {
  ...
  for (const s of spans || []) {
    ...
    годные.push({
      start: a, end: b, усиление: зажатьУсиление(s.усиление),
      вход: зажатьФейд(s.вход, b - a), выход: зажатьФейд(s.выход, b - a),
    });
  }
  ...
  for (const s of годные) {
    ...
    /* Сошлись почти вплотную. Одинаково громкие сливаем, как и раньше;
       у слитого своё появление берём от первого, а затухание — от того,
       чей конец уехал дальше: это его край и слышно. А вот разной
       громкости сливать нельзя: это два разных решения человека,
       и одно из них пропало бы молча. Поэтому второй отодвигаем вправо,
       чтобы между кусками осталась щель, — иначе одна громкость
       перескакивала бы в другую рывком. Если после сдвига от него
       ничего не остаётся — сливаем, деваться некуда. */
    if (Math.abs(s.усиление - пред.усиление) < 0.001) {
      слить(пред, s);
      continue;
    }
    const start = пред.end + ОТРЕЗОК_ФЕЙД * 2;
    if (s.end - start < ОТРЕЗОК_МИН) { слить(пред, s); continue; }
    итог.push({ start, end: s.end, усиление: s.усиление,
      вход: зажатьФейд(s.вход, s.end - start), выход: зажатьФейд(s.выход, s.end - start) });
  }
  return итог;

  function слить(пред, s) {
    if (s.end > пред.end) { пред.end = s.end; пред.выход = s.выход; }
    const длина = пред.end - пред.start;
    пред.вход = зажатьФейд(пред.вход, длина);
    пред.выход = зажатьФейд(пред.выход, длина);
  }
}
...
/* Доля оригинала в момент t: 0 снаружи, 1 в середине отрезка, линейно
   на краях — это и есть перекрёстное затухание. Появление и затухание
   лежат ВНУТРИ отрезка ([start, start+вход] и [end−выход, end]): так
   ведут себя ручки фейда у клипа в монтажной, и только так длинное
   появление слышно там, где кусок отмечен, а не заранее до него. */
function отрезокВМомент(t, spans) {
  for (const s of spans) {
    if (t <= s.start || t >= s.end) continue;
    const усиление = зажатьУсиление(s.усиление);
    const вх = зажатьФейд(s.вход, s.end - s.start);
    const вых = зажатьФейд(s.выход, s.end - s.start);
    if (t < s.start + вх) return { доля: (t - s.start) / вх, усиление };
    if (t > s.end - вых) return { доля: (s.end - t) / вых, усиление };
    return { доля: 1, усиление };
  }
...
function точкиОтрезков(spans) {
  ...
  for (const s of spans) {
    const вх = зажатьФейд(s.вход, s.end - s.start);
    const вых = зажатьФейд(s.выход, s.end - s.start);
    pts.push(Math.max(0, s.start), s.start + вх, s.end - вых, s.end);
  }
  return pts;
}
...
async function проверкаОтрезков(исхПесня, исхМинус, SR, N) {
  ...
  const кр = 0.01;
  const вх = зажатьФейд(отрезки[0].вход, отрезки[0].end - отрезки[0].start);
  const вых = зажатьФейд(отрезки[0].выход, отрезки[0].end - отрезки[0].start);
  const вход = [отрезки[0].start - кр, отрезки[0].start + вх + кр];
  const выход = [отрезки[0].end - вых - кр, отрезки[0].end + кр];
...
/* Время «минуты:секунды,доли» — общий вид для всей студии: так оно
   стоит на линейке дорожки, в списке строк, в часах редактора и
...
/* Сотые доли — там, где число смотрят, а не правят: список строк
...
/* `ждёт` — огибающая, приехавшая с черновиком РАНЬШЕ песни. Разложить
   её тогда не по чему: проверка «та ли это песня» идёт по длительности,
   а длительности ещё нет. Лежит здесь до загрузки песни (см. handleFile). */
const voice = { level: null, runs: null, ждёт: null };
...
function clearVoiceTrack() {
  voice.level = null;
  voice.runs = null;
  /* Огибающая — половина признака «минусовка нейросети потерялась»
     (см. обновитьПотерюМинусовки): пропала она — пропадает и повод
     предупреждать. */
...
function restoreVoiceTrack(text, duration) {
  ...
  /* Ровно тот случай, ради которого заведено предупреждение: огибающая
     приехала из памяти, а звучит встроенное приглушение. */
  обновитьПотерюМинусовки();
}
...
/* Положить проект в хранилище, ужимаясь, если не влезает. Вынесено из
   saveProject, потому что тем же путём в хранилище ложится и открытый
   черновик: раньше он писался одним setItem без всякой лесенки — не влез
   (а в нём и картинка фона, и огибающая голоса), и молча не сохранялся
   ВЕСЬ. После перезапуска от открытого черновика не оставалось ничего. */
function положитьПроект(data) {
  try {
    localStorage.setItem('karaoke-project', JSON.stringify(data));
    return true;
  } catch (e) { /* ниже пробуем без тяжёлого */ }
  // Скорее всего не влезла картинка — сохраняем хотя бы текст и разметку
  const ужатый = { ...data };
  delete ужатый.bg;
  try {
    localStorage.setItem('karaoke-project', JSON.stringify(ужатый));
    return true;
  } catch (e) { /* ниже пробуем совсем налегке */ }
  delete ужатый.voice;
  try {
    localStorage.setItem('karaoke-project', JSON.stringify(ужатый));
    return true;
  } catch (e) { return false; }   // localStorage недоступен
}

function saveProject() {
  положитьПроект(собратьПроект());
  /* Черновик в хранилище обновился, а проект на диске — нет: с этой
     секунды папка отстала. Чип показывает это точкой. Обновляем
     не на каждую правку, а на переход «стало грязным»: saveProject
     зовут и по ходу перетаскивания ползунка. */
  if (проектНаДиске.путь && !проектНаДиске.грязный) {
...
function обновитьПамять() {
  ...
  const песня = п.имя ? п.имя.replace(/\.[^.]+$/, '') : t('память.безИмени');
  /* Пока проект лежит на диске, чип называет ЕГО, а не файл песни:
     это то, куда уйдёт следующее «сохранить». Без проекта на диске
     называть нечего, кроме песни, — как и было. */
  const имя = проектНаДиске.путь
    ? (проектНаДиске.имя || песня).replace(/\.[^.]+$/, '') : песня;
  const гнездо = $('proj-switch');
  const чип = $('proj-chip');
  if (гнездо) гнездо.classList.toggle('hidden', !п.есть);
  if (чип) {
    // Имя файла — текст человека, только textContent
    $('proj-name').textContent = имя;
    /* Точка — «в памяти есть то, чего нет в папке». Показываем её
       только когда папка вообще выбрана: пока проект не сохраняли,
       несохранённым числится всё, и точка не сказала бы ничего. */
    const грязно = !!(проектНаДиске.путь && проектНаДиске.грязный);
    класс($('proj-dot'), 'hidden', !грязно);
    /* Открытый проект называем в подсказке: по ней видно, куда уйдёт
       следующее «сохранить», — как строка пути в монтажных программах. */
    чип.title = (п.всего
      ? t('память.строк', { 'имя': имя, n: п.размечено, 'всего': п.всего })
      : t('память.чип.подсказка'))
      + (проектНаДиске.путь ? '\n' + t('проект.чип', { 'путь': проектНаДиске.путь }) : '')
      + (грязно ? '\n' + t('проект.несохранено') : '')
      // Тональность — часть работы, и в подсказке чипа ей место
      + (тон.выбран ? ' · ' + t('тон.чип', { 'выбрано': тонПодпись(тон.выбран) }) : '');
    чип.setAttribute('aria-label', чип.title);
    собратьМенюПроекта();
  }
  ...
  // Список недавних зависит от того же: открыта песня или ещё нет
  отрисоватьНедавние();
...
function применитьЧерновик(проект) {
  ...
  /* Пишем в хранилище как есть, а не через saveProject: имя песни
     ...
     не быть вовсе — песню ещё не открыли). Но той же лесенкой:
     черновик с картинкой фона в хранилище целиком не влезает. */
  положитьПроект(проект);
  ...
  /* Огибающая голоса. Она про ПЕСНЮ, а не про разметку, и лечит здесь
     сразу две беды.

     Первая: черновик привёз огибающую, а песни ещё нет — длительности,
     по которой сверяют «та ли это песня», не существует. Раньше
     огибающая в этом месте просто пропадала, и вся надежда была
     на хранилище, куда проект мог и не влезть. Теперь ждёт песню.

     Вторая: черновик БЕЗ огибающей (сделан в браузере, где вокал не
     отделяют) ложился поверх работы, где огибающая есть, — и сносил её.
     А без огибающей строка без разметки слов держится не столько,
     сколько поётся, а привычные несколько секунд: текст на просмотре
     сменялся заметно раньше, чем певец допевал. Своей не привёз —
     чужую не трогаем. */
  voice.ждёт = null;
  if (проект.voice) {
    if (длит) {
      restoreVoiceTrack(проект.voice, длит);
    } else {
      clearVoiceTrack();
      voice.ждёт = проект.voice;
    }
  }
...
/* Подписи файла разметки. На сайте это единственный способ сохраниться,
   и называется он «черновик». В приложении «сохранить» — это проект
   папкой, а .kpunch делает другое дело: уносит ОДНИ времена, без звука.
   Одинаковые подписи у двух пар кнопок читались как два равных способа
   сохраниться, и было непонятно, какой главный.

   Разметку не трогаем — в ней стоят ключи сайта, — а поверх них пишем
   свои. Поэтому зовётся это и при запуске, и после каждой смены языка:
   перевод разметки возвращает сайтовые подписи на место. */
function обновитьПодписиЧерновика() {
  const вПриложении = !!(window.desktop && window.desktop.isDesktop);
  if (!вПриложении) return;
  const пары = [
    ['btn-draft-save', 'черновик.разметка'],
    ['btn-draft-open', 'черновик.разметкаОткрыть'],
  ];
  for (const [id, ключ] of пары) {
    const кнопка = $(id);
    if (!кнопка) continue;
    кнопка.title = t(ключ + '.подсказка');
    кнопка.setAttribute('aria-label', t(ключ));
  }
}
...
/* ============================================================
   ...
   Черновик — это разметка без звука. Он лёгкий, его удобно переслать,
   но открывать его надо поверх той же самой песни, а минусовку
   нейросети он не приносит вовсе: после перезапуска её приходится
   считать заново, по четверть часа на песню.

   Проект устроен так же, как в монтажных программах: это ПАПКА,
   в которой лежит всё сразу — сама песня, минусовка, картинка фона
   и разметка. Открыл папку — студия встала ровно туда, где её
   оставили: со звуком, с минусовкой нейросети и без единого пересчёта.

   Почему папка, а не один файл. В папку можно заглянуть обычным
   проводником: забрать оттуда минусовку .wav, подменить фон, отдать
   песню другому человеку. Имена внутри человеческие и по-русски —
   песня.mp3, минусовка.wav, фон.jpg, проект.json, — потому что читать
   их будет человек, а не студия: студия идёт по описи в проект.json.

   Только в приложении: писать папки на диск умеет оно, а браузеру
   такого не дают. На сайте остаётся черновик — им же работа
   и переносится с сайта в приложение.
   ============================================================ */
const ФОРМАТ_ПРОЕКТА = 'karaoke-punch-проект';
const ВЕРСИЯ_ПРОЕКТА = 1;
const РАСШИРЕНИЕ_ПРОЕКТА = 'karaokeproj';
const ИМЯ_ОПИСИ = 'проект.json';

/* Где лежит открытый проект. Путь помнится ради «сохранить» без
   вопросов: второй раз папку не спрашиваем, как и монтажные программы. */
/* Где лежит проект и отстал ли он от того, что в памяти.
   «грязный» — то самое несохранённое, о котором говорит точка в чипе. */
const проектНаДиске = { путь: null, имя: null, занят: false, грязный: false };

/* Мостик к диску спрашиваем функцией, а не берём window.desktop прямо.
   Причина простая: contextBridge отдаёт window.desktop только на чтение,
   подменить в нём одну функцию нельзя, — а самопроверке надо провести
   круг «сохранил — открыл» без настоящих окон выбора папки. Она кладёт
   свой мостик в window.__мостПроекта, и запись с чтением идут при этом
   настоящие, через диск. */
function мостПроекта() {
  return window.__мостПроекта || window.desktop || null;
}

function естьПроектнаяПапка() {
  const м = мостПроекта();
  return !!(м && м.projectPick);
}

const расширениеИмени = (имя) => {
  const m = /\.[^.\\/]+$/.exec(String(имя || ''));
  return m ? m[0].toLowerCase() : '';
};

const РАСШИРЕНИЯ_КАРТИНОК = {
  'image/jpeg': '.jpg', 'image/png': '.png',
  'image/webp': '.webp', 'image/gif': '.gif', 'image/avif': '.avif',
};

/* Картинка фона живёт в памяти строкой data:. В папке ей место
   отдельным файлом: проект.json остаётся читаемым, а картинку можно
   подменить, не трогая разметку. */
function изДатаУрл(url) {
  const m = /^data:([^;,]*)(;base64)?,([\s\S]*)$/.exec(String(url || ''));
  if (!m) return null;
  const тип = m[1] || 'application/octet-stream';
  let байты;
  if (m[2]) {
    const bin = atob(m[3]);
    байты = new Uint8Array(bin.length);
    for (let i = 0; i < bin.length; i++) байты[i] = bin.charCodeAt(i);
  } else {
    байты = new TextEncoder().encode(decodeURIComponent(m[3]));
  }
  return { тип, байты };
}

function вДатаУрл(байты, тип) {
  let s = '';
  const CHUNK = 0x8000;   // длинные списки аргументов ломают apply
  for (let i = 0; i < байты.length; i += CHUNK) {
    s += String.fromCharCode.apply(null, байты.subarray(i, i + CHUNK));
  }
  return `data:${тип || 'application/octet-stream'};base64,${btoa(s)}`;
}

/* Что кладём в папку: опись и список файлов. Разметка — тот же самый
   проект, что уходит в черновик и в хранилище, только без картинки:
   она поехала отдельным файлом. */
async function собратьПапкуПроекта() {
  const проект = собратьПроект();
  const файлы = [];
  const опись = { песня: null, минусовка: null, фон: null };

  if (state.songBytes && state.songBytes.length) {
    const имя = 'песня' + (расширениеИмени(state.fileName) || '.mp3');
    файлы.push({ имя, данные: state.songBytes });
    опись.песня = { файл: имя, исходноеИмя: state.fileName || имя };
  }

  /* Минусовку кладём только СВОЮ — ту, что посчитала нейросеть или
     принёс человек. Встроенное приглушение считается из песни за
     полсекунды: держать ради него сорок мегабайт .wav незачем. */
  if (state.customInst && state.instrumentalBuffer) {
    const blob = bufferToWav(state.instrumentalBuffer);
    файлы.push({ имя: 'минусовка.wav', данные: new Uint8Array(await blob.arrayBuffer()) });
    опись.минусовка = { файл: 'минусовка.wav', имя: state.instName || null };
  }

  if (state.bgImage) {
    const к = изДатаУрл(state.bgImage);
    if (к) {
      const имя = 'фон' + (РАСШИРЕНИЯ_КАРТИНОК[к.тип] || '.img');
      файлы.push({ имя, данные: к.байты });
      опись.фон = { файл: имя, тип: к.тип };
    }
  }

  const разметка = { ...проект };
  delete разметка.bg;          // картинка уехала отдельным файлом
  const опись_ = {
    формат: ФОРМАТ_ПРОЕКТА,
    версия: ВЕРСИЯ_ПРОЕКТА,
    студия: APP_VERSION,
    сохранён: new Date().toISOString(),
    ...опись,
    проект: разметка,
  };
  файлы.push({
    имя: ИМЯ_ОПИСИ,
    данные: new TextEncoder().encode(JSON.stringify(опись_, null, 1)),
  });
  return файлы;
}

/* Короткий ответ на «сохранить»: галочка на кнопке вместо значка папки.
   Окошком «сохранено» отвечать нельзя — Cmd+S жмут часто, и закрывать
   окно пришлось бы каждый раз. */
let вернутьЗначокПроекта = null;
function мигнутьСохранением() {
  const кнопка = $('btn-proj-save');
  const use = кнопка && кнопка.querySelector('use');
  if (!use) return;
  clearTimeout(вернутьЗначокПроекта);
  use.setAttribute('href', '#i-check');
  кнопка.classList.add('saved');
  вернутьЗначокПроекта = setTimeout(() => {
    use.setAttribute('href', '#i-folder-save');
    кнопка.classList.remove('saved');
  }, 1400);
}

function обновитьКнопкиПроекта() {
  const s = $('btn-proj-save');
  const o = $('btn-proj-open');
  if (s) s.disabled = проектНаДиске.занят;
  if (o) o.disabled = проектНаДиске.занят;
}

async function сохранитьПроект(какНовый) {
  if (!естьПроектнаяПапка() || проектНаДиске.занят) return false;
  const п = сведенияОПамяти();
  if (!п.есть) { alert(t('проект.нечего')); return false; }

  let путь = какНовый ? null : проектНаДиске.путь;
  if (!путь) {
    const основа = (state.fileName || проектНаДиске.имя || t('память.безИмени'))
      .replace(/\.[^.]+$/, '');
    const ответ = await мостПроекта().projectPick(`${основа}.${РАСШИРЕНИЕ_ПРОЕКТА}`);
    if (!ответ || !ответ.ok) return false;
    путь = ответ.path;
  }

  проектНаДиске.занят = true;
  обновитьКнопкиПроекта();
  try {
    const файлы = await собратьПапкуПроекта();
    const ответ = await мостПроекта().projectWrite(путь, файлы);
    if (!ответ || !ответ.ok) {
      alert(t('проект.неЗаписался', { 'причина': (ответ && ответ.error) || '' }));
      return false;
    }
    проектНаДиске.путь = путь;
    проектНаДиске.имя = ответ.name || путь;
    проектНаДиске.грязный = false;
    запомнитьПроект(путь, проектНаДиске.имя);
    обновитьПамять();
    мигнутьСохранением();
    return true;
  } catch (e) {
    alert(t('проект.неЗаписался', { 'причина': (e && e.message) || String(e) }));
    return false;
  } finally {
    проектНаДиске.занят = false;
    обновитьКнопкиПроекта();
  }
}

/* Путь можно передать готовым — так открываются недавние проекты:
   там папка уже названа, и спрашивать её окном выбора значило бы
   заставить человека ткнуть в то же самое второй раз. */
async function открытьПроект(готовыйПуть) {
  if (!естьПроектнаяПапка() || проектНаДиске.занят) return;
  let выбор = null;
  if (готовыйПуть) {
    выбор = { ok: true, path: готовыйПуть, name: имяПапки(готовыйПуть) };
  } else {
    выбор = await мостПроекта().projectOpenPick();
  }
  if (!выбор || !выбор.ok) return;
  const путь = выбор.path;

  проектНаДиске.занят = true;
  обновитьКнопкиПроекта();
  try {
    const описьОтвет = await мостПроекта().projectRead(путь, ИМЯ_ОПИСИ);
    let данные = null;
    if (описьОтвет && описьОтвет.ok) {
      try { данные = JSON.parse(new TextDecoder().decode(описьОтвет.data)); }
      catch (e) { данные = null; }
    }
    if (!данные || данные.формат !== ФОРМАТ_ПРОЕКТА || !данные.проект) {
      /* Папку могли унести или переименовать — про недавний проект это
         обычное дело. Тогда и говорим об этом прямо, и убираем его
         из списка, а не оставляем строку, которая больше не работает. */
      if (готовыйПуть) {
        забытьПроект(путь);
        alert(t('проект.пропал', { 'путь': путь }));
      } else alert(t('проект.неПрочитался'));
      return;
    }
    if (+данные.версия > ВЕРСИЯ_ПРОЕКТА
      && !confirm(t('черновик.новее',
        { v: данные.студия || данные.версия, 'своя': APP_VERSION }))) return;

    // Поверх чужой работы — только с разрешения: разметку не вернуть
    const было = сведенияОПамяти();
    const имяНового = (выбор.name || данные.проект.name || t('память.безИмени'))
      .replace(/\.[^.]+$/, '');
    if (было.размечено && !confirm(t('черновик.поверх', {
      'прежняя': (было.имя || t('память.безИмени')).replace(/\.[^.]+$/, ''),
      n: было.размечено,
      'новая': имяНового,
    }))) return;
    ...
    /* Песня. Кладём проект в хранилище и объявляем его имя ЗАРАНЕЕ:
       тогда handleFile видит ту же самую песню, не спрашивает
       «открыть другую?» и сам поднимает из проекта всё, что считается
       от длительности, — отрезки оригинала и огибающую голоса. */
    if (данные.песня && данные.песня.файл) {
      const ответ = await мостПроекта().projectRead(путь, данные.песня.файл);
      if (!ответ || !ответ.ok) { alert(t('проект.нетПесни')); return; }
      state.lines = [];
      state.fileName = проект.name || данные.песня.исходноеИмя || null;
      положитьПроект(проект);
      await handleFile(new File([ответ.data],
        данные.песня.исходноеИмя || данные.песня.файл));
    }

    /* Минусовка — до разметки: своя минусовка сбрасывает тональность
       (сменилсяЗвук), а выбор тональности приезжает вместе с разметкой. */
    if (данные.минусовка && данные.минусовка.файл && state.originalBuffer) {
      const ответ = await мостПроекта().projectRead(путь, данные.минусовка.файл);
      if (ответ && ответ.ok) {
        await handleInstFile(new File([ответ.data], данные.минусовка.файл),
          данные.минусовка.имя || null);
      }
    }

    // Картинка фона возвращается в проект тем же видом, каким жила
    if (данные.фон && данные.фон.файл) {
      const ответ = await мостПроекта().projectRead(путь, данные.фон.файл);
      if (ответ && ответ.ok) проект.bg = вДатаУрл(ответ.data, данные.фон.тип);
    }

    применитьЧерновик(проект);
    проектНаДиске.путь = путь;
    проектНаДиске.имя = выбор.name || имяНового;
    /* Только что открыли — значит, папка и память сошлись. Флаг снимаем
       ПОСЛЕ применитьЧерновик: он правит разметку, а каждая правка
       по дороге зовёт saveProject и снова помечает работу грязной. */
    проектНаДиске.грязный = false;
    запомнитьПроект(путь, проектНаДиске.имя);

    const размечено = state.lines.filter((l) => l.time != null).length;
    if (state.originalBuffer && размечено) goToStep(3);
    else if (state.originalBuffer) goToStep(2);
    else goToStep(1);
    обновитьПамять();
    alert(t('проект.открыт', { 'всего': state.lines.length, n: размечено }));
  } catch (e) {
    alert(t('проект.неПрочитался'));
  } finally {
    проектНаДиске.занят = false;
    обновитьКнопкиПроекта();
  }
...
/* ============================================================
   ...
   Проект папкой был единственным способом вернуться ко вчерашней
   работе — и каждый раз через окно выбора папки: вспомни, где она
   лежит, дойди до неё мышью. Список недавних решает ровно это и
   ничего больше: он стоит на первом шаге, пока песня не открыта,
   и хранит пути, а не работу.

   Пути — не тайна, но и не то, что стоит разносить: список живёт
   в хранилище самой студии, никуда не уходит и чистится по одному
   промаху (папку унесли — строка исчезает).
   ============================================================ */
const КЛЮЧ_НЕДАВНИХ = 'karaoke-recent';
const НЕДАВНИХ_МАКС = 6;

function имяПапки(путь) {
  const части = String(путь || '').split(/[\\/]+/).filter(Boolean);
  return (части[части.length - 1] || путь || '').replace(/\.[^.]+$/, '');
}

/* Путь в строке списка — приметой, а не целиком: полный не влезает,
   а обрезать его концом нельзя — там как раз имя папки. Показываем
   последние две ступени: «…/Музыка/Моя песня». Полный путь остаётся
   в подсказке. Обрезать средствами CSS (direction: rtl) не годится:
   с ним начальная косая черта уезжает в конец строки. */
function короткийПуть(путь) {
  const части = String(путь || '').split(/[\\/]+/).filter(Boolean);
  if (части.length <= 2) return путь;
  return '…/' + части.slice(-2).join('/');
}

function недавниеПроекты() {
  try {
    const с = JSON.parse(localStorage.getItem(КЛЮЧ_НЕДАВНИХ));
    return Array.isArray(с) ? с.filter((з) => з && typeof з.путь === 'string') : [];
  } catch (e) { return []; }
}

function положитьНедавние(список) {
  try {
    localStorage.setItem(КЛЮЧ_НЕДАВНИХ, JSON.stringify(список.slice(0, НЕДАВНИХ_МАКС)));
  } catch (e) { /* некуда писать — список просто не переживёт запуск */ }
  отрисоватьНедавние();
}

function запомнитьПроект(путь, имя) {
  if (!путь) return;
  const был = недавниеПроекты().filter((з) => з.путь !== путь);
  положитьНедавние([{ путь, имя: имя || имяПапки(путь), когда: Date.now() }, ...был]);
}

function забытьПроект(путь) {
  положитьНедавние(недавниеПроекты().filter((з) => з.путь !== путь));
}

function отрисоватьНедавние() {
  const блок = $('recent');
  const список = $('recent-list');
  if (!блок || !список) return;
  const записи = естьПроектнаяПапка() ? недавниеПроекты() : [];
  /* Показываем, только пока песня не открыта: дальше это уже не путь
     назад к работе, а лишняя строка посреди карточки трека. */
  блок.classList.toggle('hidden', !записи.length || !!state.originalBuffer);
  список.textContent = '';
  for (const з of записи) {
    const кн = document.createElement('button');
    кн.type = 'button';
    кн.className = 'recent-item';
    кн.appendChild(значокSVG('folder'));
    const имя = document.createElement('span');
    имя.className = 'recent-name';
    имя.textContent = з.имя || имяПапки(з.путь);
    кн.appendChild(имя);
    const путь = document.createElement('span');
    путь.className = 'recent-path';
    путь.textContent = короткийПуть(з.путь);
    кн.appendChild(путь);
    кн.title = t('недавние.открыть', { 'путь': з.путь });
    кн.addEventListener('click', () => открытьПроект(з.путь));
    список.appendChild(кн);
  }
}

/* Показать папку проекта в системе — «Show in Finder» монтажной
   программы. Отвечает не окном «готово», а тем, что папка открылась;
   говорим только когда НЕ вышло. */
async function показатьПапкуПроекта() {
  if (!проектНаДиске.путь || !мостПроекта() || !мостПроекта().projectReveal) return;
  const ответ = await мостПроекта().projectReveal(проектНаДиске.путь);
...
/* ---------- Меню чипа ----------/* ---------- Меню чипа ----------
   ...
   сохраниться), на сайте — черновик файлом (единственный). Пункты
   собираются заново на каждое обновитьПамять: их состав зависит
   и от языка, и от того, лежит ли проект на диске. */
function собратьМенюПроекта() {
  const меню = $('proj-menu');
  if (!меню) return;
  const пункты = [];
  if (естьПроектнаяПапка()) {
    пункты.push({ значок: 'folder-save', ключ: 'проект.сохранить', дело: () => сохранитьПроект(false) });
    пункты.push({ значок: 'folder-save', ключ: 'проект.сохранитьКак', дело: () => сохранитьПроект(true) });
    пункты.push({ значок: 'folder', ключ: 'проект.открыть', дело: () => открытьПроект() });
    пункты.push({
      значок: 'folder', ключ: 'проект.показать', выкл: !проектНаДиске.путь,
      дело: () => показатьПапкуПроекта(),
    });
  }
  /* В приложении «черновик» называется иначе: там главный способ
     сохраниться — проект папкой, а файл разметки уносит одни времена
     (см. обновитьПодписиЧерновика). Меню обязано звать их так же,
     как кнопки рядом, иначе это читалось бы как разные действия. */
  const вПриложении = естьПроектнаяПапка();
  пункты.push({
    значок: 'download', дело: () => сохранитьЧерновик(),
    ключ: вПриложении ? 'черновик.разметка' : 'черновик.сохранить',
  });
  пункты.push({
  ...
  for (const п of пункты) {
    ...
    кн.disabled = !!п.выкл || проектНаДиске.занят;
    кн.addEventListener('click', () => {
      показатьСписок($('proj-switch'), false);
      п.дело();
    });
    кн.addEventListener('keydown', (e) => {
      if (e.key === 'ArrowDown') { e.preventDefault(); перейтиПоСписку(меню, кн, 1); }
        ...
        показатьСписок($('proj-switch'), false);
        $('proj-chip').focus();
      } else if (e.key === 'Tab') показатьСписок($('proj-switch'), false);
    });
    меню.appendChild(кн);
  }
}
...
if ($('proj-chip')) {
  const гнездо = $('proj-switch');
  $('proj-chip').addEventListener('click', () => показатьСписок(гнездо, !списокВиден(гнездо)));
  $('proj-chip').addEventListener('keydown', (e) => {
    if (e.key === 'ArrowDown' || e.key === 'ArrowUp') { e.preventDefault(); показатьСписок(гнездо, true); }
  });
}

if ($('btn-proj-save')) {
  $('btn-proj-save').addEventListener('click', () => сохранитьПроект(false));
  $('btn-proj-open').addEventListener('click', () => открытьПроект());
}

/* Cmd+S / Ctrl+S — сохранить, с Shift — сохранить как. Cmd+O — открыть.
   Ускорители живут на странице, а не в меню приложения: меню у нас
   намеренно урезано до «Правки» (см. собратьМеню в main.js), и заводить
   ради двух пунктов полосу меню на Windows незачем. */
window.addEventListener('keydown', (e) => {
  if (!естьПроектнаяПапка()) return;
  if (!(e.metaKey || e.ctrlKey) || e.altKey) return;
  const кл = (e.key || '').toLowerCase();
  if (кл === 's') {
    e.preventDefault();
    сохранитьПроект(!!e.shiftKey);
  } else if (кл === 'o' && !e.shiftKey) {
    e.preventDefault();
    открытьПроект();
  }
});
...
function goToStep(n) {
  ...
  /* Клавиатуру — странице. На Windows после долгой работы (разметка
     текста, удаление вокала) фокус остаётся у окна, не доходя до
     страницы: мышь работает и текст выделяется, а буквы не идут никуда.
     Смена шага — самый частый способ, которым человек пробует «а вдруг
     теперь заработает», и заодно самое дешёвое место это починить. */
  if (window.__вернутьКлавиатуру) window.__вернутьКлавиатуру();
  ...
  /* Весть о смене шага наружу. Нужна тем частям, которые живут только
     в приложении (desktop.js) и обязаны пересмотреть свои надписи,
     когда человек пришёл на их шаг, — например метке о готовой
     разметке: она врёт, если работу с тех пор сменили. */
  document.dispatchEvent(new CustomEvent('шаг', { detail: { шаг: n } }));
}

document.querySelectorAll('.step-tab').forEach((tab) => {
  tab.addEventListener('click', () => {
    const шаг = +tab.dataset.step;
    /* В редактор — только через разбор текста (см. кРедактору): иначе
       вставленный, но ещё не разобранный текст даёт пустой редактор. */
    if (шаг === 3) кРедактору();
    else goToStep(шаг);
  });
});
...
async function handleFile(file) {
  ...
  if (другая) {
    ...
    /* В очереди не спрашиваем: человек сам сложил в неё песни и ушёл,
       а прежняя работа с каждой из них уже сохранена в свою папку
       (см. пуститьОчередь в desktop.js). Вопрос посреди очереди
       остановил бы её до первого возвращения к экрану. */
    if (размечено && !window.__очередьИдёт) {
  ...
  try {
    ...
    /* Байты песни держим при себе — из них собирается проект папкой.
       Копию снимаем ДО разбора: decodeAudioData забирает буфер себе,
       и после него data пуст. Копия — это исходный файл человека,
       а не пересжатая нами дорожка: в папке лежит ровно то, что он
       выбрал, вплоть до обложки и тегов. */
    const байтыПесни = new Uint8Array(data.slice(0));
    ...
    /* Огибающая, приехавшая с черновиком ДО песни: длину узнали только
       сейчас, теперь есть с чем сверить. Держим её отдельно от хранилища
       нарочно — проект с картинкой фона туда мог и не влезть, а без
...
/* ---------- Минусовка нейросети не пережила перезапуск ----------
   ...
   «голос» на дорожке стоит на месте, всё выглядит так, будто вокал
   убран нейросетью, — а звучит вычитание середины. Человек идёт
   в караоке, слышит оригинал и решает, что сломалась студия.

   Признак беды описан состоянием, а не тем, где мы работаем: огибающая
   голоса ЕСТЬ, а минусовка НЕ СВОЯ — значит, звучит встроенное
   приглушение. После разделения нейросетью и после загрузки своего
   файла state.customInst истинно, и предупреждения нет: надоедать
   тому, кто только что убрал вокал, незачем. На сайте нейросети нет
   вовсе, и условие там просто не сходится — своего кода веб-версии
   для этого не нужно. */
function потерянаМинусовкаНейросети() {
  return voiceReady() && !state.customInst;
}

function обновитьПотерюМинусовки() {
  const узел = $('voice-lost');
  if (узел) узел.classList.toggle('hidden', !потерянаМинусовкаНейросети());
}

/* `подпись` — как назвать минусовку в интерфейсе. Обычно это имя
   выбранного файла, но проект папкой хранит минусовку под своим
   именем (минусовка.wav), а показать надо то, что было: «Минусовка
   нейросети». */
async function handleInstFile(file, подпись) {
...
/* Цвет партии. Один источник правды — оформление: холст переменных CSS
   не читает, поэтому раньше числа лежали и здесь, и в style.css, и
   разъезжались бы при первой же правке. Теперь их держит state.style,
   а в CSS они приезжают переменными --part-* (см. applyStyle).
   Ключ стиля новый: в проектах постарше его нет, и умолчание
   подкладывает styleFromSaved. */
const КЛЮЧ_ПАРТИИ = { 1: 'part1', 2: 'part2', 3: 'partBoth' };
const ЦВЕТ_ПАРТИИ_ПО_УМОЛЧАНИЮ = { 1: '#38bdf8', 2: '#fb7185', 3: '#fbbf24' };

function цветПартии(п) {
  const ключ = КЛЮЧ_ПАРТИИ[п];
  if (!ключ) return null;
  const свой = state.style && state.style[ключ];
  return свой || ЦВЕТ_ПАРТИИ_ПО_УМОЛЧАНИЮ[п];
}
...
/* Переход к редактору. Тут не просто смена шага: текст из поля надо
   разобрать на строки, сверить с прежней разметкой и разложить времена.

   Оттого и беда, которую это лечит: кнопка «К редактору» внизу звала
   этот разбор, а вкладка «3 Редактор» вверху — сразу goToStep(3).
   Человек вставлял текст, жал вкладку и попадал в ПУСТОЙ редактор:
   строк-то ещё нет. Возвращался, жал нижнюю кнопку — и всё появлялось.
   Теперь обе двери ведут через одну прихожую. */
function кРедактору() {
  const raw = $('lyrics-input').value;
  const texts = raw.split('\n').map((s) => s.trim()).filter(Boolean);
  if (!texts.length) {
    /* Поле пустое, а разметка есть — значит, работа уже сделана
       (открыли проект, вернулись из караоке). Ругаться не за что:
       просто пускаем в редактор. */
    if (state.lines.length) { goToStep(3); return; }
    alert(t('текст.пусто'));
    return;
  }
...
/* Просвет между блоками строк на дорожке: две строки подряд рисуются
   не впритык, иначе на глаз они сливаются в один блок. Из-за него же
   у соседних строк нет ТОЧНОГО совпадения конца и начала — это надо
   помнить всюду, где стык ищут по временам (см. timelineHit). */
const ЗАЗОР_БЛОКОВ = 0.02;
...
function lineSpan(lines, index) {
  ...
  const limit = докуда != null ? докуда - ЗАЗОР_БЛОКОВ : (audio.duration || start + SING_DUR);
  ...
  if (line.words && line.words.length) {
    ...
    /* Конец, пришедший от нейросети (или от разметки слов, которую потом
       сбросили): это конец последнего расслышанного слова — измерение,
       а не выдумка. Раньше его тут выбрасывали, и строка без разметки
       слов держалась ровно привычную длину. Стоило огибающей голоса
       не оказаться под рукой — открыли черновик, перезапустили
       приложение, — и текст на просмотре начинал сменяться заметно
       раньше, чем поётся. Голосом его по-прежнему продлеваем ниже:
       распев обрывать нельзя. */
    core = fit(line.end + (start - line.time));
  } else {
...
/* Слова внутри строки, распев и «сколько уже спето» переехали
   в timing.js — чистую арифметику разметки, которую гоняют
   быстрые проверки из Node (см. test/timing.test.js). */
...
/* ---------- Единый кегль на всю песню ----------
   ...
   караоке, просмотра редактора и кадра видео, — поэтому все
...
/* Ширины строк в долях кегля: строка с em = 12,5 при кегле 40 px займёт
   ...
   и считали ширину пропорциональной, поэтому в просмотре редактора
...
/* Места двух закреплённых строк, в пикселях от верха поверхности.
   ...
   просмотра и кадра видео, поэтому все трое кладут строки туда же. */
...
/* Единый кегль для сцены на экране. Считаем по ширине самой сцены:
   караоке меряется по караоке, просмотр редактора — по себе.
...
/* То же самое для узла, который уже в руках, — и для полей ввода.
   ...
   В списке строк таких подписей полторы сотни, и панель инспектора
...
function tickPlayer() {
  ...
  if ($('step-4').classList.contains('active')) {
    updatePlayerUI();
    updateStageFill();
    // Не пора ли хвалить: считается от конца последней строки, а не
    // от конца песни (см. времяФинала)
  }
...
/* ============================================================
   ...
   Песня кончилась — и караоке обязано сказать спасибо. Так принято
   везде, и мы делаем это с перебором нарочно: цифра заведомо
   невозможная, и весь смысл в перегибе. «Спел на 384 951 726 %»
   смешно именно тем, что так не бывает; поставь мы честные 87 % —
   вышел бы экзамен, а не праздник.

   ЯЗЫК ПОХВАЛЫ — ПО ПЕСНЕ. Не по интерфейсу: обращаются к тому, кто
   только что стоял с микрофоном, и услышать он должен свой язык, даже
   если студия у него английская. Какой это язык, решает алфавит
   текста — двух языков нам хватает, и гадать сложнее незачем.

   Показывается только на шаге «Караоке» и только когда песня доиграла
   сама. Перемотал в конец руками — не праздник; идёт запись видео —
   тем более: в кадр это попасть не должно.
   ============================================================ */
/* Через сколько секунд после ПОСЛЕДНЕГО СЛОВА выходит похвала.

   Не после песни: у песни бывает проигрыш на полминуты, и всё это
   время певец стоял бы с микрофоном и ждал, спел он или нет. Слово
   кончилось — через пару секунд его хвалят, а музыка играет себе
   дальше. Так это и устроено в караоке. */
const ФИНАЛ_ПАУЗА = 3;
/* Куда бежит счётчик. Бежит он ВСЕГДА с нуля — так видно сам бег,
   а не только итог. Потолок нарочно скромнее прежнего миллиарда:
   девятизначное число читается как ошибка, а не как шутка, и в кадре
   его приходилось мельчить, чтобы влезло. */
const ФИНАЛ_СЧЁТ_ОТ = 200000;
const ФИНАЛ_СЧЁТ_ДО = 2000000;
const ФИНАЛ_БЕГ = 2200;        // сколько миллисекунд цифра бежит вверх
const ЛОГО_ПОДЛОЖКА = 0.28;    // до какой яркости гаснет логотип под счётом
const ЛОГО_ГАСНЕТ = 0.45;      // за сколько секунд он туда уходит

/* Язык песни по алфавиту текста. Кириллица есть — русский, нет —
   английский. Считаем по буквам, а не по одной первой строке: в тексте
   попадаются и латинские вставки, и цифры, и знаки. */
function языкПесни() {
  const текст = state.lines.length
    ? state.lines.map((l) => l.text).join(' ')
    : ($('lyrics-input') ? $('lyrics-input').value : '');
  let кир = 0;
  let лат = 0;
  for (const ch of текст) {
    if (/[А-Яа-яЁё]/.test(ch)) кир++;
    else if (/[A-Za-z]/.test(ch)) лат++;
  }
  if (!кир && !лат) return I18N.язык();   // текста нет — пусть будет язык студии
  return кир >= лат ? 'ru' : 'en';
}

function случайнаяПохвала(язык) {
  const список = (I18N.ПОХВАЛЫ && I18N.ПОХВАЛЫ[язык]) || [];
  if (!список.length) return '';
  return список[Math.floor(Math.random() * список.length)];
}

/* Число сплошняком, без разбивки на разряды.

   Сначала разряды были: «182 943 257» читается легче. Но в этом
   и беда — читать его не надо. Смысл цифры в том, что она нелепо
   огромная, и сплошная колонна знаков говорит это с одного взгляда,
   а разбитая на тройки выглядит как настоящий счёт, который кто-то
   считал. */
function счётЦифрами(n) {
  return String(Math.round(n));
}

/* Когда в кадре выходит похвала: через ФИНАЛ_ПАУЗА после конца
   последней спетой строки. Нет разметки — считаем по концу песни:
   хвалить всё равно за что-то надо.

   На ЭКРАНЕ финала нет вовсе. Он был: после песни студия показывала
   похвалу и счёт окном поверх сцены. Но караоке — это то, что видно
   на сцене, а праздник с логотипами место имеет в готовом видео,
   рядом с водяным знаком: там он часть подарка, а в студии — окно,
   которое надо закрыть. */
function времяФинала() {
  const строки = syncedLines();
  if (!строки.length) return audio.duration || null;
  const конец = lineSpan(строки, строки.length - 1).end;
  return конец + ФИНАЛ_ПАУЗА;
}
...
function applyStyle() {
  ...
  /* Цвета партий — всему документу разом: ими красятся ряды списка
     ...
     в оформлении. Ставим на корень, потому что список и кнопки лежат
     вне сцены. */
  const корень = document.documentElement;
  корень.style.setProperty('--part-1', цветПартии(1));
  корень.style.setProperty('--part-2', цветПартии(2));
  корень.style.setProperty('--part-both', цветПартии(3));

  /* Оформление поменялось, а строки те же самые: слепок просмотра
...
function сбросПоДвойному(ползунок, умолчание, применить) {
  ...
  ползунок.addEventListener('dblclick', () => {
    const v = typeof умолчание === 'function' ? умолчание() : умолчание;
    применить(v);
    /* И сам кружок обязан встать на место. Панель отрезка нарочно
       не навязывает значение полю, пока оно в фокусе (иначе она спорила
       бы с тянущей рукой), — а после двойного щелчка поле как раз
       в фокусе: значение возвращалось, а кружок оставался где был.
       Человек так и написал: «сбрасывает значения, но сам ползунок
       остаётся на месте». */
    ставитьЗначение(ползунок, String(v));
  });
}
...
document.addEventListener('keydown', (e) => {
  ...
  // разворачивать под ним просмотр нечего, см. окноПоверхСтудии
...
/* ============================================================
   ...
   Тональность выбирали вслепую: жали ±1 и слушали, пока не станет
   удобно петь. Зная, в чём песня записана, человек видит, куда он её
   переносит: «ля минор, а с +2 будет си минор».

   Считает поток по чистому вокалу (key-worker.js в настольной части):
   метод Маклауда ищет ОДНУ основную частоту и работает на одноголосом
   звуке, а на полном миксе честно выдаёт мусор. Нет разделения — нет
   и ответа: соврать хуже, чем промолчать, и строка тогда не выходит.

   Имена нот даём и словами, и буквами: словами читает тот, кто поёт,
   буквами — тот, кто играет. Чёрные клавиши пишем диезами, все
   двенадцать одинаково: выбирать между «ре-диез» и «ми-бемоль» по
   правилам гармонии здесь не за чем — это подсказка певцу, а не нотный
   стан.
   ============================================================ */
const НОТЫ_БУКВЫ = ['C', 'C♯', 'D', 'D♯', 'E', 'F', 'F♯', 'G', 'G♯', 'A', 'A♯', 'B'];
const НОТЫ_СЛОВА = {
  ru: ['до', 'до-диез', 'ре', 'ре-диез', 'ми', 'фа', 'фа-диез', 'соль',
    'соль-диез', 'ля', 'ля-диез', 'си'],
  en: ['C', 'C♯', 'D', 'D♯', 'E', 'F', 'F♯', 'G', 'G♯', 'A', 'A♯', 'B'],
};

// Найденная тональность песни: { тоника, лад } или null
let тональностьПесни = null;

/* Короткое обозначение: Am, A♯m, C. Им подписан сам переключатель —
   человек крутит стрелки и сразу видит, в какую тональность попадёт,
   а не читает про это строкой ниже. */
function букваТональности(тоника, лад) {
  const к = ((тоника % 12) + 12) % 12;
  return НОТЫ_БУКВЫ[к] + (лад === 'минор' ? 'm' : '');
}

function имяТональности(тоника, лад) {
  const к = ((тоника % 12) + 12) % 12;
  const язык = I18N.английский() ? 'en' : 'ru';
  const слово = НОТЫ_СЛОВА[язык][к];
  return t('тон.имя', { 'нота': слово, 'лад': t(лад === 'минор' ? 'тон.минор' : 'тон.мажор'),
    'буква': букваТональности(тоника, лад) });
}

function поставитьТональностьПесни(тоника, лад) {
  тональностьПесни = { тоника, лад };
  обновитьТон();
}

function обновитьНайденнуюТональность() {
  const строка = $('key-found');
  if (!строка) return;
  if (!тональностьПесни) {
    строка.classList.add('hidden');
    писать(строка, '');
    return;
  }
  строка.classList.remove('hidden');
  /* Только исходная тональность песни. Куда попадёт сдвиг, написано
     теперь у самого переключателя — «+2 (Bm)», — и повторять это
     строкой ниже значит занимать высоту тем, что уже видно. */
  писать(строка, t('тон.песняВ', {
    'тональность': имяТональности(тональностьПесни.тоника, тональностьПесни.лад),
  }));
}
...
function обновитьТон() {
  ...
  /* Число и, если тональность песни известна, сама нота: «+2 (Bm)».
     Без ноты человек видел «+2» и лез глазами в строку под кнопками —
     а спрашивал он ровно про то, во что попадёт. Нота приписывается
     мельче: главное здесь всё-таки полутона. */
  писать(вал, тонПодпись(выбран));
  const нотаРядом = $('key-val-note');
  if (нотаРядом) {
    const есть = !!тональностьПесни;
    нотаРядом.classList.toggle('hidden', !есть);
    писать(нотаРядом, есть
      ? букваТональности(тональностьПесни.тоника + выбран, тональностьПесни.лад) : '');
  }
...
/* ============================================================
   ...
   ============================================================ */
let ключМенюЭкспорта = '';

function собратьМенюЭкспорта(насильно) {
  const меню = $('export-menu');
  if (!меню) return;
  /* Пересобираем, только когда меняется СОСТАВ или язык. Зовут эту
     функцию отовсюду, где меняется разметка слов (updateWordExportBtn),
     а перестройка узлов на каждый такой зов — холостая работа: её
     ловит раздел самопроверки «кадрыРедактора», который требует нуля
     правок дерева на паузе. */
  const ключ = I18N.язык() + ':' + (anyWords() ? 'слова' : '');
  if (!насильно && ключ === ключМенюЭкспорта && меню.childElementCount) return;
  ключМенюЭкспорта = ключ;
  const пункты = [
...
/* ---------- Сколько памяти у машины ----------
   Спрашиваем один раз и запоминаем: на сайте ответа нет вовсе (там
   и записи такой не бывает), в приложении отвечает главный процесс. */
let памятьМашины = null;
async function узнатьМашину() {
  if (памятьМашины !== null) return памятьМашины;
  памятьМашины = 0;
  try {
    const м = window.desktop && window.desktop.machineInfo
      ? await window.desktop.machineInfo() : null;
    памятьМашины = (м && +м.памятьГБ) || 0;
  } catch (e) { памятьМашины = 0; }
  return памятьМашины;
}

/* Тесной считаем машину с 8 ГБ и меньше: именно на такой (MacBook Air
   M1, 8 ГБ) запись 2K и кончилась чёрным экраном. Там 2K не запрещаем —
   человек вправе, — но по умолчанию ставим Full HD и говорим прямо,
   что выбор тяжёлый. */
const ПАМЯТЬ_ТЕСНО = 8;

function обновитьЦенуВидео() {
  const строка = $('video-size');
  const выбор = $('video-quality');
  if (!строка || !выбор) return;
  const качество = Number(выбор.value) || 1080;
  const мб = Math.round(весРолика(качество) / 1048576);
  const тяжело = памятьМашины > 0 && памятьМашины <= ПАМЯТЬ_ТЕСНО && качество >= 1440;
  строка.classList.toggle('тяжело', тяжело);
  /* Разметка своя, из словаря, — не чужой ввод: жирным выделен сам вес,
     ради него строка и существует. */
  строка.innerHTML = t(тяжело ? 'экспорт.цена.тесно' : 'экспорт.цена',
    { 'мб': мб, 'время': fmtTime(длинаРолика()), 'гб': памятьМашины });
}
...
function показатьОкноВидео(да) {
  ...
  if (да) {
    /* Цену считаем при каждом открытии: длина ролика зависит от песни
       и от подводки, а они между открытиями меняются. */
    узнатьМашину().then(() => {
      const выбор = $('video-quality');
      /* На тесной машине по умолчанию Full HD. Ставим один раз и только
         пока человек сам не выбрал: переставлять его выбор при каждом
         открытии — значит спорить с ним. */
      if (выбор && памятьМашины > 0 && памятьМашины <= ПАМЯТЬ_ТЕСНО && !выбор.dataset.свой) {
        выбор.value = '1080';
      }
      обновитьЦенуВидео();
    });
...
/* ============================================================
   ...
   Три поверхности, у каждой своя работа:
     • список строк — навигация и правка текста. Никаких кнопок
   ...
   Плюс просмотр караоке, один в один со сценой.
   ============================================================ */
const editor = {
  pxPerSec: 40,
  scrollT: 0,
  ...
  темпЖдём: null,     // счёт идёт в рабочем потоке: { буфер, обещание }
  drag: null,    // что тащим прямо сейчас, см. timelineHit
  stageKey: '',
  /* Что уже нарисовано на просмотре — слепок ЕГО СОДЕРЖИМОГО,
...
function spansKey(lines) {
  ...
  for (const l of lines) {
    ...
    if (l.ручноеНачало) k += '!';
    /* Конец входит в ключ ВСЕГДА, когда он есть: размах строки считается
       по нему и без пометки «поставлен руками» (см. lineSpan). Пока
       машинный конец в ключ не входил, дорожка после его правки
       оставалась нарисованной по-старому. */
    if (l.ручнойКонец) k += '#';
    if (l.end != null) k += `>${l.end.toFixed(3)}`;
...
/* ============================================================
   ...
   программах) или Shift+щелчком в списке строк. Границы выделения —
...
/* ============================================================
   ...
   Как считается — в tempo.js: спектральный поток начал (КПФ окном
   в 93 мс с шагом 5 мс, логарифмическое сжатие полос, сумма только
   ПОЛОЖИТЕЛЬНЫХ приращений), автокорреляция по 60–190 ударам
   в минуту, уточнение периода и фазы гребёнкой. Там же и разбор,
   почему по громкости это не читалось и что означает уверенность.

   ПОЧЕМУ В РАБОЧЕМ ПОТОКЕ. Спектральный поток стоит дорого: на песне
   в три минуты это двадцать четыре тысячи преобразований Фурье,
   500–630 мс на нашей машине против 60–70 мс у прежней огибающей.
   Полсекунды замершего окна при открытии редактора — это заметно,
   поэтому счёт уехал в рабочий поток, как у тональности и разделения
   вокала. Окна прогресса нет нарочно: ждать нечего, редактор
   открывается сразу, а показатель уверенности пока стоит на «…»
   и сменяется числом, когда поток ответит.
...
/* Оценка темпа на месте, без рабочего потока. Синхронная и потому
   годится самопроверке (ей нужны числа сразу) и запасным путём, если
   поток не завёлся. Сам алгоритм — в tempo.js, общий с потоком. */
function оценитьТемп(buf) {
  if (!buf || !buf.length) return null;
  return ТЕМП.оценить(buf.getChannelData(0), buf.sampleRate);
}

/* Тот же счёт, но в рабочем потоке: окно при этом не замирает.

   Дорожка уезжает КОПИЕЙ (передачей владения над копией, не над
   оригиналом): отдать сам буфер студии нельзя — он ещё играет.
   Копируем только разбираемый кусок, а не всю песню.

   Поток не завёлся (старый браузер, запрет политики безопасности) —
   считаем на месте: лучше замереть на полсекунды, чем остаться без
   темпа вовсе. */
let темпПоток = null;      // один на всю студию, заводится при первом счёте
let темпЗапросов = 0;      // номер запроса: см. ниже, зачем он нужен
const темпЖдущие = new Map();

/* Заказы НУМЕРУЮТСЯ, и ответы разбираются по номеру. Без этого второй
   заказ, поданный до ответа на первый, отбирал бы у него обработчик
   сообщений: первое обещание не разрешалось бы никогда, а тот, кто его
   ждал, ждал бы вечно. Заказы накладываются легко — открыли редактор,
   тут же досчиталась минусовка. */
function темпПоднятьПоток() {
  if (темпПоток) return темпПоток;
  const w = new Worker(`tempo.js?v=${APP_VERSION}`);
  w.onmessage = (e) => {
    const m = e.data || {};
    const ждёт = темпЖдущие.get(m.номер);
    if (!ждёт) return;
    темпЖдущие.delete(m.номер);
    ждёт(m.type === 'error' ? null : (m.итог || null));
  };
  w.onerror = () => {
    /* Разбитый поток больше не оживить: заводим новый со следующего
       раза, а всех ждущих отпускаем ни с чем — они досчитают на месте */
    try { w.terminate(); } catch (e2) {}
    if (темпПоток === w) темпПоток = null;
    const были = [...темпЖдущие.values()];
    темпЖдущие.clear();
    for (const ждёт of были) ждёт(undefined);
  };
  темпПоток = w;
  return w;
}

function темпВПотоке(buf) {
  const данные = buf.getChannelData(0);
  const нужно = Math.min(данные.length, Math.round(ТЕМП.ТЕМП_ОТРЕЗОК * buf.sampleRate));
  const кусок = данные.slice(0, нужно);
  let w;
  try { w = темпПоднятьПоток(); } catch (e) { return Promise.resolve(оценитьТемп(buf)); }
  const номер = ++темпЗапросов;
  return new Promise((resolve) => {
    // undefined — поток разбился; считаем на месте, чтобы не остаться без темпа
    темпЖдущие.set(номер, (итог) => resolve(итог === undefined ? оценитьТемп(buf) : итог));
    w.postMessage({ номер, данные: кусок.buffer, sampleRate: buf.sampleRate }, [кусок.buffer]);
  });
}
...
/* Оценка темпа для нынешней песни, с кэшем. Считаем по минусовке:
   ...
   Возвращает ОБЕЩАНИЕ: счёт идёт в рабочем потоке. Тем, кому число
   нужно прямо сейчас, годится editor.темпАвто — он всегда под рукой
   и пуст ровно до тех пор, пока поток не ответил.

   Пока считаем, помним об этом в editor.темпЖдём: второй заход за тем
   же самым не поднимает второй счёт, а ждёт первый. И проверяем на
   выходе, ТА ЛИ ещё песня: за полсекунды человек успевает открыть
   другую, и опоздавший ответ не имеет права переписать её темп. */
function посчитатьТемп(силой) {
  ...
  if (!силой && editor.темпАвто && editor.темпИсточник === источник) {
    return Promise.resolve(editor.темпАвто);
  }
  if (!силой && editor.темпЖдём && editor.темпЖдём.буфер === buf) {
    return editor.темпЖдём.обещание;
  }
  const обещание = темпВПотоке(buf).then((итог) => {
    // Песню сменили, пока считали, — ответ уже не про неё
    if ((state.instrumentalBuffer || state.originalBuffer) !== buf) return null;
    if (editor.темпЖдём && editor.темпЖдём.буфер === buf) editor.темпЖдём = null;
    editor.темпАвто = итог;
    ...
    const переписали = !!(итог && !state.сетка.свой);
    if (переписали) {
      state.сетка.bpm = итог.bpm;
      state.сетка.фаза = итог.фаза;
    }
    // Число в поле поменялось не из поля — показываем действующее силой
    обновитьСетку(переписали);
    // Сетка нарисована по прежнему темпу — перерисовываем по новому
    if (переписали && state.сетка.вкл) drawTimeline();
    return итог;
  });
  editor.темпЖдём = { буфер: buf, обещание };
  обновитьСетку();   // показатель встаёт на «…»: считаем
  return обещание;
}
...
function renderEditList() {
  ...
  /* Ряды собраны заново, и пометки текущей строки на них нет: её ставит
     просмотр (см. renderEditStage). Сбрасываем его слепок, иначе
...
for (const [край, id] of [['вход', 'orig-in'], ['выход', 'orig-out']]) {
  const п = $(id);
  if (!п) continue;
  п.addEventListener('input', () => {
    const первый = !('тянули' in п.dataset);
    п.dataset.тянули = '1';
    поставитьФейдОтрезка(край, +п.value, первый);
  });
...
/* ============================================================
   ...
   него: пока он включён, список строк, просмотр и панель выбранной
...
$('edit-list').addEventListener('input', (e) => {
  ...
  $('lyrics-input').value = state.lines.map((l) => l.text).join('\n');
  saveProject();
  editor.stageKey = ''; // заставляем просмотр перерисоваться
...
/* --- Мини-сцена просмотра ---
   ...
   холстом и читает раскладку, а следом список строк всплывает к текущей.
   Поэтому у просмотра два ключа, а не один.
   ...
   * `editor.stageDrawn` — слепок того, что НА САМОМ ДЕЛЕ нарисовано:
   ...
   Беда, которую это лечит. Каждое движение мыши при перетаскивании
     ...
     просмотр пересобирался целиком, хотя строка поётся та же самая
     и на экране от этого не менялось ни пикселя. Замерено: за три
     секунды перетаскивания 360 перерисовок, из них 357 — в пустоту,
     и с каждой из них подгонка кегля и прокрутка списка строк. Прокрутка
...
function renderEditStage() {
  ...
  /* Просмотр рисуется ТЕМ ЖЕ кодом, что и сцена караоке: те же
...
/* Высоты полос. Числами задана не высота, а ДОЛЯ: сколько места
   полоса получает из того, что досталось дорожке. Раньше это были
   именно пиксели (16/18/28/38/30/20) с коэффициентом 0,8 на низком
   экране — и дорожка на любом мониторе оставалась одного роста, хотя
   на 27 дюймах под ней пустовало полэкрана. Теперь высоту дорожки
   целиком решает разделитель (см. «Деление высоты» ниже), а эти веса
   делят её между полосами; прежние числа взяты весами, поэтому на
...
const ПОРЯДОК_ПОЛОС = ['orig', 'voice', 'wave', 'lines', 'words'];

/* Сколько пикселей досталось полосам. Ставит resizeTimeline по живой
   высоте канваса; timelineLanes() только делит это число, потому что
   её зовут и на каждый кадр отрисовки, и на каждую проверку попадания —
   мерить там что-то в документе было бы не по карману.
   Начальное значение — прежняя высота дорожки: до первого resizeTimeline
   раскладка обязана быть осмысленной. */
let высотаПолос = 122;
...
function timelineLanes() {
  const есть = ПОРЯДОК_ПОЛОС.filter((к) => к !== 'voice' || voiceReady());
  const вес = есть.reduce((с, к) => с + ВЕС_ПОЛОС[к], 0);
  /* Ниже этого делить нечего: на 60 px пяти полосам не разойтись, и
     дорожка честнее вылезет за край, чем схлопнется в кашу. */
  const остаток = Math.max(60, высотаПолос - LANE_RULER);
  const L = {};
  let y = 0;
  L.ruler = { y, h: LANE_RULER }; y += LANE_RULER;
  let отдано = 0;
  есть.forEach((к, i) => {
    /* Последней полосе — весь остаток, а не её доля: округление
       по дороге теряет пиксель-другой, и внизу дорожки оставалась бы
       щель, а колонка заголовков разъезжалась бы с канвасом. */
    const h = i === есть.length - 1
      ? остаток - отдано
      : Math.round(остаток * ВЕС_ПОЛОС[к] / вес);
    отдано += h;
    L[к] = { y, h };
    y += h;
  });
...
/* ============================================================
   Деление высоты: окна сверху, дорожка снизу

   Беда, которую это лечит: дорожка была мелкой. Полосы стояли
   жёсткими пикселями, всё остальное место забирали три верхних окна —
   и на любом мониторе дорожка выходила одного роста, хотя монтаж
   идёт именно по ней.

   Решено так же, как в Logic Pro и Final Cut: между окнами и дорожкой
   стоит разделитель, и сколько кому высоты — решает человек. Хранится
   не число пикселей, а ДОЛЯ рабочего места: на другом мониторе доля
   даёт осмысленную высоту, а пиксели с ноутбука на большом экране
   оставили бы дорожку прежней крохой.

   Пределы держим сами, а не только в CSS: окнам всегда остаётся
   МИН_ОКОН, дорожке — не меньше прежней своей высоты и не больше
   МАКС_ДОРОЖКИ (расплывшаяся по вертикали волна читается не лучше
   мелкой). Когда справка редактора развёрнута, окнам разрешено сжаться
   сильнее: иначе на 1280×800 сумма минимумов не влезала в подложку.
   ============================================================ */
const КЛЮЧ_ДЕЛЕНИЯ = 'karaoke-split';
const ДОЛЯ_ДОРОЖКИ = 0.46;   // столько рабочего места дорожке по умолчанию
const МИН_ДОРОЖКИ = 140;     // ровно столько занимала вся дорожка до правки
const МАКС_ДОРОЖКИ = 460;
const МИН_ОКОН = 176;        // три верхних окна: заголовок и пять строк

function прочитатьДолю() {
  try {
    const v = parseFloat(localStorage.getItem(КЛЮЧ_ДЕЛЕНИЯ));
    if (v >= 0.15 && v <= 0.85) return v;
  } catch (e) { /* хранилище недоступно — остаёмся на доле по умолчанию */ }
  return ДОЛЯ_ДОРОЖКИ;
}

let доляДорожки = прочитатьДолю();

/* Сколько высоты делится между окнами и дорожкой. Это ровно их
   нынешняя сумма: окна стоят «весь остаток» (flex: 1 1 0), дорожка —
   своим числом, значит сумма не зависит от того, как их поделили,
   и мерить её можно в любой момент. Складывать её из высоты шага
   за вычетом соседей было бы точнее на бумаге и хуже на деле: поля
   и margin'ы соседей в offsetHeight не входят, и оценка расходилась
   с настоящей на десяток пикселей.

   Единственное, чего этот способ не видит, — окна, упёршиеся в свой
   нижний предел из CSS. Поэтому предел в CSS нарочно ниже нашего
   (80 против МИН_ОКОН): пока делит код, до него не доходит. */
function рабочееМесто() {
  const шаг = $('step-3');
  const сетка = шаг && шаг.querySelector(':scope > .editor-grid');
  const дорожка = шаг && шаг.querySelector(':scope > .timeline-wrap');
  if (!шаг || !сетка || !дорожка) return null;
  // Спрятанный шаг и заход простукивания (окна убраны) мерить нечего
  if (!шаг.classList.contains('active') || сетка.offsetHeight === 0) return null;
  // На узком экране студия перестаёт быть высотой в окно — делить нечего
  if (getComputedStyle(шаг).display !== 'flex') return null;
  const пул = сетка.offsetHeight + дорожка.offsetHeight;
  return пул > 80 ? { шаг, сетка, дорожка, пул } : null;
}

function применитьДеление() {
  const м = рабочееМесто();
  if (!м) return;
  let h = Math.round(м.пул * доляДорожки);
  h = Math.min(Math.max(h, МИН_ДОРОЖКИ), МАКС_ДОРОЖКИ);
  // Окнам всегда остаётся своё; на совсем низком окне дорожка уступает
  h = Math.min(h, Math.max(60, м.пул - МИН_ОКОН));
  м.шаг.style.setProperty('--tl-h', h + 'px');
}

/* Разделитель. Тянут мышью — дорожка растёт вверх, окна ужимаются;
   стрелки с клавиатуры двигают на 12 px, двойной щелчок возвращает
   долю по умолчанию. Доля пишется в хранилище на отпускании кнопки,
   а не на каждом движении: за одно перетаскивание их сотни. */
function завестиРазделитель() {
  const узел = $('ed-splitter');
  if (!узел) return;

  const сохранить = () => {
    try { localStorage.setItem(КЛЮЧ_ДЕЛЕНИЯ, доляДорожки.toFixed(3)); }
    catch (e) { /* некуда писать — доля живёт до конца сеанса */ }
  };
  const поставить = (h, пул) => {
    доляДорожки = Math.min(0.85, Math.max(0.15, h / пул));
    resizeTimeline();
    drawTimeline();
  };

  let тянем = 0;      // pointerId, 0 — не тянем
  let началоY = 0;
  let началоH = 0;
  let пул = 0;

  узел.addEventListener('pointerdown', (e) => {
    const м = рабочееМесто();
    if (!м) return;
    тянем = e.pointerId;
    началоY = e.clientY;
    началоH = м.дорожка.offsetHeight;
    пул = м.пул;
    узел.classList.add('тянут');
    узел.setPointerCapture(e.pointerId);
    e.preventDefault();
  });
  узел.addEventListener('pointermove', (e) => {
    if (тянем !== e.pointerId) return;
    // Разделитель над дорожкой: тянут вверх — дорожка выше
    поставить(началоH + (началоY - e.clientY), пул);
  });
  const отпустить = (e) => {
    if (тянем !== e.pointerId) return;
    тянем = 0;
    узел.classList.remove('тянут');
    сохранить();
  };
  узел.addEventListener('pointerup', отпустить);
  узел.addEventListener('pointercancel', отпустить);

  узел.addEventListener('keydown', (e) => {
    const шаг = e.key === 'ArrowUp' ? 12 : e.key === 'ArrowDown' ? -12 : 0;
    if (!шаг) return;
    const м = рабочееМесто();
    if (!м) return;
    e.preventDefault();
    поставить(м.дорожка.offsetHeight + шаг, м.пул);
    сохранить();
  });
  узел.addEventListener('dblclick', () => {
...
/* ============================================================
   Ширина инспектора: просмотр слева, инспектор справа

   Инспектор стоял жёсткими 210 пикселями. Беда была видна на снимке:
   он занимает всю правую колонку, а показывает почти пустоту — слово
   не выбрано; при этом нижние поля обрезаны, потому что дорожка под
   ним выросла. Одному нужно шире (в поля набирают время до тысячных),
   другому уже (в просмотре кегль считается от ширины окна) — значит,
   решать должен человек, а не наше число.

   Устроено так же, как деление высоты выше: разделитель тянут мышью,
   стрелки ← и → двигают его с клавиатуры, двойной щелчок возвращает
   умолчание. Хранится ДОЛЯ рабочей ширины, а не пиксели: снятые
   с ноутбука пиксели на большом мониторе оставили бы инспектор
   прежней узкой полосой.
   ============================================================ */
const КЛЮЧ_ИНСПЕКТОРА = 'karaoke-insp';
/* Панель занимает правый край во всю высоту, и раздела в ней три:
   слово, строка, отрезок. Прежние 0.17 (210 px) считались для панели
   в один ряд с окнами — теперь ей есть куда развернуться, и кнопки
   на такой ширине встают в два столбца вместо шести рядов. */
const ДОЛЯ_ИНСПЕКТОРА = 0.21;
const МИН_ИНСПЕКТОРА = 168;     // уже — и время «1:27,440» не влезает в поле
const МАКС_ИНСПЕКТОРА = 520;
const МИН_ОКОН_РЯДОМ = 380;     // список строк и просмотр вдвоём

function прочитатьДолюИнспектора() {
  try {
    const v = parseFloat(localStorage.getItem(КЛЮЧ_ИНСПЕКТОРА));
    if (v >= 0.08 && v <= 0.5) return v;
  } catch (e) { /* хранилище недоступно — остаёмся на доле по умолчанию */ }
  return ДОЛЯ_ИНСПЕКТОРА;
}

let доляИнспектора = прочитатьДолюИнспектора();

/* Сколько ширины делится между окнами. Это ширина самой сетки: три
   окна и разделитель стоят в ней колонками, и сумма их ширин от того,
   как их поделили, не зависит.

   На узком экране (max-width: 900px) колонка в сетке одна — окна идут
   друг за другом своей высотой, и делить нечего. Узнаём это по самой
   раскладке, а не по ширине окна: правило живёт в CSS, и повторять
   его число здесь значило бы завести второй источник правды. */
function рабочаяШирина() {
  const шаг = $('step-3');
  const сетка = шаг && шаг.querySelector(':scope > .editor-grid');
  if (!шаг || !сетка) return null;
  if (!шаг.classList.contains('active') || сетка.offsetHeight === 0) return null;
  const колонок = getComputedStyle(сетка).gridTemplateColumns.split(/\s+/).filter(Boolean).length;
  if (колонок < 3) return null;
  const w = сетка.clientWidth;
  return w > 320 ? { сетка, w } : null;
}

function применитьШиринуИнспектора() {
  const м = рабочаяШирина();
  if (!м) return;
  let w = Math.round(м.w * доляИнспектора);
  w = Math.min(Math.max(w, МИН_ИНСПЕКТОРА), МАКС_ИНСПЕКТОРА);
  // Списку строк и просмотру всегда остаётся своё
  w = Math.min(w, Math.max(120, м.w - МИН_ОКОН_РЯДОМ));
  м.сетка.style.setProperty('--insp-w', w + 'px');
}

/* Разделитель просмотра и инспектора. Тянут влево — инспектор шире,
   просмотр ужимается; стрелки с клавиатуры двигают на 16 px, двойной
   щелчок возвращает долю по умолчанию. Доля пишется в хранилище
   на отпускании кнопки, а не на каждом движении: за одно
   перетаскивание их сотни. */
function завестиРазделительИнспектора() {
  const узел = $('ed-vsplitter');
  if (!узел) return;

  const сохранить = () => {
    try { localStorage.setItem(КЛЮЧ_ИНСПЕКТОРА, доляИнспектора.toFixed(3)); }
    catch (e) { /* некуда писать — доля живёт до конца сеанса */ }
  };
  const поставить = (w, полоса) => {
    доляИнспектора = Math.min(0.5, Math.max(0.08, w / полоса));
    применитьШиринуИнспектора();
    /* И ширину списка следом. Ширину делят трое, а список стоит
       числом пикселей: расширив панель, мы забирали остаток у одного
       просмотра, и он схлопывался до полусотни точек — при своём
       пределе в 260. Пересчёт списка возвращает деление на троих. */
    применитьШиринуСписка();
    /* Кегль в просмотре считается от ширины его окна (fitStageLines),
       а она только что изменилась. Без этого строки остались бы
       прежнего размера до следующего изменения окна. */
  ...
  узел.addEventListener('pointerdown', (e) => {
    const м = рабочаяШирина();
    const панель = $('sel-panel');
    if (!м || !панель) return;
    тянем = e.pointerId;
    началоX = e.clientX;
    началоW = панель.offsetWidth;
    полоса = м.w;
    узел.classList.add('тянут');
    узел.setPointerCapture(e.pointerId);
    e.preventDefault();
  ...
  узел.addEventListener('pointermove', (e) => {
    if (тянем !== e.pointerId) return;
    // Разделитель слева от инспектора: тянут влево — инспектор шире
    поставить(началоW + (началоX - e.clientX), полоса);
  });
  const отпустить = (e) => {
    if (тянем !== e.pointerId) return;
    тянем = 0;
  ...
  узел.addEventListener('keydown', (e) => {
    const шаг = e.key === 'ArrowLeft' ? 16 : e.key === 'ArrowRight' ? -16 : 0;
    if (!шаг) return;
    const м = рабочаяШирина();
    const панель = $('sel-panel');
    if (!м || !панель) return;
    e.preventDefault();
    поставить(панель.offsetWidth + шаг, м.w);
    сохранить();
  });
  узел.addEventListener('dblclick', () => {
...
/* ============================================================
   Ширина списка строк: список слева, просмотр справа

   Растянуть можно было любое окно, кроме списка строк: он стоял
   в сетке долей 1.1 к 1 и на широком экране забирал больше половины
   редактора — под текст, которому столько ширины не нужно, а просмотру
   и инспектору её не хватало. Теперь между списком и просмотром стоит
   такой же разделитель, как между просмотром и инспектором.

   Хранится ДОЛЯ ширины окон, а не пиксели: на другом мониторе доля
   даёт осмысленную ширину, а снятые с ноутбука пиксели оставили бы
   список прежней узкой полосой.
   ============================================================ */
const КЛЮЧ_СПИСКА = 'karaoke-list-w';
const ДОЛЯ_СПИСКА = 0.52;    // 1.1 к 1 — столько список занимал раньше
const МИН_СПИСКА = 200;      // уже — и номер со строкой не разглядеть
const МИН_ПРОСМОТРА = 260;   // просмотру всегда остаётся своё

function прочитатьДолюСписка() {
  try {
    const v = parseFloat(localStorage.getItem(КЛЮЧ_СПИСКА));
    if (v >= 0.15 && v <= 0.8) return v;
  } catch (e) { /* хранилище недоступно — остаёмся на доле по умолчанию */ }
  return ДОЛЯ_СПИСКА;
}

let доляСписка = прочитатьДолюСписка();

/* Ширина, которую делят список и просмотр: это ширина их сетки.
   Разделитель стоит в ней колонкой, поэтому сумма от деления
   не зависит. На узком экране (max-width: 900px) колонка одна —
   окна идут друг за другом, и делить нечего. */
function ширинаОкон() {
  const шаг = $('step-3');
  const сетка = шаг && шаг.querySelector(':scope > .editor-grid');
  if (!шаг || !сетка) return null;
...
function применитьШиринуСписка() {
  const м = ширинаОкон();
  if (!м) return;
  /* Ширину делят трое: список, просмотр и инспектор. Инспектор свою
     уже занял (применитьШиринуИнспектора зовут первой), поэтому
     считаем от остатка — иначе просмотр между ними схлопнулся бы
     в ничто.

     Съеденное считаем по живой раскладке, а не по числам из головы:
     кроме самой панели, ширину едят два разделителя и ЧЕТЫРЕ зазора
     сетки. Забыли зазоры — и просмотр оказывался уже своего предела
     на те самые сорок точек. */
  const панель = $('sel-panel');
  const зазор = parseFloat(getComputedStyle(м.сетка).columnGap) || 0;
  const разделители = ['ed-lsplitter', 'ed-vsplitter']
    .reduce((с, id) => с + ($(id) ? $(id).offsetWidth : 0), 0);
  const занято = (панель ? панель.offsetWidth : 0) + разделители + зазор * 4;
  const свободно = Math.max(МИН_СПИСКА + МИН_ПРОСМОТРА, м.w - занято);
  let w = Math.round(м.w * доляСписка);
  w = Math.max(w, МИН_СПИСКА);
  w = Math.min(w, Math.max(МИН_СПИСКА, свободно - МИН_ПРОСМОТРА));
  м.сетка.style.setProperty('--list-w', w + 'px');
}

/* Разделитель списка и просмотра. Тянут вправо — список шире, просмотр
   ужимается; стрелки двигают на 16 px, двойной щелчок возвращает долю
   по умолчанию. Доля пишется на отпускании кнопки, а не на каждом
   движении: за одно перетаскивание их сотни. */
function завестиРазделительСписка() {
  const узел = $('ed-lsplitter');
  if (!узел) return;

  const сохранить = () => {
    try { localStorage.setItem(КЛЮЧ_СПИСКА, доляСписка.toFixed(3)); }
    catch (e) { /* некуда писать — доля живёт до конца сеанса */ }
  };
  const поставить = (w, полоса) => {
    доляСписка = Math.min(0.8, Math.max(0.15, w / полоса));
    применитьШиринуСписка();
    /* Кегль в просмотре считается от ширины его окна (fitStageLines),
       а она только что изменилась. */
    fitStageLines($('edit-stage'));
  };
  ...
  узел.addEventListener('pointermove', (e) => {
    if (тянем !== e.pointerId) return;
    // Разделитель справа от списка: тянут вправо — список шире
    поставить(началоW + (e.clientX - началоX), полоса);
  });
  const отпустить = (e) => {
    if (тянем !== e.pointerId) return;
    тянем = 0;
  ...
  узел.addEventListener('keydown', (e) => {
    const шаг = e.key === 'ArrowRight' ? 16 : e.key === 'ArrowLeft' ? -16 : 0;
    if (!шаг) return;
    const м = ширинаОкон();
    const список = document.querySelector('#step-3 .ed-lines');
    if (!м || !список) return;
    e.preventDefault();
    поставить(список.offsetWidth + шаг, м.w);
    сохранить();
  });
  узел.addEventListener('dblclick', () => {
...
function resizeTimeline() {
  /* Сначала делим высоту между окнами и дорожкой, потом меряем, сколько
     после этого досталось полосам: порядок обратный дал бы раскладку
     от прежнего размера — на кадр позже, чем нужно. */
  применитьДеление();
  // Заодно и ширины окон: их делят те же события — открытие редактора
  // и изменение размеров окна
  применитьШиринуИнспектора();
  применитьШиринуСписка();
  const c = $('timeline');
  const обёртка = c.parentElement;   // .tl-canvas-wrap
  if (обёртка) {
    const cs = getComputedStyle(обёртка);
    const свободно = обёртка.clientHeight
      - (parseFloat(cs.paddingTop) || 0) - (parseFloat(cs.paddingBottom) || 0);
    // Спрятанный шаг мерить бессмысленно: там ноль, а не высота дорожки
    if (свободно > 40) высотаПолос = Math.round(свободно);
  }
...
function drawOrigLane(g, lane, W) {
  ...
  spans.forEach((s, i) => {
    ...
    g.stroke();
    /* Скосы появления и затухания — как у клипа в монтажной: видно
       с одного взгляда, что кусок входит не рывком. Пока фейд самый
       короткий (те же 50 мс, что были всегда), скос уже пары пикселей
       и в глаза не лезет. */
    const вх = зажатьФейд(s.вход, s.end - s.start);
    const вых = зажатьФейд(s.выход, s.end - s.start);
    const wвх = Math.max(0, tToX(s.start + вх) - x0);
    const wвых = Math.max(0, x1 - tToX(s.end - вых));
    if (wвх > 1.5 || wвых > 1.5) {
      g.save();
      roundRect(g, x0, y, w, h, 4);
      g.clip();
      g.fillStyle = 'rgba(12, 8, 12, 0.45)';
      if (wвх > 1.5) {
        g.beginPath();
        g.moveTo(x0, y); g.lineTo(x0 + wвх, y); g.lineTo(x0, y + h);
        g.closePath(); g.fill();
      }
      if (wвых > 1.5) {
        g.beginPath();
        g.moveTo(x1, y); g.lineTo(x1 - wвых, y); g.lineTo(x1, y + h);
        g.closePath(); g.fill();
      }
      g.strokeStyle = 'rgba(253, 164, 175, 0.8)';
      g.lineWidth = 1;
      g.beginPath();
      if (wвх > 1.5) { g.moveTo(x0, y + h); g.lineTo(x0 + wвх, y); }
      if (wвых > 1.5) { g.moveTo(x1 - wвых, y); g.lineTo(x1, y + h); }
      g.stroke();
      g.restore();
    }
...
function drawLineBlocks(g, lane, W) {
  ...
  for (const sp of editorSpans()) {
    ...
    /* Партия дуэта — полоской по верху блока. Не заливкой: заливка уже
       ...
       вместо них. */
    const цветП = цветПартии(партияСтроки(sp.line));
    if (цветП) {
      g.fillStyle = цветП;
  ...
  if (y >= L.lines.y && y < L.lines.y + L.lines.h) {
    ...
    for (let i = 0; i < spans.length - 1; i++) {
      ...
      /* Сваренными считаем и те строки, между которыми стоит один
         рисовальный зазор: блоки на дорожке нарочно не касаются
...
function updateWordInfo() {
  ...
  /* Заголовок окна за выбором больше НЕ бегает. Раньше он повторял
     то же слово, что и открытый раздел под ним: окно «СТРОКА»,
     а в нём раздел «СТРОКА». Одного раза довольно — окно называется
     «Параметры», а что именно выбрано, говорит открытый раздел. */
...
/* Появление и затухание отдельного отрезка. Край — 'вход' или 'выход';
   зажимаем здесь же, потому что ползунок мог остаться от прошлого,
   более длинного отрезка. */
function поставитьФейдОтрезка(край, v, сИсторией) {
  const s = выбранныйОтрезок();
  if (!s) return;
  const было = зажатьФейд(s[край], s.end - s.start);
  const стало = зажатьФейд(v, s.end - s.start);
  if (Math.abs(было - стало) < 0.001) return;
  if (сИсторией) pushHistory();
  s[край] = стало;
  saveProject();
...
function обновитьОтрезок() {
  ...
  писать(поле, s ? t('ред.отрезокОт', { 'от': fmtTimeCs(s.start), 'до': fmtTimeCs(s.end) })
    : t('ред.отрезокНет'));
  ...
  /* Ползунок фейда ходит до половины отрезка: у секундного куска
     пятисекундное появление — не настройка, а недоразумение, поэтому
     предел ставим по самому отрезку, а не общий на всех. */
  for (const [край, id] of [['вход', 'orig-in'], ['выход', 'orig-out']]) {
    const п = $(id);
    const ч = $(id + '-val');
    const предел = s ? Math.max(ОТРЕЗОК_ФЕЙД, Math.min(ФЕЙД_МАКС, (s.end - s.start) / 2)) : ФЕЙД_МАКС;
    const знач = s ? зажатьФейд(s[край], s.end - s.start) : ОТРЕЗОК_ФЕЙД;
    if (п) {
      if (п.disabled !== !s) п.disabled = !s;
      const макс = предел.toFixed(2);
      if (п.max !== макс) п.max = макс;
      if (document.activeElement !== п) ставитьЗначение(п, String(знач));
    }
    if (ч) {
      писать(ч, знач.toFixed(2));
      класс(ч, 'off', !s);
    }
  }
  ...
  /* Отрезков нет вовсе — раздела в панели тоже нет: показывать нечего,
     а место он занимал всегда. Как только отрезок сделан, раздел
     возвращается, даже если он не выбран: выбирают его щелчком
     по дорожке, и панель должна быть на месте. */
  класс($('orig-panel'), 'нет-отрезков', !отрезкиОригинала().length);
  /* Пока отрезка нет — объясняем, что это и как его завести. Выбрали —
     подсказка уходит: место нужно тому, ради чего панель и открыта. */
  класс($('orig-how'), 'hidden', !!s);
}
...
const ИНСПЕКТОР = (() => {
  ...
  const РАЗДЕЛЫ = ['insp-word', 'insp-line', 'insp-orig'];
  /* Какой раздел отвечает за что: выбрали слово — открывается «Слово»,
     строку — «Строка», отрезок оригинала — «Оригинал». */
  const ПО_ВЫБОРУ = { слово: 'insp-word', строка: 'insp-line', отрезок: 'insp-orig' };
  ...
  function применить() {
    ...
    РАЗДЕЛЫ.forEach((id) => {
      const el = $(id);
      // Умолчание — свёрнуто: разделов три, и все разом в панель не влезают
  ...
  function подтянуть() {
    ...
    РАЗДЕЛЫ.forEach((р) => {
      ...
      if (!el) return;
      const надо = р === id;
      if (el.open !== надо) el.open = надо;   // toggle сам запишет это в память
    });
  }
...
function обновитьСетку(силой) {
  ...
  if (знак) {
    ...
    if (state.сетка.свой && есть) {
      знак.textContent = t('ред.доли.руками');
      знак.dataset.уровень = 'руками';
      знак.title = t('ред.доли.руками.подсказка');
    } else if (!есть && editor.темпЖдём) {
      /* Счёт идёт в рабочем потоке. Прочерк тут врал бы: он значит
         «темпа нет», а темп будет через полсекунды */
      знак.textContent = '…';
      знак.dataset.уровень = 'считаем';
      знак.title = t('ред.доли.считаем');
    } else if (!есть) {
      знак.textContent = '—';
...
function сброситьСеткуКАвтомату() {
  const а = editor.темпАвто;
  /* Темпа ещё нет — поднимаем счёт и уходим: он идёт в рабочем потоке,
     а как ответит, сам поставит найденное (свой при этом уже снят). */
  if (!а) {
...
завестиРазделитель();
завестиРазделительИнспектора();
завестиРазделительСписка();
...
/* ---------- Знак студии в углу кадра ----------
   ...
   кто уже знает, а название нигде не написано. Рядом с логотипом
   стоит имя студии — тем же фирменным Bungee Shade, что и в заставке.

   СОБРАН ПО-МОНТАЖНОМУ, а не как придётся. Было: имя отдельной
   строкой НАД логотипом, разной с ним прозрачности, у самого края
   кадра. Так знаки не ставят — ни на телеканале, ни в редакторе
   видео. Принято иначе, и теперь так же у нас:

     • знак — одна связка в строку: слева картинка, справа имя,
       по общей средней линии. Имя над картинкой читается как две
       разные надписи, а не как один знак;
     • угол правый нижний — самый частый для знака автора; текст
       песни по умолчанию стоит посередине кадра и с ним не спорит;
     • отступ 5 % высоты кадра — «безопасное поле» вещания: ближе
       к краю знак срежет любая обрезка под другое соотношение;
     • величина скромная: картинка в 7 % высоты кадра, буквы вдвое
       ниже её. Прежние 9 % при имени сверху занимали угол целиком;
     • прозрачность одна на всю связку: половинки одного знака
       не бывают разной плотности.

   Кегль и отступы считаются от высоты кадра, чтобы знак одинаково
   смотрелся и в HD, и в 2K. */
const ЗНАК_ТЕКСТ = 'Karaoke Punch';
const ЗНАК_ШРИФТ = '"Bungee Shade", Impact, sans-serif';
/* Кегль имени — от высоты картинки знака, а не сам по себе: в связке
   они держатся вместе и вместе же меняются с размером кадра. */
const ЗНАК_СТОРОНА = 0.065;  // сторона картинки, доля высоты кадра
const ЗНАК_ПОЛЕ = 0.05;      // отступ от края — безопасное поле вещания
/* Имя ниже картинки почти вдвое. Bungee Shade — широкое начертание,
   и «Karaoke Punch» в полный рост занимал бы четверть ширины кадра:
   знак автора столько места не берёт. */
const знакКегль = (H) => Math.max(10, Math.round(H * ЗНАК_СТОРОНА * 0.45));
...
function рисоватьЗнак(g2d, W, H, watermark) {
  const size = Math.round(H * ЗНАК_СТОРОНА);   // сторона картинки
  const margin = Math.round(H * ЗНАК_ПОЛЕ);
  const кегль = знакКегль(H);
  const зазор = Math.round(size * 0.25);       // просвет между картинкой и именем
  g2d.save();
  g2d.globalAlpha = 0.7;                       // одна на всю связку
  g2d.font = `400 ${кегль}px ${ЗНАК_ШРИФТ}`;
  g2d.textAlign = 'left';
  /* Середина строки — общая ось связки: имя стоит не под картинкой
     и не над ней, а точно против её середины. */
  g2d.textBaseline = 'middle';
  const ширинаИмени = g2d.measureText(ЗНАК_ТЕКСТ).width;
  const правый = W - margin;
  const низ = H - margin;                      // нижняя граница связки
  const ось = низ - size / 2;
  const ширина = (watermark ? size + зазор : 0) + ширинаИмени;
  const левый = правый - ширина;
  if (watermark) g2d.drawImage(watermark, левый, низ - size, size, size);
  const xИмени = левый + (watermark ? size + зазор : 0);
  ...
  g2d.lineJoin = 'round';
  g2d.strokeText(ЗНАК_ТЕКСТ, xИмени, ось);
  g2d.fillStyle = '#ffffff';
  g2d.fillText(ЗНАК_ТЕКСТ, xИмени, ось);
  ...
  рисоватьЗнак.последний = {
    ...
    низ,
    связка: { левый, правый, верх: низ - size, ось, сторона: size, поле: margin },
  };
}
...
/* ---------- Финал в кадре видео ----------
   ...
   То же самое, что на экране после песни, но нарисованное на холсте
   и дописанное в хвост записи. Видео потому и записывают, что его
   потом смотрят: концерт, который обрывается на последней ноте,
   выглядит недоделанным, а «браво» на конце — это то, чем караоке
   и заканчивается.

   Фразу и число выбираем ОДИН РАЗ на всю запись (см. собратьВидео):
   иначе они менялись бы каждый кадр, и вместо похвалы вышла бы
   мельтешащая бессмыслица.

   Фон — тот же, что у песни, но притемнённый: текста тут мало,
   и картинка не должна с ним спорить. */
/* Тот же моноширинный, каким числа набраны в студии: пока цифра
   бежит, строка не должна дёргаться от разной ширины цифр. Холст
   переменных CSS не читает, поэтому список шрифтов здесь свой. */
const ЧИСЛА_ШРИФТ = 'ui-monospace, SFMono-Regular, Menlo, Consolas, monospace';
/* Финал идёт тремя ударами, а не одним. Сначала во весь экран
   вылетает надпись «Karaoke Punch», следом — логотип, и только потом
   поверх него встают похвала и счёт.

   Появления РЕЗКИЕ: не растворение, а удар. Каждый кадр появляется
   сразу и целиком, но с коротким перелётом по размеру — влетает
   чуть крупнее и за полторы десятых доезжает до своего. Растворение
   тут читалось бы как «плавно кончается», а нужно ровно обратное:
   концерт не затухает, он ставит точку. */
const ФАЗА_ЗНАК = 0.85;         // докуда во весь экран стоит надпись
const ФАЗА_ЛОГО = 1.7;          // докуда — логотип
const УДАР = 0.16;              // за столько секунд перелёт доезжает
const ФИНАЛ_В_КАДРЕ = 9;        // сколько секунд финала дописываем к записи

/* Похвала и счёт для хвоста ролика. Выбираются ОДИН РАЗ на всю запись:
   меняйся они каждый кадр — вместо похвалы вышло бы мельтешение. Язык —
   по песне, как и на экране. Выходит похвала через пару секунд после
   ПОСЛЕДНЕГО СЛОВА, а не после песни: у песни бывает проигрыш
   на полминуты, и всё это время в кадре стояла бы допетая строка.

   Общая на оба пути записи — и на сборку, и на запись в реальном
   времени: ролик не должен зависеть от того, каким путём он вышел. */
function финалДляРолика() {
  const язык = языкПесни();
  return {
    язык,
    фраза: случайнаяПохвала(язык),
    подпись: t('финал.подпись', null, язык),
    цель: Math.floor(ФИНАЛ_СЧЁТ_ОТ + Math.random() * (ФИНАЛ_СЧЁТ_ДО - ФИНАЛ_СЧЁТ_ОТ)),
    финалС: времяФинала() != null ? времяФинала() : (audio.duration || 0),
  };
}

/* ---------- Сколько будет весить ролик ----------

   Беда, ради которой это сделано: человек на MacBook Air с 8 ГБ выбрал 2K,
   ушёл записывать длинную песню — и приложение показало чёрный экран.
   В журнале «страница погибла». Про цену выбора не говорилось нигде:
   в окне стояли только названия «HD», «Full HD», «2K», а что за ними
   полтысячи мегабайт и сколько минут записи — человек узнавал потом.

   Считаем от того же битрейта, которым и пишем (битрейтВидео — одно
   место на обе надобности, иначе оценка разойдётся с записью), плюс
   звук и накладные расходы контейнера. Замерено на живой записи:
   при 18 Мбит/с выходит 2,36 МБ/с — множитель ниже это и учитывает. */
function битрейтВидео(качество) {
  return качество >= 1440 ? 18_000_000 : качество >= 1080 ? 12_000_000 : 7_000_000;
}

/* Длина ролика — ровно та, которую напишет recordVideo: песня (или
   хвост похвалы, если он длиннее) плюс подводка с начальным отсчётом. */
function длинаРолика() {
  const песня = audio.duration || 0;
  const финал = времяФинала() != null ? времяФинала() : песня;
  return подводкаПесни() + Math.max(песня, финал + ФИНАЛ_В_КАДРЕ);
}

/* Весть о записи главному процессу: он держит её у себя и вписывает
   в журнал, если страница погибнет. На сайте моста нет — там и молчим. */
function сообщитьОЗаписи(что) {
  try {
    if (window.desktop && window.desktop.exportState) window.desktop.exportState(что);
  } catch (e) { /* мост не отвечает — запись от этого не страдает */ }
}

/* Частота кадров записи. Тридцать — столько же, сколько у обычного
   видео, и столько, сколько успевает нарисовать самая слабая машина
   из тех, на которых мы это гоняли. */
...
/* Это ВЕРХНЯЯ ГРАНИЦА, и в окне так и написано — «не больше N МБ,
   ...
   с двумя короткими строками двадцать четыре секунды дали 7,8 МБ против
   потолка в 22. Занижать тут нельзя: число нужно, чтобы человек понял,
   во что ввязывается, а не чтобы угадать байты. */
function весРолика(качество) {
  const звук = 128_000;                       // дорожка AAC/opus
  const сНакладными = 1.09;                   // контейнер и заголовки
  return длинаРолика() * (битрейтВидео(качество) + звук) / 8 * сНакладными;
}
const ФИНАЛ_КАДР_БЕГ = 2.2;     // за столько секунд цифра добегает до итога

/* Перелёт удара: 1.28 в начале, 1 к концу. Обратная кубическая —
   быстро осаживается, и от этого движение читается как удар,
   а не как наплыв. */
function ударНаМиг(прошло, начало) {
  const доля = Math.min(1, Math.max(0, (прошло - начало) / УДАР));
  return 1 + 0.28 * Math.pow(1 - доля, 3);
}

function drawФиналКадра(g2d, W, H, bgImg, прошло, финал, лого) {
  const st = state.style;

  // Фон финала — свой, тёмный: и надпись, и логотип, и цифра стоят
  // на нём в полную силу, а картинка песни тут только мешала бы
  ...
  /* УДАР ПЕРВЫЙ: надпись во весь экран. Тем же фирменным начертанием,
     что и знак в углу кадра, — только теперь она главная. */
  if (прошло < ФАЗА_ЗНАК) {
    const k = ударНаМиг(прошло, 0);
    g2d.save();
    g2d.translate(W / 2, H / 2);
    g2d.scale(k, k);
    g2d.textAlign = 'center';
    g2d.textBaseline = 'middle';
    // Кегль подбираем под ширину: надпись обязана занять весь экран
    let кегль = Math.round(H * 0.3);
    g2d.font = `400 ${кегль}px ${ЗНАК_ШРИФТ}`;
    const нужно = W * 0.84;
    const есть = g2d.measureText(ЗНАК_ТЕКСТ).width;
    if (есть > 0) кегль = Math.round(кегль * (нужно / есть));
    g2d.font = `400 ${кегль}px ${ЗНАК_ШРИФТ}`;
    g2d.fillStyle = '#ffffff';
    g2d.fillText(ЗНАК_ТЕКСТ, 0, 0);
    g2d.restore();
    drawФиналКадра.последний = { фаза: 'знак', кегль, доля: 0 };
    return;
  }

  /* УДАР ВТОРОЙ: логотип во весь экран. Дальше он остаётся стоять
     и служит подложкой для похвалы — «поверх этого», как и задумано.

     Появляется он ударом, а вот УХОДИТ в подложку тихо: размер
     не меняется вовсе, меняется только яркость, и та плавно.
     Раньше на третьем ударе логотип разом и уменьшался (0.74 → 0.62),
     и терял яркость одним кадром, и получал новый удар — со стороны
     это читалось как рывок или подмена картинки. */
  const третья = прошло >= ФАЗА_ЛОГО;
  if (лого && лого.width) {
    const k = ударНаМиг(Math.min(прошло, ФАЗА_ЛОГО - 0.001), ФАЗА_ЗНАК);
    const сторона = Math.min(W, H) * 0.74 * k;
    const w = сторона * (лого.width / лого.height);
    const гаснет = Math.min(1, Math.max(0, (прошло - ФАЗА_ЛОГО) / ЛОГО_ГАСНЕТ));
    g2d.save();
    g2d.globalAlpha = 1 - (1 - ЛОГО_ПОДЛОЖКА) * (1 - Math.pow(1 - гаснет, 3));
    g2d.drawImage(лого, (W - w) / 2, (H - сторона) / 2, w, сторона);
    g2d.restore();
  }
  if (!третья) {
    drawФиналКадра.последний = { фаза: 'лого', доля: 0 };
    return;
  }
  ...
  /* Похвала — ВСЕГДА одной строкой: кегль подбираем под ширину кадра,
     а не ломаем фразу переносом. Перенос делал из одной похвалы то две
     строки, то три, и весь столбец под ней прыгал от песни к песне. */
  const k3 = ударНаМиг(прошло, ФАЗА_ЛОГО);
  const базовыйКегль = H * 0.062;
  g2d.font = `700 ${Math.round(базовыйКегль)}px ${family}`;
  const шириной = g2d.measureText(финал.фраза).width;
  const ужать = шириной > макс ? макс / шириной : 1;
  const кегльФразы = Math.max(12, Math.round(базовыйКегль * ужать * k3));
  g2d.font = `700 ${кегльФразы}px ${family}`;
  const ряды = [финал.фраза];

  const шагРяда = кегльФразы * 1.28;
  const кегльЧисла = Math.round(H * 0.155);
  // Подпись крупнее прежнего: она объясняет шутку, и мелкую её
  // просто не читали
  const кегльПодписи = Math.round(H * 0.042);
  const всего = ряды.length * шагРяда + кегльЧисла * 1.15 + кегльПодписи * 2.4;
  let y = H / 2 - всего / 2 + шагРяда / 2;

  g2d.fillStyle = st.active;
  ряды.forEach((строка) => {
    g2d.fillText(строка, W / 2, y);
    y += шагРяда;
  });

  /* Цифра бежит вверх с замедлением — тем же движением, что и на
     экране, чтобы кадр и просмотр не расходились. */
  const доля = Math.min(1, Math.max(0, (прошло - ФАЗА_ЛОГО) / ФИНАЛ_КАДР_БЕГ));
  const плавно = 1 - Math.pow(1 - доля, 3);
  const число = счётЦифрами(финал.цель * плавно) + '%';
  y += кегльЧисла * 0.62;
  g2d.font = `700 ${кегльЧисла}px ${ЧИСЛА_ШРИФТ}`;
  g2d.fillStyle = st.accent;
  g2d.fillText(число, W / 2, y);

  y += кегльЧисла * 0.62 + кегльПодписи * 1.6;
  g2d.font = `400 ${кегльПодписи}px ${family}`;
  g2d.fillStyle = st.inactive;
  g2d.fillText(финал.подпись, W / 2, y);

  // Для самопроверки: что именно нарисовано в последнем кадре финала
  drawФиналКадра.последний = { фаза: 'счёт', ряды, число, подпись: финал.подпись,
    доля, кегльФразы, кегльЧисла };
}
...
function drawVideoFrame(g2d, W, H, bgImg, pos, watermark) {
  ...
  const chunksFor = (text, kind, line) => {
    ...
    if (kind === 'cur') {
      ...
      /* Ноты режем поштучно: при «заливке» они закрашиваются одна
         за другой, как на сцене (см. заливкаНот), — одним куском заливка
         ползла бы сквозь весь ряд с постоянной скоростью. При «подсветке»
         ряд загорается целиком, и на сцене, и здесь: цвет там не зависит
         от доли (см. drawLineAt). */
  ...
  const drawLineAt = (text, cy, kind, line, кегль, партия) => {
    ...
    const свой = цветПартии(партия || 0) || st.active;
    g2d.font = font(fs);
    ...
    rows.forEach((row, r) => {
      ...
      row.forEach((c, i) => {
        ...
        if (kind !== 'cur' || c.p == null) {
          ...
          /* «Подсветка»: цветом загорается ВСЯ строка целиком, как только
             до неё дошла очередь, — а не спетые слова по одному. Так это
             сделано на экране: [data-effect="highlight"] .stage-line.current
             в style.css красит саму строку, и неспетые слова наследуют цвет
             от неё (замерено: при акценте #b84783 все четыре слова строки
             на экране были #b84783). Кадр же красил слово за словом,
             переключая каждое на его половине, — человек выбирал
             «Подсветку», а в файле получал «Заливку», и написал об этом. */
          g2d.fillStyle = st.effect === 'highlight' ? st.accent : свой;
          g2d.fillText(c.text, x, y);
        }
        x += widths[i];
      });
...
/* В каком виде писать ролик.

   MP4 (H.264 + AAC) — первым, и вот почему. WebM не открывает
   QuickTime, его не принимает часть сервисов, и на айфоне такой ролик
   не посмотреть вовсе: человек делает караоке, чтобы им поделиться,
   а получал файл, который открывается у него одного. В приложении
   H.264 есть всегда — Chromium несёт кодек с собой; в браузере он
   есть не у всех (Safari, старые сборки), поэтому WebM остаётся
   запасным: он лучше, чем ничего.

   Строка кодека без уровня (`avc1`) намеренно: с ней Chromium сам
   подбирает профиль под размер кадра, а прибитый baseline 3.0
   не рассчитан на 1080p и выше.
...
/* ============================================================
   ...
   Как было. Студия рисовала кадры на холст, отдавала их MediaRecorder
   и надеялась, что машина успеет: запись шла в реальном времени, кадр
   за кадром, вместе со звуком. Беда в том, что между «мы отдали кадр»
   и «кадру поставили метку времени» лежит чужой конвейер — захват,
   кодек, сборка контейнера, — и задержка в нём нам не видна и нам
   не подвластна.

   Человек прислал ролик и написал: «в экспортированном видео закраска
   отстаёт». Замерили: привязали время файла к песне по самому звуку
   (окно звука из ролика нашли в минусовке проекта — сошлось с точностью
   до 28 мс), а потом сравнили площадь закраски в кадрах файла с тем,
   что рисует студия. Картинка отставала от звука на 270–410 мс.

   И это не наша медлительность: на том же проекте наш цикл отдавал
   30,3 кадра в секунду, а отрисовка кадра стоила 0,2 мс. Тот же экспорт,
   пересобранный здесь, дал 0–80 мс. Разница — машина: где кодек
   не поспевает, там задержка и копится. Поправить её постоянным сдвигом
   нельзя: у каждой машины и у каждого прогона она своя.

   Как стало. Реального времени нет вовсе.

   • Звук сводится разом, в OfflineAudioContext: та же смесь, те же
     отрезки оригинала, тот же эквалайзер — но посчитанные, а не
     сыгранные. Выходит точный звук с точностью до отсчёта.
   • Кадры рисуются по одному и кодируются через VideoEncoder, и метку
     времени КАЖДОМУ кадру ставим мы сами: кадр номер i — это ровно
     i/30 секунды. Ничего не зависит от того, успевает машина или нет:
     не успевает — значит, сборка идёт дольше, но кадр стоит на своём
     месте.
   • Контейнер складываем сами (mp4-muxer / webm-muxer, оба у нас
     в проекте).

   На быстрой машине выходит быстрее реального времени, на медленной —
   медленнее, но верно и там и там. А человеку это ещё и видно: полоса
   хода больше не привязана к длине песни.

   Запасной путь остался. Где нет WebCodecs или нужного кодека (старый
   браузер на сайте), студия по-прежнему пишет в реальном времени —
   лучше ролик со сдвигом, чем никакого. В приложении этот путь
   не используется: Chromium несёт кодеки с собой.
   ============================================================ */

/* Кодек картинки под размер кадра. Уровень H.264 обязан покрывать
   разрешение: до 720p хватает 3.1, до 1080p — 4.0, а на 2K нужен 5.0.
   Прибитый уровень 4.0 «avc1.640028» на 2560×1440 просто не заводится —
   проверено: isConfigSupported отвечает «не поддержан». */
function кодекКартинки(мп4, H) {
  if (!мп4) return 'vp09.00.10.08';
  return H >= 1440 ? 'avc1.640032' : H >= 1080 ? 'avc1.640028' : 'avc1.64001f';
}

function кодекЗвука(мп4) {
  return мп4 ? 'mp4a.40.2' : 'opus';
}

/* Можно ли собрать ролик самим. Спрашиваем не «есть ли WebCodecs»,
   а «возьмётся ли кодек за ИМЕННО ЭТОТ кадр»: на 2K ответ другой,
   чем на 1080p. */
async function сборкаДоступна(mime, W, H) {
  if (typeof VideoEncoder !== 'function' || typeof AudioEncoder !== 'function') return false;
  if (typeof VideoFrame !== 'function' || typeof AudioData !== 'function') return false;
  const мп4 = mime.startsWith('video/mp4');
  if (мп4 && typeof Mp4Muxer === 'undefined') return false;
  if (!мп4 && typeof WebMMuxer === 'undefined') return false;
  try {
    const к = await VideoEncoder.isConfigSupported({
      codec: кодекКартинки(мп4, H), width: W, height: H,
      bitrate: битрейтВидео(H), framerate: КАДРОВ_В_СЕКУНДУ,
    });
    if (!к || !к.supported) return false;
    const частота = state.originalBuffer ? state.originalBuffer.sampleRate : 48000;
    const з = await AudioEncoder.isConfigSupported({
      codec: кодекЗвука(мп4), sampleRate: частота, numberOfChannels: 2, bitrate: ЗВУК_БИТРЕЙТ,
    });
    return !!(з && з.supported);
  } catch (e) { return false; }
}

/* Звук ролика — посчитанный, а не сыгранный. Граф тот же, что в плеере
   (собратьМикс), и расписание отрезков оригинала то же (расписатьОтрезки):
   в ролик обязано попасть ровно то, что человек слышал в караоке.
   Разница одна — контекст считающий, и потому выходит точно. */
async function свестиЗвукДляРолика(всяДлина, подводка) {
  const исход = state.originalBuffer;
  const частота = исход.sampleRate;
  const каналов = Math.min(2, Math.max(1, исход.numberOfChannels));
  const офф = new OfflineAudioContext(каналов, Math.ceil(всяДлина * частота), частота);
  const смесь = собратьМикс(офф);
  смесь.output.connect(офф.destination);
  const сМинусовкой = !!state.instrumentalBuffer;
  const у = усиленияМикса(state.vocalMix, сМинусовкой, false);
  смесь.vocalGain.gain.value = у.вокал;
  смесь.instGain.gain.value = у.минусовка;
  const песня = офф.createBufferSource();
  песня.buffer = исход;
  песня.connect(смесь.vocalGain);
  песня.start(подводка);
  if (сМинусовкой) {
    const минус = офф.createBufferSource();
    минус.buffer = state.instrumentalBuffer;
    минус.connect(смесь.instGain);
    минус.start(подводка);
  }
  /* Подводка — это тишина ПЕРЕД песней, поэтому ноль песни здесь
     приходится на секунду «подводка», и расписание считается от неё. */
  расписатьОтрезки(офф, смесь.vocalGain, смесь.instGain, у, отрезкиОригинала(), подводка);
  return офф.startRendering();
}

const ЗВУК_БИТРЕЙТ = 192_000;
const ОЧЕРЕДЬ_КОДЕКА = 8;      // больше в очередь не кладём — иначе память
const КЛЮЧЕВОЙ_ЧЕРЕЗ = КАДРОВ_В_СЕКУНДУ * 2;   // ключевой кадр раз в две секунды

/* Не даём очереди кодека расти без края: ждём, пока разгребёт.
   Без этого на длинной песне в памяти копились бы тысячи кадров. */
async function подождатьОчередь(кодек) {
  while (кодек.encodeQueueSize > ОЧЕРЕДЬ_КОДЕКА) {
    await new Promise((r) => setTimeout(r, 4));
  }
}

/* ---------- Куски ролика — сразу на диск ----------

   Сборщик контейнера отдаёт готовые куски по ходу дела, и держать их
   все в памяти незачем: именно на этом получался чёрный экран
   (двухсотмегабайтный ролик плюс разжатые песня и минусовка на машине
   с восемью гигабайтами). В приложении кусок сразу уходит в файл.

   Отдаёт их сборщик СИНХРОННО, а запись на диск идёт через мост
   и асинхронно, — поэтому куски складываются в очередь, а сбрасываем
   её мы сами, когда накопится. Каждый кусок несёт своё место в файле:
   в самом конце сборщик возвращается назад и правит длину.

   Копию кусочка делаем нарочно (slice): буфер, отданный сборщиком,
   он вправе переиспользовать под следующий кусок. */
function потокРолика() {
  const очередь = [];
  let ждёт = 0;
  return {
    положить(данные, место) {
      очередь.push({ данные: данные.slice(), место });
      ждёт += данные.byteLength;
    },
    ждёт: () => ждёт,
    async сбросить() {
      while (очередь.length) {
        const к = очередь.shift();
        ждёт -= к.данные.byteLength;
        const ответ = await window.desktop.clipWrite(к.данные, к.место);
        if (!ответ || !ответ.ok) throw new Error((ответ && ответ.error) || 'кусок не записался');
      }
    },
  };
}

const КУСКОВ_ДО_СБРОСА = 8 * 1024 * 1024;   // копим не больше восьми мегабайт

async function собратьРолик(о) {
  const { canvas, g2d, W, H, quality, mime, bgImg, watermark, логоФинала, финал, подводка } = о;
  const мп4 = mime.startsWith('video/mp4');
  const всяДлина = подводка + Math.max(audio.duration, финал.финалС + ФИНАЛ_В_КАДРЕ);
  const кадров = Math.ceil(всяДлина * КАДРОВ_В_СЕКУНДУ);
  ...
  const Сборщик = мп4 ? Mp4Muxer : WebMMuxer;
  const ext = мп4 ? 'mp4' : 'webm';
  const имяФайла = t('экспорт.имяВидео',
    { 'имя': (state.fileName || 'song').replace(/\.[^.]+$/, ''), ext });

  /* В приложении пишем прямо на диск, кусками: место человек выбирает
     ЗАРАНЕЕ, до первого кадра, — не собирать же полчаса, чтобы потом
     услышать «отмена». На сайте диска нет, там по-прежнему собираем
     в памяти и отдаём скачиванием. */
  const наДиск = !!(window.desktop && window.desktop.clipOpen);
  let поток = null;
  if (наДиск) {
    const открыт = await window.desktop.clipOpen(имяФайла);
    if (!открыт || !открыт.ok) {   // передумал — это не беда
      videoExport.active = false;
      сообщитьОЗаписи({ идёт: false, отменено: true });
      $('export-overlay').classList.add('hidden');
      return;
  ...
  const цель = наДиск
    ? new Сборщик.StreamTarget({ onData: (данные, место) => поток.положить(данные, место) })
    : new Сборщик.ArrayBufferTarget();
  const сборщик = new Сборщик.Muxer(мп4 ? {
    target: цель,
    video: { codec: 'avc', width: W, height: H, frameRate: КАДРОВ_В_СЕКУНДУ },
    audio: { codec: 'aac', numberOfChannels: каналов, sampleRate: частота },
    /* Оглавление в начале файла удобно для показа по сети, но требует
       держать всё в памяти до конца. Пишем на диск — значит, оглавление
       уходит в хвост: для файла на своём компьютере это ничем не хуже,
       а памяти не стоит вовсе. */
    fastStart: наДиск ? false : 'in-memory',
  } : {
    target: цель,
    video: { codec: 'V_VP9', width: W, height: H, frameRate: КАДРОВ_В_СЕКУНДУ },
    audio: { codec: 'A_OPUS', numberOfChannels: каналов, sampleRate: частота },
  });

  let беда = null;
  const звукКодек = new AudioEncoder({
    output: (кусок, мета) => сборщик.addAudioChunk(кусок, мета),
    error: (e) => { беда = беда || e; },
  });
  звукКодек.configure({
    codec: кодекЗвука(мп4), sampleRate: частота, numberOfChannels: каналов, bitrate: ЗВУК_БИТРЕЙТ,
  });
  const видеоКодек = new VideoEncoder({
    output: (кусок, мета) => сборщик.addVideoChunk(кусок, мета),
    error: (e) => { беда = беда || e; },
  });
  видеоКодек.configure({
    codec: кодекКартинки(мп4, H), width: W, height: H,
    bitrate: битрейтВидео(quality), framerate: КАДРОВ_В_СЕКУНДУ,
    latencyMode: 'quality',
  });

  const прибрать = (сорвалось) => {
    try { звукКодек.close(); } catch (e) { /* уже закрыт */ }
    try { видеоКодек.close(); } catch (e) { /* уже закрыт */ }
    /* Недописанный файл убираем: огрызок ролика хуже, чем его отсутствие,
       — его легко принять за готовый. */
    if (наДиск) { try { window.desktop.clipClose(true); } catch (e) { /* уже закрыт */ } }
    videoExport.active = false;
    сообщитьОЗаписи({ идёт: false, отменено: videoExport.cancelled || !!сорвалось });
    $('export-overlay').classList.add('hidden');
  };

  try {
    // ---- Звук: он считается быстро, поэтому идёт первым ----
    const ШАГ = 4096;
    const перемеж = new Float32Array(ШАГ * каналов);
    for (let p = 0; p < звук.length; p += ШАГ) {
      if (videoExport.cancelled) { прибрать(); return; }
      const n = Math.min(ШАГ, звук.length - p);
      for (let ch = 0; ch < каналов; ch++) {
        const д = звук.getChannelData(ch);
        for (let i = 0; i < n; i++) перемеж[i * каналов + ch] = д[p + i];
      }
      звукКодек.encode(new AudioData({
        format: 'f32', sampleRate: частота, numberOfFrames: n, numberOfChannels: каналов,
        timestamp: Math.round((p / частота) * 1e6),
        data: перемеж.subarray(0, n * каналов),
      }));
      await подождатьОчередь(звукКодек);
      if (поток && поток.ждёт() > КУСКОВ_ДО_СБРОСА) await поток.сбросить();
    }
    await звукКодек.flush();
    if (поток) await поток.сбросить();
    if (беда) throw беда;

    // ---- Картинка: кадр за кадром, метку ставим сами ----
    $('export-status').textContent = t('экспорт.записываем');
    const началось = performance.now();
    for (let i = 0; i < кадров; i++) {
      if (videoExport.cancelled) { прибрать(); return; }
      /* Вот ради этой строки всё и затевалось: время кадра — не «сколько
         натикало», а ровно его номер, делённый на частоту кадров. */
      const время = i / КАДРОВ_В_СЕКУНДУ - подводка;
      if (время >= финал.финалС) {
        drawФиналКадра(g2d, W, H, bgImg, время - финал.финалС, финал, логоФинала);
      } else {
        drawVideoFrame(g2d, W, H, bgImg, время, watermark);
      }
      const кадр = new VideoFrame(canvas, {
        timestamp: Math.round((i / КАДРОВ_В_СЕКУНДУ) * 1e6),
        duration: Math.round(1e6 / КАДРОВ_В_СЕКУНДУ),
      });
      видеоКодек.encode(кадр, { keyFrame: i % КЛЮЧЕВОЙ_ЧЕРЕЗ === 0 });
      кадр.close();
      await подождатьОчередь(видеоКодек);
      if (поток && поток.ждёт() > КУСКОВ_ДО_СБРОСА) await поток.сбросить();
      if (беда) throw беда;
      if (i % 5 === 0) {
        const доля = (i + 1) / кадров;
        $('export-fill').style.width = `${(доля * 100).toFixed(1)}%`;
        const прошло = (performance.now() - началось) / 1000;
        const осталось = доля > 0.02 ? Math.round(прошло / доля - прошло) : null;
        $('export-status').textContent = осталось == null
          ? t('экспорт.записываемХод', { at: fmtTime(Math.max(0, время)), 'всего': fmtTime(всяДлина) })
          : t('экспорт.сборкаХод', { 'процент': Math.round(доля * 100), 'осталось': fmtTime(осталось) });
        /* Отдаём управление браузеру: без этого окно замирает и человек
           решает, что студия повисла. */
        await new Promise((r) => setTimeout(r, 0));
      }
    }
    await видеоКодек.flush();
    if (беда) throw беда;
    сборщик.finalize();
    if (videoExport.cancelled) { прибрать(); return; }

    if (наДиск) {
      await поток.сбросить();
      const итог = await window.desktop.clipClose(false);
      прибрать();
      /* Файл уже лежит там, где человек его назначил, — сказать об этом
         надо: скачивания, за которым он привык следить, тут не было. */
      if (итог && итог.path) alert(t('экспорт.роликЗаписан', { 'путь': итог.path }));
    } else {
      прибрать();
      download(new Blob([цель.buffer], { type: мп4 ? 'video/mp4' : 'video/webm' }), имяФайла);
    }
  } catch (e) {
    прибрать(true);
    try {
      if (window.desktop && window.desktop.logError) {
        window.desktop.logError('сборка ролика: ' + ((e && e.stack) || e.message || e));
      }
    } catch (_) { /* журнал не должен мешать */ }
    alert(t('экспорт.сборкаНеВышла', { 'беда': (e && e.message) || String(e) }));
  }
}
...
async function exportVideo() {
  ...
  const финал = финалДляРолика();
  const подводка = подводкаПесни();

  /* Главный путь: собрать ролик самим, не в реальном времени. Тогда
     метка времени у каждого кадра наша, и картинка не может отстать
     от звука, сколько бы машина ни думала (см. «Ролик СОБИРАЕТСЯ»). */
  if (await сборкаДоступна(mime, W, H)) {
    сообщитьОЗаписи({
      идёт: true, качество: quality, формат: mime.startsWith('video/mp4') ? 'mp4' : 'webm',
      всегоС: Math.round(подводка + Math.max(audio.duration, финал.финалС + ФИНАЛ_В_КАДРЕ)),
      мб: Math.round(весРолика(quality) / 1048576),
    });
    await собратьРолик({ canvas, g2d, W, H, quality, mime, bgImg, watermark, логоФинала, финал, подводка });
    return;
  }

  // Запасной путь: записать в реальном времени (старый браузер на сайте)
  ...
  const recorder = new MediaRecorder(stream, mime
    ? { mimeType: mime, videoBitsPerSecond: битрейтВидео(quality) }
    : undefined);
  const chunks = [];
  ...
  const cleanup = () => {
    ...
    videoExport.active = false;
    сообщитьОЗаписи({ идёт: false, отменено: videoExport.cancelled });
    $('export-overlay').classList.add('hidden');
  };
  ...
  const финалКадра = финал;
  const финалС = финал.финалС;

  sources.forEach((s) => s.start(t0));
  /* БЕЗ нарезки по секундам. Раньше здесь стояло recorder.start(1000),
     и это тихо портило каждый выгруженный ролик: получив срок нарезки,
     браузер отдаёт заголовок сразу первым куском и вернуться к нему,
     чтобы вписать длину, уже не может. В плеерах такой файл — «прямой
     эфир»: полоса времени бесполезна, перемотки нет, часть сервисов
     его не принимает. Замерено: с нарезкой длительность выходит
     Infinity, без неё — настоящие секунды.

     Памяти это не стоит ничего: куски мы и так копили до конца записи,
     чтобы собрать из них один файл. */
  /* Говорим главному процессу, что запись пошла: если страница
     погибнет посреди неё, в журнале будет не голое «страница погибла»,
     а качество, формат, сколько прошло и пик памяти. */
  сообщитьОЗаписи({
    идёт: true, качество: quality, формат: mime.startsWith('video/mp4') ? 'mp4' : 'webm',
    всегоС: Math.round(длинаРолика()), мб: Math.round(весРолика(quality) / 1048576),
  });
  recorder.start();
  ...
  const ticker = makeTicker(1000 / КАДРОВ_В_СЕКУНДУ);
  /* Сколько заняла прошлая отрисовка — из неё считается упреждение
     (см. УПРЕЖДЕНИЕ ниже). Первый кадр идёт без поправки на отрисовку:
     мерить ещё нечего. */
  let рисовалось = 0;
  let finished = false;
  const stop = () => {
  ...
  ticker.onTick = () => {
    ...
    /* ---- Упреждение: кадр рисуем не «на сейчас», а на то мгновение,
       которое он и правда покажет.
       ...
       караоке». Так и было. Замерено щелчком в звуке против метки в кадре:
       картинка отставала от звука на 10–45 мс — всегда в одну сторону.

       Отстаёт она вот почему: кадр, нарисованный в такт T, висит в ролике
       до следующего такта, то есть показывает песню в среднем на полкадра
       позже, чем она есть. Это и есть весь систематический сдвиг —
       семнадцать миллисекунд при тридцати кадрах.

       Цену отрисовки добавляем сверху не из теории, а на всякий случай:
       метку кадру ставят по концу отрисовки, и на медленной машине это
       может стать заметным. Замерено на этой: даже 2K с фотографией фона,
       кеглем 145 % и размытием стоит меньше миллисекунды, так что вся
       поправка сейчас — те самые полкадра. Считается она на ходу как раз
       затем, чтобы там, где отрисовка дорога, поправка выросла сама.

       На управление записью поправка не влияет: остановка и полоса хода
       по-прежнему идут по настоящему pos. */
    const время = pos + ПОЛКАДРА + рисовалось;
    const началоОтрисовки = performance.now();
    /* Похвала выходит через пару секунд после последнего слова —
       так же, как на экране, — и дальше стоит до конца песни, пока
       играет проигрыш. Если песня обрывается сразу за словом,
       дописываем похвале её время: звука в этом хвосте нет, дорожки
       кончились вместе с песней, и это верно — аплодисменты мы
       не подделываем. */
    const всяДлина = Math.max(duration, финалС + ФИНАЛ_В_КАДРЕ);
    if (pos >= всяДлина) { stop(); return; }
    if (время >= финалС) {
      /* Знак в углу в финале не рисуем: надпись и логотип и так
         во весь экран, второй раз то же самое в углу — шум. */
      drawФиналКадра(g2d, W, H, bgImg, время - финалС, финалКадра, логоФинала);
    } else {
      // Время НЕ прижимаем: на подводке оно отрицательное, и ровно
      // по нему кадр рисует начальный отсчёт
      drawVideoFrame(g2d, W, H, bgImg, время, watermark);
    }
    /* Цену этой отрисовки запоминаем для следующего кадра. Потолок в две
       десятых — на случай разовой задержки (окно свернули, машина ушла
       думать): без него одна такая заминка увела бы картинку вперёд. */
    рисовалось = Math.min(0.2, (performance.now() - началоОтрисовки) / 1000);
    /* Наружу — для самопроверки: раздел «упреждениеКадра» смотрит,
       что поправка жива и не выродилась в ноль (см. desktop/main.js). */
    videoExport.упреждение = ПОЛКАДРА + рисовалось;
...
document.addEventListener('keydown', (e) => {
  ...
  /* Пробел в рабочем месте — ВСЕГДА пуск и пауза, даже когда фокус
     остался на кнопке.

     Беда, о которой сказал человек: «ставлю паузу, нажимаю магнит,
     хочу так же пробелом запустить трек, а он снова нажимает магнит».
     Так и было: щелчок оставляет на кнопке фокус, а пробел по правилам
     браузера жмёт кнопку под фокусом — и вместо игры переключался
     магнит. В монтажных программах пробел не отнимает у себя ничто:
     он транспорт, и точка.

     Исключения только там, где пробел — это буква: поля ввода
     и правка текста. Список ещё держит открытым свой пробел: там
     он раскрывает выбор, и отнимать его незачем. Кнопки жмутся
     по-прежнему клавишей Enter. */
  const пишут = наборТекста(active) || tag === 'SELECT';
  if (пишут) return;
...
/* Значок прав у кнопки «Сохранить видео»: подсказка говорит коротко,
   щелчок открывает руководство, где то же самое написано целиком. */
const кнопкаПрав = $('btn-rights');
if (кнопкаПрав) {
  кнопкаПрав.addEventListener('click', () => {
    if (вПриложении()) открытьРуководство(true);
...
/* ============================================================
   ...
   Три темы монтажной оболочки студии (см. .studio в style.css):
   «фирменная» (signature, по умолчанию) — прежний синевато-чёрный
   грунт и зелёный ровно в четырёх местах; «нейтральная» (neutral) —
   серый грунт без оттенка и жёлтая рамка выделения; «стальная»
   (steel) — холодный серый грунт, панели светлее фона, синий акцент
   и заголовки полос темнее рабочего поля, манера больших монтажных
   программ. Устроено как язык: атрибут на <html>, ключ в localStorage,
   переключатель собирается кодом и стоит в шапке рядом с языковым.
...
/* Какой значок у какой темы: фирменная — палитра (в ней и есть цвет),
   нейтральная — круг, наполовину залитый серым, стальная — два фейдера.
   Названия тем с кнопки ушли: слова в шапке занимали больше места, чем
   переключатель языка рядом, а прочитать их нужно ровно один раз.
   Теперь они стоят в пунктах списка, в подсказке и в aria-label —
   там, где их ищут. */
const ЗНАЧОК_ТЕМЫ = { signature: 'palette', neutral: 'contrast', steel: 'faders' };

/* ---------- Выпадающий переключатель: общее устройство ----------

   Их в шапке два — тема и язык, — и устроены они одинаково: одна
   кнопка со значком нынешнего выбора и уголком, а по нажатию — список
   с названиями и галкой у выбранного. Обойма из кнопок подряд годилась,
   пока выбор был из двух, но тем стало три, а два переключателя рядом
   разного устройства читались как разные органы управления, хотя
   делают одно и то же.

   Поэтому устройство здесь одно на оба, а темы и языки только
   подставляют своё: значок, названия, подсказки и что делать с выбором.
   Классы у собранных узлов двойные — общие `pick-*` (их читают стили
   и этот код) и свои `theme-*` / `lang-*` (по ним переключатель находят
   снаружи: самопроверка, ручная проверка в консоли).

   С клавиатуры работает как обычное меню: на кнопке Enter, пробел
   или ↓ открывают список и ставят фокус на выбранный пункт, в списке
   ↑ и ↓ ходят по пунктам по кругу, Enter выбирает, Esc закрывает
   и возвращает фокус на кнопку. Щёлкнули мимо — тоже закрывается.

   Открыт список или нет, спрашиваем у самой разметки (`aria-expanded`
   у кнопки), а не помним переменной: список пересобирается на каждой
   смене языка и темы, и переменная пережила бы узлы, которых уже нет. */

function списокВиден(узел) {
  const кнопка = узел.querySelector('.pick-btn');
  return !!кнопка && кнопка.getAttribute('aria-expanded') === 'true';
}

function показатьСписок(узел, да) {
  const кнопка = узел.querySelector('.pick-btn');
  const список = узел.querySelector('.pick-menu');
  if (!кнопка || !список) return;
  // Два открытых списка разом ни к чему: в шапке они стоят вплотную
  if (да) закрытьСписки(узел);
  список.classList.toggle('hidden', !да);
  кнопка.setAttribute('aria-expanded', да ? 'true' : 'false');
  if (да) {
    const текущий = список.querySelector('.pick-item[aria-checked="true"]')
      || список.querySelector('.pick-item');
    if (текущий) текущий.focus();
  }
}

function закрытьСписки(кроме) {
  document.querySelectorAll('.pick-switch').forEach((узел) => {
    if (узел !== кроме) показатьСписок(узел, false);
  });
}

/* Соседний пункт по кругу: с последнего ↓ ведёт на первый, а с первого
   ↑ — на последний. Так ходят меню в системе, и на двух-трёх пунктах
   упереться в край было бы особенно обидно. */
function перейтиПоСписку(список, откуда, шаг) {
  const пункты = [...список.querySelectorAll('.pick-item')];
  if (!пункты.length) return;
  const i = пункты.indexOf(откуда);
  const к = ((i < 0 ? 0 : i + шаг) + пункты.length) % пункты.length;
  пункты[к].focus();
}

/* Собрать переключатель во всех гнёздах, какие найдутся. Гнездо одно
   (шапка), но перебираем все: разметка не должна диктовать их число.

   о.гнездо   — селектор гнезда (.theme-switch, .lang-switch);
   о.свой     — что дописать к общим классам (theme, lang);
   о.поле     — имя data-атрибута с кодом выбора (dataset.theme/.lang);
   о.коды     — что показывать пунктами;
   о.выбран   — какой код выбран сейчас;
   о.выбрать  — что сделать по щелчку по пункту;
   о.значок   — значок кода (у языка он один на все пункты);
   о.подпись, о.подсказка — название пункта и его подсказка;
   о.подсказкаКнопки — подсказка самой кнопки.

   Имя списка (aria-label) берётся с гнезда: там оно уже стоит
   в разметке и переводится через data-i18n-aria — второму списку
   надписей взяться неоткуда. */
function собратьВыпадающий(о) {
  document.querySelectorAll(о.гнездо).forEach((узел) => {
    узел.textContent = '';
    узел.classList.add('pick-switch');
    const выбранный = о.выбран();

    const кнопка = document.createElement('button');
    кнопка.type = 'button';
    кнопка.className = `pick-btn ${о.свой}-btn`;
    кнопка.dataset[о.поле] = выбранный;
    кнопка.setAttribute('aria-haspopup', 'menu');
    кнопка.setAttribute('aria-expanded', 'false');
    кнопка.appendChild(значокSVG(о.значок(выбранный)));
    кнопка.appendChild(значокSVG('caret', 'pick-caret'));
    кнопка.title = о.подсказкаКнопки(выбранный);
    кнопка.setAttribute('aria-label', кнопка.title);
    узел.appendChild(кнопка);

    const список = document.createElement('div');
    список.className = `pick-menu ${о.свой}-menu hidden`;
    список.setAttribute('role', 'menu');
    список.setAttribute('aria-label', узел.getAttribute('aria-label') || '');
    о.коды.forEach((код) => {
      const пункт = document.createElement('button');
      пункт.type = 'button';
      пункт.className = `pick-item ${о.свой}-item`;
      пункт.setAttribute('role', 'menuitemradio');
      пункт.dataset[о.поле] = код;
      пункт.appendChild(значокSVG(о.значок(код)));
      const подпись = document.createElement('span');
      подпись.textContent = о.подпись(код);
      пункт.appendChild(подпись);
      пункт.appendChild(значокSVG('check', 'pick-tick'));
      пункт.title = о.подсказка(код);
      пункт.setAttribute('aria-label', пункт.title);
      пункт.setAttribute('aria-checked', код === выбранный ? 'true' : 'false');
      пункт.addEventListener('click', () => {
        /* Порядок важен: сначала закрываем (пока список ещё тот самый),
           потом применяем выбор — он пересобирает переключатель, и
           прежние узлы уходят вместе со списком. Фокус после этого
           возвращаем на свежую кнопку, иначе он падал бы на body. */
        показатьСписок(узел, false);
        о.выбрать(код);
        const свежая = узел.querySelector('.pick-btn');
        if (свежая) свежая.focus();
      });
      пункт.addEventListener('keydown', (e) => {
        if (e.key === 'ArrowDown') { e.preventDefault(); перейтиПоСписку(список, пункт, 1); }
        else if (e.key === 'ArrowUp') { e.preventDefault(); перейтиПоСписку(список, пункт, -1); }
        else if (e.key === 'Escape') { e.preventDefault(); показатьСписок(узел, false); кнопка.focus(); }
        else if (e.key === 'Tab') показатьСписок(узел, false);
      });
      список.appendChild(пункт);
    });
    узел.appendChild(список);

    кнопка.addEventListener('click', () => показатьСписок(узел, !списокВиден(узел)));
    кнопка.addEventListener('keydown', (e) => {
      if (e.key === 'ArrowDown' || e.key === 'ArrowUp') {
        e.preventDefault();
        показатьСписок(узел, true);
      }
    });
  });
}

/* Щёлкнули мимо — закрываем. Ловим на всём документе: щелчок по самой
   кнопке или по пункту доходит сюда тоже, поэтому проверяем, из
   переключателя ли он пришёл. */
document.addEventListener('pointerdown', (e) => {
  if (e.target && e.target.closest && e.target.closest('.pick-switch')) return;
  закрытьСписки(null);
});
/* Esc закрывает список откуда угодно: фокус мог уехать с пункта
   (скажем, обратно на кнопку) — а список так и остался бы висеть */
document.addEventListener('keydown', (e) => {
  if (e.key !== 'Escape') return;
  const открытый = document.querySelector('.pick-switch:has(.pick-btn[aria-expanded="true"])');
  if (!открытый) return;
  показатьСписок(открытый, false);
  const кнопка = открытый.querySelector('.pick-btn');
  if (кнопка) кнопка.focus();
});
...
function собратьПереключателиТемы() {
  собратьВыпадающий({
    ...
    коды: ТЕМЫ,
    выбран: () => тема,
    выбрать: установитьТему,
    значок: (код) => ЗНАЧОК_ТЕМЫ[код] || 'palette',
    подпись: (код) => t('тема.' + код),
    подсказка: (код) => t('тема.' + код + '.полно'),
    подсказкаКнопки: (код) => t('тема.выбрать', { 'имя': t('тема.' + код) }),
  });
}
...
/* Переключатель языка — выпадающий список, ровно того же устройства,
   что и переключатель темы рядом (см. собратьВыпадающий выше). Раньше
   это была пара кнопок «Рус» и «Eng»: устройство своё, а дело то же
   самое, и два соседних органа управления читались как разные.

   Значок — глобус, а не флаг: флаг обозначает страну, а не язык, и
   английским пользуются далеко не только в одной из них. Названия
   языков собираются кодом: подписи одинаково читаются на любом языке,
   и держать их в разметке незачем. Место одно — шапка, но перебираем
   все .lang-switch, чтобы разметка не диктовала их число. */
function собратьПереключателиЯзыка() {
  собратьВыпадающий({
    ...
    коды: I18N.ЯЗЫКИ,
    выбран: () => I18N.язык(),
    выбрать: (код) => I18N.установить(код),
    // Значок у всех пунктов один: рисовать язык картинкой нечем,
    // а флаги — не про язык. Название пункта и говорит, какой он.
    значок: () => 'globe',
    подпись: (код) => t('язык.' + код + '.имя'),
    подсказка: (код) => t('язык.' + код + '.полно'),
    подсказкаКнопки: (код) => t('язык.выбрать', { 'имя': t('язык.' + код + '.имя') }),
  });
}
...
(function init() {
  ...
  applyStyle();
  // Фон ставим через setBgImage: он же обновляет просмотр и кнопки
