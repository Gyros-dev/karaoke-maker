...
async function exportVideo() {
  ...
  /* Похвала и счёт для хвоста записи. Выбираются ОДИН РАЗ на всю
     запись: меняйся они каждый кадр — вместо похвалы вышло бы
     мельтешение. Язык — по песне, как и на экране.

     Выходит она через пару секунд после ПОСЛЕДНЕГО СЛОВА, а не после
     песни: у песни бывает проигрыш на полминуты, и всё это время
     в кадре стояла бы допетая строка. */
  const язык = языкПесни();
  const финалС = времяФинала() != null ? времяФинала() : duration;
  const финалКадра = {
    язык,
