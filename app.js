...
/* Песня доиграла сама. Ровно три оговорки, и каждая про то, чтобы
   праздник не вылез там, где его не ждут. */
function песняКончилась() {
  if (!$('step-4') || !$('step-4').classList.contains('active')) return;
  if (videoExport.active) return;      // идёт запись — в кадр это не должно попасть
  if (audio.stopAt != null) return;    // играли отрывок, а не песню
  показатьФинал();
}
